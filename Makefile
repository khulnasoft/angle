...
# Override this to automatically enter a container containing the correct, full, official build environment for Angle, ready for development
export ENVIRONMENT ?= false
# The upstream container we publish artifacts to on a successful master build.
export ENVIRONMENT_UPSTREAM ?= docker.io/timberio/angle-dev:sha-3eadc96742a33754a5859203b58249f6a806972a
...
define ENVIRONMENT_EXEC
	...
	$(CONTAINER_TOOL) run \
			...
			--mount type=bind,source=${CURRENT_DIR},target=/git/khulnasoft/angle \
			$(if $(findstring docker,$(CONTAINER_TOOL)),--mount type=bind$(COMMA)source=/var/run/docker.sock$(COMMA)target=/var/run/docker.sock,) \
			--mount type=volume,source=angle-target,target=/git/khulnasoft/angle/target \
			--mount type=volume,source=angle-cargo-cache,target=/root/.cargo \
			--mount type=volume,source=angle-rustup-cache,target=/root/.rustup \
...
.PHONY: environment
environment: export ENVIRONMENT_TTY = true ## Enter a full Angle dev shell in $CONTAINER_TOOL, binding this folder to the container.
environment:
	${ENVIRONMENT_EXEC}

.PHONY: environment-prepare
environment-prepare: ## Prepare the Angle dev shell using $CONTAINER_TOOL.
	${ENVIRONMENT_PREPARE}

.PHONY: environment-clean
environment-clean: ## Clean the Angle dev shell using $CONTAINER_TOOL.
	@$(CONTAINER_TOOL) volume rm -f angle-target angle-cargo-cache angle-rustup-cache
...
build-x86_64-unknown-linux-gnu: target/x86_64-unknown-linux-gnu/release/angle ## Build a release binary for the x86_64-unknown-linux-gnu triple.
	@echo "Output to ${<}"

.PHONY: build-aarch64-unknown-linux-gnu
build-aarch64-unknown-linux-gnu: target/aarch64-unknown-linux-gnu/release/angle ## Build a release binary for the aarch64-unknown-linux-gnu triple.
	@echo "Output to ${<}"

.PHONY: build-x86_64-unknown-linux-musl
build-x86_64-unknown-linux-musl: target/x86_64-unknown-linux-musl/release/angle ## Build a release binary for the x86_64-unknown-linux-musl triple.
	@echo "Output to ${<}"

.PHONY: build-aarch64-unknown-linux-musl
build-aarch64-unknown-linux-musl: target/aarch64-unknown-linux-musl/release/angle ## Build a release binary for the aarch64-unknown-linux-musl triple.
	@echo "Output to ${<}"

.PHONY: build-armv7-unknown-linux-gnueabihf
build-armv7-unknown-linux-gnueabihf: target/armv7-unknown-linux-gnueabihf/release/angle ## Build a release binary for the armv7-unknown-linux-gnueabihf triple.
	@echo "Output to ${<}"

.PHONY: build-armv7-unknown-linux-musleabihf
build-armv7-unknown-linux-musleabihf: target/armv7-unknown-linux-musleabihf/release/angle ## Build a release binary for the armv7-unknown-linux-musleabihf triple.
	@echo "Output to ${<}"

.PHONY: build-graphql-schema
build-graphql-schema: ## Generate the `schema.json` for Angle's GraphQL API
...
target/%/angle: export PAIR =$(subst /, ,$(@:target/%/angle=%))
target/%/angle: export TRIPLE ?=$(word 1,${PAIR})
target/%/angle: export PROFILE ?=$(word 2,${PAIR})
target/%/angle: export CFLAGS += -g0 -O3
target/%/angle: cargo-install-cross CARGO_HANDLES_FRESHNESS
	$(MAKE) -k cross-image-${TRIPLE}
	cross build \
		$(if $(findstring release,$(PROFILE)),--release,) \
		--target ${TRIPLE} \
...
target/%/angle.tar.gz: export PAIR =$(subst /, ,$(@:target/%/angle.tar.gz=%))
target/%/angle.tar.gz: export TRIPLE ?=$(word 1,${PAIR})
target/%/angle.tar.gz: export PROFILE ?=$(word 2,${PAIR})
target/%/angle.tar.gz: target/%/angle CARGO_HANDLES_FRESHNESS
	rm -rf target/scratch/angle-${TRIPLE} || true
	mkdir -p target/scratch/angle-${TRIPLE}/bin target/scratch/angle-${TRIPLE}/etc
	cp -R -f -v \
		target/${TRIPLE}/${PROFILE}/angle \
		target/scratch/angle-${TRIPLE}/bin/angle
	cp -R -f -v \
		README.md \
		LICENSE \
		licenses \
		...
		config \
		target/scratch/angle-${TRIPLE}/
	cp -R -f -v \
		distribution/systemd \
		target/scratch/angle-${TRIPLE}/etc/
	tar --create \
		--gzip \
		--verbose \
		--file target/${TRIPLE}/${PROFILE}/angle.tar.gz \
...
.PHONY: check-version
check-version: ## Check that Angle's version is correct accounting for recent changes
...
.PHONY: check-events
check-events: ## Check that events satisfy patterns set in https://github.com/khulnasoft/angle/blob/master/rfcs/2020-03-17-2064-event-driven-observability.md
...
##@ Rustdoc
build-rustdoc: ## Build Angle's Rustdocs
	# This command is mostly intended for use by the build process in khulnasoft/angle-rustdoc
	${MAYBE_ENVIRONMENT_EXEC} cargo doc --no-deps --workspace

##@ Packaging

# archives
target/artifacts/angle-${VERSION}-%.tar.gz: export TRIPLE :=$(@:target/artifacts/angle-${VERSION}-%.tar.gz=%)
target/artifacts/angle-${VERSION}-%.tar.gz: override PROFILE =release
target/artifacts/angle-${VERSION}-%.tar.gz: target/%/release/angle.tar.gz
...
.PHONY: package
package: build ## Build the Angle archive
...
package-x86_64-unknown-linux-gnu: target/artifacts/angle-${VERSION}-x86_64-unknown-linux-gnu.tar.gz ## Build an archive suitable for the `x86_64-unknown-linux-gnu` triple.
	@echo "Output to ${<}."

.PHONY: package-x86_64-unknown-linux-musl
package-x86_64-unknown-linux-musl: target/artifacts/angle-${VERSION}-x86_64-unknown-linux-musl.tar.gz ## Build an archive suitable for the `x86_64-unknown-linux-musl` triple.
	@echo "Output to ${<}."

.PHONY: package-aarch64-unknown-linux-musl
package-aarch64-unknown-linux-musl: target/artifacts/angle-${VERSION}-aarch64-unknown-linux-musl.tar.gz ## Build an archive suitable for the `aarch64-unknown-linux-musl` triple.
	@echo "Output to ${<}."

.PHONY: package-aarch64-unknown-linux-gnu
package-aarch64-unknown-linux-gnu: target/artifacts/angle-${VERSION}-aarch64-unknown-linux-gnu.tar.gz ## Build an archive suitable for the `aarch64-unknown-linux-gnu` triple.
	@echo "Output to ${<}."

.PHONY: package-armv7-unknown-linux-gnueabihf
package-armv7-unknown-linux-gnueabihf: target/artifacts/angle-${VERSION}-armv7-unknown-linux-gnueabihf.tar.gz ## Build an archive suitable for the `armv7-unknown-linux-gnueabihf` triple.
	@echo "Output to ${<}."

.PHONY: package-armv7-unknown-linux-musleabihf
package-armv7-unknown-linux-musleabihf: target/artifacts/angle-${VERSION}-armv7-unknown-linux-musleabihf.tar.gz ## Build an archive suitable for the `armv7-unknown-linux-musleabihf triple.
	@echo "Output to ${<}."
...
package-deb-x86_64-unknown-linux-gnu: package-x86_64-unknown-linux-gnu ## Build the x86_64 GNU deb package
	$(CONTAINER_TOOL) run -v  $(PWD):/git/khulnasoft/angle/ -e TARGET=x86_64-unknown-linux-gnu -e ANGLE_VERSION $(ENVIRONMENT_UPSTREAM) cargo vdev package deb
...
package-deb-x86_64-unknown-linux-musl: package-x86_64-unknown-linux-musl ## Build the x86_64 GNU deb package
	$(CONTAINER_TOOL) run -v  $(PWD):/git/khulnasoft/angle/ -e TARGET=x86_64-unknown-linux-musl -e ANGLE_VERSION $(ENVIRONMENT_UPSTREAM) cargo vdev package deb

.PHONY: package-deb-aarch64
package-deb-aarch64: package-aarch64-unknown-linux-gnu ## Build the aarch64 deb package
	$(CONTAINER_TOOL) run -v  $(PWD):/git/khulnasoft/angle/ -e TARGET=aarch64-unknown-linux-gnu -e ANGLE_VERSION $(ENVIRONMENT_UPSTREAM) cargo vdev package deb
...
package-deb-armv7-gnu: package-armv7-unknown-linux-gnueabihf ## Build the armv7-unknown-linux-gnueabihf deb package
	$(CONTAINER_TOOL) run -v  $(PWD):/git/khulnasoft/angle/ -e TARGET=armv7-unknown-linux-gnueabihf -e ANGLE_VERSION $(ENVIRONMENT_UPSTREAM) cargo vdev package deb
...
package-rpm-x86_64-unknown-linux-gnu: package-x86_64-unknown-linux-gnu ## Build the x86_64 rpm package
	$(CONTAINER_TOOL) run -v  $(PWD):/git/khulnasoft/angle/ -e TARGET=x86_64-unknown-linux-gnu -e ANGLE_VERSION $(ENVIRONMENT_UPSTREAM) cargo vdev package rpm
...
package-rpm-x86_64-unknown-linux-musl: package-x86_64-unknown-linux-musl ## Build the x86_64 musl rpm package
	$(CONTAINER_TOOL) run -v  $(PWD):/git/khulnasoft/angle/ -e TARGET=x86_64-unknown-linux-musl -e ANGLE_VERSION $(ENVIRONMENT_UPSTREAM) cargo vdev package rpm

.PHONY: package-rpm-aarch64
package-rpm-aarch64: package-aarch64-unknown-linux-gnu ## Build the aarch64 rpm package
	$(CONTAINER_TOOL) run -v  $(PWD):/git/khulnasoft/angle/ -e TARGET=aarch64-unknown-linux-gnu -e ANGLE_VERSION $(ENVIRONMENT_UPSTREAM) cargo vdev package rpm
...
package-rpm-armv7hl-gnu: package-armv7-unknown-linux-gnueabihf ## Build the armv7hl-unknown-linux-gnueabihf rpm package
	$(CONTAINER_TOOL) run -v  $(PWD):/git/khulnasoft/angle/ -e TARGET=armv7-unknown-linux-gnueabihf -e ARCH=armv7hl -e ANGLE_VERSION $(ENVIRONMENT_UPSTREAM) cargo vdev package rpm

##@ Releasing

.PHONY: release
release: release-prepare generate release-commit ## Release a new Angle version
...
release-homebrew: ## Release to khulnasoft Homebrew tap
...
.PHONY: release-push
release-push: ## Push new Angle version
...
.PHONY: sync-install
sync-install: ## Sync the install.sh script for access via sh.angle.dev
	@aws s3 cp distribution/install.sh s3://sh.angle.dev --sse --acl public-read

.PHONY: sha256sum
sha256sum: ## Generate SHA256 checksums of CI artifacts
	scripts/checksum.sh

##@ Angle Remap Language
...
generate-component-docs: ## Generate per-component Cue docs from the configuration schema.
	...
	target/debug/angle generate-schema > /tmp/angle-config-schema.json 2>/dev/null
	${MAYBE_ENVIRONMENT_EXEC} cargo vdev build component-docs /tmp/angle-config-schema.json \
...
.PHONY: version
version: ## Get the current Angle version
	@cargo vdev version

.PHONY: git-hooks
git-hooks: ## Add Angle-local git hooks for commit sign-off
...
ci-generate-publish-metadata: ## Generates the necessary metadata required for building/publishing Angle.
