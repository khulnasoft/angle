...
fn main() {
    ...
    #[cfg(feature = "protobuf-build")]
    {
        ...
        tonic_build::configure()
            ...
            .compile_with_config(
                prost_build,
                &[
                    "lib/angle-core/proto/event.proto",
                    "proto/dnstap.proto",
    ...
    let rust_version = tracker
        ...
        .expect("Cargo-provided environment variables should always exist!");
    let build_desc = tracker.get_env_var("ANGLE_BUILD_DESC");

    // Get the git short hash of the HEAD.
    // Note that if Angle is compiled within a container, proper git permissions must be set for
    // the repo directory.
    // In CI build workflows this will have been pre-configured by running the command
    // "git config --global --add safe.directory /git/khulnasoft/angle", from the vdev package
    // subcommands.
    ...
    constants.add_required_constant("DEBUG", "Level of debug info for Angle.", debug);
