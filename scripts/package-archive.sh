...
# ENV VARS
#
#   $OVERWRITE      overwrite Angle binary even if it already exists (default "true")
...
ARCHIVE_VERSION="${ANGLE_VERSION:-"$(cargo vdev version)"}"
...
ARCHIVE_DIR_NAME="angle-$TARGET"
ARCHIVE_DIR="$TARGET_DIR/$ARCHIVE_DIR_NAME"
ARCHIVE_NAME="angle-$ARCHIVE_VERSION-$TARGET.$ARCHIVE_TYPE"
...
echo "Packaging the Angle archive"
...
# Copy the angle binary to /bin

mkdir -p "$ARCHIVE_DIR/bin"
cp -av "$TARGET_DIR/release/angle" "$ARCHIVE_DIR/bin"
...
if [[ $TARGET == *linux* ]]; then
  ...
  cp -av distribution/systemd/angle.service "$ARCHIVE_DIR/etc/systemd"
  mkdir -p "$ARCHIVE_DIR/etc/init.d"
  cp -av distribution/init.d/angle "$ARCHIVE_DIR/etc/init.d"
fi
...
(
  ...
  if [ "$ARCHIVE_TYPE" == "tar.gz" ]; then
    ...
    powershell '$progressPreference = "silentlyContinue"; Compress-Archive -DestinationPath angle-'"$ARCHIVE_VERSION"'-'"$TARGET"'.'"$ARCHIVE_TYPE"' -Path "./'"$ARCHIVE_DIR_NAME"'/*"'
  else
