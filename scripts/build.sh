...
#   $FEATURES       a list of Angle features to include when building (default "default")
...
BINARY_PATH="$TARGET_DIR/release/angle"
...
if [ -f "$BINARY_PATH" ] && [ "$OVERWRITE" == "false" ]; then
  echo "Angle binary already exists at:"
  echo ""
  echo "    $BINARY_PATH"
  echo ""
