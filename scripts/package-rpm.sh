...
PACKAGE_VERSION="${ANGLE_VERSION:-"$(cargo vdev version)"}"
ARCHIVE_NAME="angle-$PACKAGE_VERSION-$TARGET.tar.gz"
...
cp -av "$ARCHIVE_PATH" "$RPMBUILD_DIR/SOURCES/angle-$ARCH.tar.gz"
...
rpmbuild \
  ...
  --define "_arch $ARCH" \
  --nodebuginfo \
  -ba distribution/rpm/angle.spec
...
mv -v "$RPMBUILD_DIR/RPMS/$ARCH/angle-$CLEANED_VERSION-$RELEASE.$ARCH.rpm" "target/artifacts/angle-${CLEANED_VERSION}-${RELEASE}.${ARCH}.rpm"
