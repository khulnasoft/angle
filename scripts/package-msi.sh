...
cp target/artifacts/angle-"${ARCHIVE_VERSION}"-x86_64-pc-windows-msvc.zip target/msi-x64
pushd target/msi-x64
# shellcheck disable=SC2016
powershell '$progressPreference = "silentlyContinue"; Expand-Archive angle-'"$ARCHIVE_VERSION"'-x86_64-pc-windows-msvc.zip'
...
#     C:\a\angle\angle\target\msi-x64\angle.wxs(6) : error CNDL0108 : The Product/@Version attribute's value, '0.29.0.custom.a28ecdc', is not a valid version.
...
popd
cp target/msi-x64/angle.msi target/artifacts/angle-"${ARCHIVE_VERSION}"-x64.msi
