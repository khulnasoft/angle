...
def create_release_file!(new_version)
  release_log_path = "#{RELEASE_REFERENCE_DIR}/#{new_version}.log"
  git_log = Angle::GitLogCommit.from_file!(release_log_path)
  commits = Angle::Commit.from_git_log!(git_log)
...
def get_commits_since(last_version)
  Angle::Commit.fetch_since(last_version)
end
...
def get_existing_commits!
  releases = Angle::Release.all!(RELEASE_REFERENCE_DIR)
  ...
  log_commits =
    release_log_paths.collect do |release_log_path|
      git_log = Angle::GitLogCommit.from_file!(release_log_path)
      Angle::Commit.from_git_log!(git_log)
    end.flatten
