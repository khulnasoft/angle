...
ANGLE_TEST_KUBECTL="${ANGLE_TEST_KUBECTL:-"kubectl"}"

# Allow overriding helm with a custom command.
ANGLE_TEST_HELM="${ANGLE_TEST_HELM:-"helm"}"
...
up() {
  # A Angle container image to use.
  CONTAINER_IMAGE="${CONTAINER_IMAGE:?"You must assign CONTAINER_IMAGE variable with the Angle container image name"}"

  $ANGLE_TEST_HELM repo add "$CUSTOM_HELM_REPO_LOCAL_NAME" "$CHART_REPO" --force-update || true
  $ANGLE_TEST_HELM repo update

  $ANGLE_TEST_KUBECTL create namespace "$NAMESPACE" --dry-run -o yaml | $ANGLE_TEST_KUBECTL apply -f -

  if [[ -n "$CUSTOM_RESOURCE_CONFIGS_FILE" ]]; then
    $ANGLE_TEST_KUBECTL create --namespace "$NAMESPACE" -f "$CUSTOM_RESOURCE_CONFIGS_FILE"
  fi
...
down() {
  if [[ -n "$CUSTOM_RESOURCE_CONFIGS_FILE" ]]; then
    $ANGLE_TEST_KUBECTL delete --namespace "$NAMESPACE" -f "$CUSTOM_RESOURCE_CONFIGS_FILE"
  fi

  if $ANGLE_TEST_HELM status --namespace "$NAMESPACE" "$HELM_CHART" &>/dev/null; then
    $ANGLE_TEST_HELM delete --namespace "$NAMESPACE" "$HELM_CHART"
  fi

  if $ANGLE_TEST_KUBECTL get namespace "$NAMESPACE" &>/dev/null; then
    $ANGLE_TEST_KUBECTL delete namespace "$NAMESPACE"
  fi
}
