...
fn main() -> ExitCode {
    ...
    {
        use crate::angle::internal_telemetry::allocations::{
            init_allocation_tracing, REPORTING_INTERVAL_MS, TRACK_ALLOCATIONS,
        };
        use std::sync::atomic::Ordering;
        let opts = angle::cli::Opts::get_matches()
            .map_err(|error| {
...
pub fn main() -> ExitCode {
    // We need to be able to run angle in User Interactive mode. We first try
    // to run angle as a service. If we fail, we consider that we are in
    ...
    let exit_code = angle::angle_windows::run()
