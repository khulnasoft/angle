...
pub struct Opts {
    /// Angle config files in TOML format to test.
    #[arg(id = "config-toml", long, value_delimiter(','))]
    paths_toml: Vec<PathBuf>,

    /// Angle config files in JSON format to test.
    #[arg(id = "config-json", long, value_delimiter(','))]
    paths_json: Vec<PathBuf>,

    /// Angle config files in YAML format to test.
    #[arg(id = "config-yaml", long, value_delimiter(','))]
    paths_yaml: Vec<PathBuf>,

    /// Any number of Angle config files to test. If none are specified the
    /// default config path `/etc/angle/angle.yaml` will be targeted.
    #[arg(value_delimiter(','))]
    paths: Vec<PathBuf>,
