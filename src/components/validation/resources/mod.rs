...
use angle_lib::{config::DataType, event::Event};
...
impl ResourceCodec {
    ...
    pub fn into_decoder(&self) -> angle_lib::Result<Decoder> {
...
fn deserializer_config_to_serializer(config: &DeserializerConfig) -> encoding::Serializer {
    let serializer_config = match config {
        ...
        DeserializerConfig::Protobuf(config) => {
            SerializerConfig::Protobuf(angle_lib::codecs::encoding::ProtobufSerializerConfig {
                protobuf: angle_lib::codecs::encoding::ProtobufSerializerOptions {
...
fn serializer_config_to_deserializer(
...
) -> angle_lib::Result<decoding::Deserializer> {
    let deserializer_config = match config {
        ...
        SerializerConfig::Protobuf(config) => {
            DeserializerConfig::Protobuf(angle_lib::codecs::decoding::ProtobufDeserializerConfig {
                protobuf: angle_lib::codecs::decoding::ProtobufDeserializerOptions {
                    desc_file: config.protobuf.desc_file.clone(),
                    message_type: config.protobuf.message_type.clone(),
                },
            })
