use crate::{
    ...
    sources::angle::AngleConfig as AngleSourceConfig,
    test_util::next_addr,
};
...
fn build_input_edge() -> (InputEdge, impl Into<BoxedSource>) {
    ...
    let input_source = AngleSourceConfig::from_address(input_listen_addr.as_socket_addr());
...
fn build_output_edge() -> (OutputEdge, impl Into<BoxedSink>) {
    ...
    let mut output_sink = AngleSinkConfig::from_address(output_listen_addr.as_uri());
