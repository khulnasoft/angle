...
use crate::{
    ...
    proto::angle::Server as AngleServer,
};
use crate::{
    config::ConfigBuilder,
    sinks::angle::AngleConfig as AngleSinkConfig,
...
const ANGLE_SINK_KEY: &str = "_telemetry_out";
...
pub struct Telemetry {
    ...
    service: AngleServer<EventForwardService>,
...
impl Telemetry {
    ...
    pub fn attach_to_config(config_builder: &mut ConfigBuilder) -> Self {
        ...
        // Attach an internal logs and internal metrics source, and send them on to a dedicated Angle
        ...
        let mut angle_sink = AngleSinkConfig::from_address(listen_addr.as_uri());
        ...
        angle_sink.batch.timeout_secs = Some(0.1);
        angle_sink.request.retry_attempts = 0;
        ...
        Self {
            listen_addr,
            service: AngleServer::new(EventForwardService::from(tx)),
            rx,
        }
    }
    ...
    pub async fn into_collector(
        ...
        // from the Angle sink that we're using to collect telemetry.
