...
use angle_lib::event::{Event, Metric};
...
///
/// [component_spec]: https://github.com/khulnasoft/angle/blob/master/docs/specs/component.md
#[derive(Default)]
...
fn filter_events_by_metric_and_component<'a>(
    ...
    let metrics: Vec<&Metric> = telemetry_events
        .iter()
        .flat_map(|e| {
            if let angle_lib::event::Event::Metric(m) = e {
                Some(m)
            } else {
                None
            }
