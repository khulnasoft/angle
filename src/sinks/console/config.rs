...
use crate::{
    ...
    sinks::{console::sink::WriterSink, Healthcheck, AngleSink},
};
...
impl SinkConfig for ConsoleSinkConfig {
    async fn build(&self, _cx: SinkContext) -> crate::Result<(AngleSink, Healthcheck)> {
        ...
        let sink: AngleSink = match self.target {
            Target::Stdout => AngleSink::from_event_streamsink(WriterSink {
                output: io::stdout(),
                transformer,
                encoder,
            }),
            Target::Stderr => AngleSink::from_event_streamsink(WriterSink {
                output: io::stderr(),
                transformer,
                encoder,
            }),
