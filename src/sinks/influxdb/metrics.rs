...
impl InfluxDbSvc {
    pub fn new(config: InfluxDbConfig, client: HttpClient) -> crate::Result<AngleSink> {
        ...
        #[allow(deprecated)]
        Ok(AngleSink::from_event_sink(sink))
    }
}
...
mod tests {
    ...
    fn test_encode_counter() {
        ...
        let line_protocols = encode_events(ProtocolVersion::V2, events, Some("angle"), None, &[]);
        assert_eq!(
            line_protocols,
    ...
    fn test_encode_histogram_v1() {
        let events = vec![Metric::new(
            ...
            MetricKind::Absolute,
            MetricValue::AggregatedHistogram {
                buckets: angle_lib::buckets![1.0 => 1, 2.1 => 2, 3.0 => 3],
                count: 6,
                sum: 12.5,
            },
        )
    ...
    fn test_encode_distribution_zero_counts_stats() {
        let events = vec![Metric::new(
            ...
            MetricKind::Incremental,
            MetricValue::Distribution {
                samples: angle_lib::samples![1.0 => 0, 2.0 => 0],
                statistic: StatisticKind::Histogram,
            },
        )
        .with_namespace(Some("ns"))
    ...
    fn test_encode_with_some_tags() {
        ...
        assert_eq!(
            line_protocols[0],
            "angle.cpu,datacenter=us-east,host=local,metric_type=gauge value=2.5 1542182950000000011"
        );
        assert_eq!(
            line_protocols[1],
            "angle.mem,datacenter=us-east,host=local,metric_type=gauge,normal_tag=value,true_tag=true value=1000 1542182950000000011"
        );
    }
}
