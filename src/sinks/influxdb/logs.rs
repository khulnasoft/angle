...
use angle_lib::lookup::lookup_v2::OptionalValuePath;
use angle_lib::lookup::PathPrefix;
use angle_lib::schema;
...
pub struct InfluxDbLogsConfig {
    ...
    ///
    /// When specified, the measurement name is `<namespace>.angle`.
    ///
    #[configurable(
    ...
    #[configurable(metadata(docs::examples = "angle-logs"))]
    ...
    ///
    /// By default Angle adds `metric_type` as well as the configured `log_schema.host_key` and
    ...
    // `host_key`, `message_key`, and `source_type_key` are `Option` as we want `angle generate`
...
impl SinkConfig for InfluxDbLogsConfig {
    async fn build(&self, cx: SinkContext) -> crate::Result<(AngleSink, Healthcheck)> {
        ...
        #[allow(deprecated)]
        Ok((AngleSink::from_event_sink(sink), healthcheck))
    }
            ...
            (None, Some(namespace)) => {
                warn!(
                    "Option `namespace` has been deprecated. Use `measurement` instead. \
                       For example, you can use `measurement=<namespace>.angle` for the \
                       same effect."
                );
                Ok(format!("{}.angle", namespace))
            }
...
mod tests {
    ...
    use angle_lib::event::{BatchNotifier, BatchStatus, Event, LogEvent};
    ...
    fn test_config_measurement_from_namespace() {
        ...
        assert_eq!("ns.angle", sink_config.get_measurement().unwrap());
    }
    ...
    fn test_encode_event_apply_rules() {
        ...
        assert_eq!("angle", line_protocol.0);
    ...
    fn assert_line_protocol(i: i64, value: Option<&str>) {
        //angle,metric_type=logs key0="value0",message="message_value" 1000000000
...
mod integration_tests {
    ...
    use angle_lib::codecs::BytesDeserializerConfig;
    use angle_lib::config::{LegacyKey, LogNamespace};
    use angle_lib::event::{BatchNotifier, BatchStatus, Event, LogEvent};
    use angle_lib::lookup::{owned_value_path, path};
    ...
    async fn influxdb2_logs_put_data() {
        ...
        LogNamespace::Angle.insert_source_metadata(
            "file",
            &mut namespaced_log,
        ...
        LogNamespace::Angle.insert_standard_angle_source_metadata(
            &mut namespaced_log,
            "file",
            now,
        );
        let schema = BytesDeserializerConfig
            .schema_definition(LogNamespace::Angle)
            .with_metadata_field(
