...
use angle_lib::event::{KeyString, MetricTags};
...
pub struct InfluxDb1Settings {
    ...
    #[configurable(metadata(docs::examples = "angle-database"))]
    ...
    #[configurable(metadata(docs::examples = "angle-source"))]
...
pub struct InfluxDb2Settings {
    ...
    ///
    /// Only relevant when using InfluxDB v2.x and above.
    #[configurable(metadata(docs::examples = "angle-bucket"))]
...
mod tests {
    ...
    fn test_influxdb1_test_write_uri() {
        ...
        let uri = settings
            ...
            .unwrap();
        assert_eq!("http://localhost:8086/write?consistency=quorum&db=angle_db&rp=autogen&p=secret&u=writer&precision=ns", uri.to_string())
    }
