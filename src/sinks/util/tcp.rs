...
use angle_lib::json_size::JsonSize;
use angle_lib::{ByteSizeOf, EstimatedJsonEncodedSizeOf};
...
impl TcpSinkConfig {
    ...
    pub fn build(
        ...
        encoder: impl Encoder<Event, Error = angle_lib::codecs::encoding::Error>
            + Clone
            + Send
            + Sync
            + 'static,
    ) -> crate::Result<(AngleSink, Healthcheck)> {
        ...
        Ok((
            AngleSink::from_event_streamsink(sink),
...
struct TcpSink<E>
where
    E: Encoder<Event, Error = angle_lib::codecs::encoding::Error> + Clone + Send + Sync,
{
...
impl<E> TcpSink<E>
where
    E: Encoder<Event, Error = angle_lib::codecs::encoding::Error> + Clone + Send + Sync + 'static,
{
...
where
    E: Encoder<Event, Error = angle_lib::codecs::encoding::Error>
        + Clone
        + Send
        + Sync
        + Sync
