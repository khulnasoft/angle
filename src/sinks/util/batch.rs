...
use angle_lib::stream::BatcherSettings;
...
impl<D: SinkBatchSettings + Clone> BatchConfig<D, Unmerged> {
    ...
    ///
    /// `BatcherSettings` is effectively the `angle_core` spiritual successor of
    ...
    /// `angle_core`, and use that instead of `BatcherSettings`.
