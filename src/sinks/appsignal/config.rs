...
use crate::{
    ...
    sinks::{
        ...
        BuildError, Healthcheck, HealthcheckError, AngleSink,
    },
};
...
impl AppsignalConfig {
    ...
    pub(super) fn build_sink(&self, http_client: HttpClient) -> crate::Result<AngleSink> {
        let batch_settings = self.batch.into_batcher_settings()?;

        let endpoint = endpoint_uri(&self.endpoint, "angle/events")?;
...
impl SinkConfig for AppsignalConfig {
    async fn build(&self, cx: SinkContext) -> crate::Result<(AngleSink, Healthcheck)> {
        ...
        let healthcheck = healthcheck(
            endpoint_uri(&self.endpoint, "angle/healthcheck")?,
...
mod test {
    ...
    fn endpoint_uri_with_path() {
        let uri = endpoint_uri("https://appsignal-endpoint.net", "angle/events");
        assert_eq!(
            uri.expect("Not a valid URI").to_string(),
            "https://appsignal-endpoint.net/angle/events"
        );
    }
