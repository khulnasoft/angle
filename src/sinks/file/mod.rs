...
pub struct FileSinkConfig {
    ...
    #[configurable(metadata(docs::examples = "/tmp/angle-%Y-%m-%d.log"))]
    #[configurable(metadata(
        docs::examples = "/tmp/application-{{ application_id }}-%Y-%m-%d.log"
    ))]
    #[configurable(metadata(docs::examples = "/tmp/angle-%Y-%m-%d.log.zst"))]
    pub path: Template,
...
impl GenerateConfig for FileSinkConfig {
    fn generate_config() -> toml::Value {
        toml::Value::try_from(Self {
            path: Template::try_from("/tmp/angle-%Y-%m-%d.log").unwrap(),
...
impl SinkConfig for FileSinkConfig {
    async fn build(
        ...
        Ok((
            super::AngleSink::from_event_streamsink(sink),
            future::ok(()).boxed(),
        ))
    }
...
mod tests {
    ...
    use angle_lib::{event::LogEvent, sink::AngleSink};
