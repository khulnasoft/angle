...
use angle_lib::lookup::event_path;
use angle_lib::lookup::lookup_v2::ConfigValuePath;
use angle_lib::schema::Requirement;
use vrl::value::Kind;
...
fn query_examples() -> HashMap<String, String> {
    HashMap::<_, _>::from_iter([("X-Powered-By".to_owned(), "Angle".to_owned())])
}
...
fn default_index() -> Template {
    Template::try_from("angle-%Y.%m.%d").expect("unable to parse template")
}
...
impl SinkConfig for ElasticsearchConfig {
    async fn build(&self, cx: SinkContext) -> crate::Result<(AngleSink, Healthcheck)> {
        ...
        let stream = AngleSink::from_event_streamsink(sink);
