...
use snafu::ResultExt;
use angle_lib::codecs::{encoding::SerializerConfig, TextSerializerConfig};
use angle_lib::config::DataType;
use angle_lib::lookup::lookup_v2::OptionalTargetPath;
...
impl SinkConfig for PulsarSinkConfig {
    async fn build(&self, _cx: SinkContext) -> crate::Result<(AngleSink, Healthcheck)> {
        ...
        Ok((AngleSink::from_event_streamsink(sink), hc))
    }
