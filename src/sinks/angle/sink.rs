...
use angle_lib::stream::{batcher::data::BatchReduce, BatcherSettings, DriverResponse};
use angle_lib::{config::telemetry, ByteSizeOf, EstimatedJsonEncodedSizeOf};

use super::service::AngleRequest;
use crate::{
...
impl<S> AngleSink<S>
where
    S: Service<AngleRequest> + Send + 'static,
    ...
    async fn run_inner(self: Box<Self>, input: BoxStream<'_, Event>) -> Result<(), ()> {
        input
            ...
            .map(|event_collection| {
                ...
                let encoded_events = proto_angle::PushEventsRequest {
...
#[async_trait]
impl<S> StreamSink<Event> for AngleSink<S>
where
    S: Service<AngleRequest> + Send + 'static,
    S::Future: Send + 'static,
    S::Response: DriverResponse + Send + 'static,
