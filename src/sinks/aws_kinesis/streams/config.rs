...
use snafu::Snafu;
use angle_lib::configurable::{component::GenerateConfig, configurable_component};
...
impl RetryLogic for KinesisRetryLogic {
    ...
    fn is_retriable_error(&self, error: &Self::Error) -> bool {
        if let SdkError::ServiceError(inner) = error {
            ...
            // partition fail but the others do not, for example), Angle
            // does not retry. This line only covers a failure for the entire
            // request.
            //
            // https://github.com/khulnasoft/angle/issues/359
