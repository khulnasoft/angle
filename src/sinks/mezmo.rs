...
impl SinkConfig for LogdnaConfig {
    async fn build(
        &self,
        cx: SinkContext,
    ) -> crate::Result<(super::AngleSink, super::Healthcheck)> {
...
impl SinkConfig for MezmoConfig {
    async fn build(
        ...
        #[allow(deprecated)]
        Ok((super::AngleSink::from_event_sink(sink), healthcheck))
    }
...
mod tests {
    ...
    fn encode_event() {
        ...
        event1.as_mut_log().insert("app", "notangle");
        event1.as_mut_log().insert("magic", "angle");
        ...
        assert_eq!(event1_out.get("app").unwrap(), &json!("notangle"));
        assert_eq!(event2_out.get("file").unwrap(), &json!("log.txt"));
        assert_eq!(event3_out.get("app").unwrap(), &json!("angle"));
        ...
        assert_sink_compliance(&HTTP_SINK_TAGS, async {
            ...
            for _ in 0..partitions.len() {
                ...
                for (i, line) in output.iter().enumerate() {
                    ...
                    assert_eq!(line.get("app").unwrap(), &json!("angle"));
