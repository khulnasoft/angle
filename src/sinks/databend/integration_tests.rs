...
use angle_lib::event::{BatchNotifier, BatchStatus, BatchStatusReceiver, Event, LogEvent};
...
fn databend_user() -> String {
    std::env::var("DATABEND_USER").unwrap_or_else(|_| "angle".into())
}

fn databend_password() -> String {
    std::env::var("DATABEND_PASSWORD").unwrap_or_else(|_| "angle".into())
}
