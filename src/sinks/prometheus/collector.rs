...
use angle_lib::event::metric::{samples_to_buckets, MetricSketch, MetricTags, Quantile};
use angle_lib::prometheus::parser::{proto, METRIC_NAME_LABEL};
...
mod tests {
    ...
    fn encodes_counter_text() {
        assert_eq!(
            ...
            indoc! { r#"
                # HELP angle_hits hits
                # TYPE angle_hits counter
                angle_hits{code="200"} 10 1612325106789
            "#}
        );
    }
    ...
    fn encodes_counter_request() {
        assert_eq!(
            ...
            write_request!("angle_hits", "hits", Counter ["" @ 1612325106789 = 10.0 ["code" => "200"]])
        );
    }
    ...
    fn encode_counter<T: MetricCollector>() -> T::Output {
        ...
        encode_one::<T>(Some("angle"), &[], &[], &metric)
    }
    ...
    fn encodes_gauge_text() {
        assert_eq!(
            ...
            indoc! { r#"
                # HELP angle_temperature temperature
                # TYPE angle_temperature gauge
                angle_temperature{code="200"} -1.1 1612325106789
            "#}
        );
    }
    ...
    fn encodes_gauge_request() {
        assert_eq!(
            ...
            write_request!("angle_temperature", "temperature", Gauge ["" @ 1612325106789 = -1.1 ["code" => "200"]])
        );
    }
    ...
    fn encodes_set_text() {
        assert_eq!(
            ...
            indoc! { r#"
                # HELP angle_users users
                # TYPE angle_users gauge
                angle_users 1 1612325106789
            "#}
        );
    }
    ...
    fn encodes_set_request() {
        assert_eq!(
            ...
            write_request!("angle_users", "users", Gauge [ "" @ 1612325106789 = 1.0 []])
        );
    }
    ...
    fn encodes_expired_set_request() {
        assert_eq!(
            ...
            write_request!("angle_users", "users", Gauge ["" @ 1612325106789 = 0.0 []])
        );
    }
    ...
    fn encodes_distribution_text() {
        assert_eq!(
            ...
            indoc! {r#"
                # HELP angle_requests requests
                # TYPE angle_requests histogram
                angle_requests_bucket{le="0"} 0 1612325106789
                angle_requests_bucket{le="2.5"} 6 1612325106789
                angle_requests_bucket{le="5"} 8 1612325106789
                angle_requests_bucket{le="+Inf"} 8 1612325106789
                angle_requests_sum 15 1612325106789
                angle_requests_count 8 1612325106789
            "#}
        );
    }
    ...
    fn encodes_distribution_request() {
        assert_eq!(
            ...
            write_request!(
                "angle_requests", "requests", Histogram [
    ...
    fn encode_distribution<T: MetricCollector>() -> T::Output {
        let metric = Metric::new(
            ...
            MetricKind::Absolute,
            MetricValue::Distribution {
                samples: angle_lib::samples![1.0 => 3, 2.0 => 3, 3.0 => 2],
                statistic: StatisticKind::Histogram,
            },
        )
        .with_timestamp(Some(timestamp()));
        encode_one::<T>(Some("angle"), &[0.0, 2.5, 5.0], &[], &metric)
    }
    ...
    fn encodes_histogram_text() {
        assert_eq!(
            ...
            indoc! {r#"
                # HELP angle_requests requests
                # TYPE angle_requests histogram
                angle_requests_bucket{le="1"} 1 1612325106789
                angle_requests_bucket{le="2.1"} 3 1612325106789
                angle_requests_bucket{le="3"} 6 1612325106789
                angle_requests_bucket{le="+Inf"} 6 1612325106789
                angle_requests_sum 11.5 1612325106789
                angle_requests_count 6 1612325106789
            "#}
        );
    }
    ...
    fn encode_summary<T: MetricCollector>() -> T::Output {
        let metric = Metric::new(
            ...
            MetricKind::Absolute,
            MetricValue::AggregatedSummary {
                quantiles: angle_lib::quantiles![0.01 => 1.5, 0.5 => 2.0, 0.99 => 3.0],
                count: 6,
                sum: 12.0,
            },
        )
