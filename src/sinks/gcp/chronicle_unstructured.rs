...
mod integration_tests {
    ...
    use angle_lib::event::{BatchNotifier, BatchStatus};
    ...
    async fn config_build(
        log_type: &str,
        auth_path: &str,
    ) -> crate::Result<(AngleSink, crate::sinks::Healthcheck)> {
    ...
    async fn publish_events() {
        ...
        let (sink, healthcheck) =
            config_build(&log_type, "/home/angle/scripts/integration/gcp/auth.json")
                .await
    ...
    async fn invalid_credentials() {
        ...
        let sink = config_build(
            &log_type,
            "/home/angle/scripts/integration/gcp/invalidauth.json",
        )
        .await;
