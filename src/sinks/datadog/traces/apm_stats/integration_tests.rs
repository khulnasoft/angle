...
/// The port on which the Agent will send traces to angle, and angle `datadog_agent` source will
/// listen on
fn angle_receive_port() -> u16 {
    std::env::var("ANGLE_PORT")
...
/// The port for the http server to receive data from angle
const fn server_port_for_angle() -> u16 {
    1234
}
...
/// The agent url to post traces to [Agent -> Angle].
fn trace_agent_to_angle_url() -> String {
    std::env::var("TRACE_AGENT_TO_ANGLE_URL")
        .unwrap_or_else(|_| "http://127.0.0.1:8126/v0.3/traces".to_owned())
}
...
/// At a later time we could parse the trace payloads from angle and the agent and compare those
...
async fn receive_the_stats(
    ...
    rx_agent_angle: &mut Receiver<StatsPayload>,
) -> (StatsPayload, StatsPayload) {
    let timeout = sleep(Duration::from_secs(35));
    tokio::pin!(timeout);

    let mut stats_agent_angle = None;
    let mut stats_agent_only = None;

    // wait on the receive of stats payloads. expect one from agent, two from angle.
    // The second payload from angle should be the aggregate.
    loop {
        tokio::select! {
            d1 = rx_agent_angle.recv() => {
                stats_agent_angle = d1;
                if stats_agent_only.is_some() && stats_agent_angle.is_some() {
                    break;
                }
            },
            d2 = rx_agent_only.recv() => {
                stats_agent_only = d2;
                if stats_agent_angle.is_some() && stats_agent_only.is_some() {
                    break;
                }
            },
            _ = &mut timeout => break,
    ...
    assert!(
        stats_agent_angle.is_some(),
        "received no stats from angle"
    );
    assert!(stats_agent_only.is_some(), "received no stats from agent");

    (stats_agent_only.unwrap(), stats_agent_angle.unwrap())
}

/// Compares the stats payloads (specifically the bucket for the time window we sent events on)
/// between the Angle and Agent for consistency.
fn validate_stats(agent_stats: &StatsPayload, angle_stats: &StatsPayload) {
    let agent_bucket = agent_stats.stats.first().unwrap().stats.first().unwrap();

    let angle_bucket = angle_stats.stats.first().unwrap().stats.first().unwrap();
    ...
    assert!(
        agent_bucket.duration == angle_bucket.duration,
        "bucket durations do not match"
    );
    assert!(
        agent_bucket.stats.len() == angle_bucket.stats.len(),
        "angle and agent reporting different number of buckets"
    );

    let agent_s = agent_bucket.stats.first().unwrap();
    let angle_s = angle_bucket.stats.first().unwrap();

    info!("\nagent_stats : {:?}", agent_s);
    info!("\nangle_stats : {:?}", angle_s);

    assert!(agent_s.service == angle_s.service);
    assert!(agent_s.name == angle_s.name);
    assert!(agent_s.resource == angle_s.resource);
    assert!(agent_s.http_status_code == angle_s.http_status_code);
    assert!(agent_s.r#type == angle_s.r#type);
    assert!(agent_s.db_type == angle_s.db_type);

    // hit counts should match
    assert!(agent_s.hits == angle_s.hits);
    assert!(agent_s.hits == 2);

    assert!(agent_s.errors == angle_s.errors);
    assert!(agent_s.duration == angle_s.duration);
    assert!(agent_s.synthetics == angle_s.synthetics);
    assert!(agent_s.top_level_hits == angle_s.top_level_hits);
}

/// Starts the angle instance with a datadog agent source and a datadog traces sink.
...
async fn start_angle() -> (RunningTopology, ShutdownErrorReceiver) {
    let dd_agent_address = format!("0.0.0.0:{}", angle_receive_port());
    ...
    let dd_traces_endpoint = format!("http://127.0.0.1:{}", server_port_for_angle());
    let cfg = format!(
        indoc! { r#"
...
/// container feeds traces into the Datadog Agent source of Angle, which outputs to the traces
...
/// through angle. Consumers of the Agent's stats payloads expect the Aggregation of stats to be
...
#[tokio::test]
async fn apm_stats_e2e_test_dd_agent_to_angle_correctness() {
    trace_init();

    // channels for the servers to send us back data on
    let (tx_agent_angle, mut rx_agent_angle) = mpsc::channel(32);
    ...
    // starts the angle source and sink
    // panics if angle errors during startup
    let (_topology, _shutdown) = start_angle().await;

    // the URLs of the Agent trace endpoints that traces will be sent to
    let urls = vec![trace_agent_only_url(), trace_agent_to_angle_url()];
    ...
    let (stats_agent, stats_angle) =
        receive_the_stats(&mut rx_agent_only, &mut rx_agent_angle).await;

    // compare the stats from agent and angle for consistency
    validate_stats(&stats_agent, &stats_angle);
}
