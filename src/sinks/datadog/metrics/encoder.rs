...
// Given the angle source type, return the OriginService value associated with that integration, if any.
fn source_type_to_service(source_type: &str) -> Option<u32> {
    match source_type {
        ...
        // These are the sources for which metrics truly originated from this Angle instance.
        ...
        // However, if the upstream Angle instance did not set Origin Metadata (for example if it is an
        // older version version), we will at least set the OriginProduct and OriginCategory.
        "kafka" | "nats" | "redis" | "gcp_pubsub" | "http_client" | "http_server" | "angle" => {
            Some(0)
        }
...
fn generate_origin_metadata(
    ...
    // An upstream angle source or a transform has set the origin metadata already.
    // Currently this is only possible by these scenarios:
    //     - `datadog_agent` source receiving the metadata on ingested metrics
    //     - `angle` source receiving events with EventMetadata that already has the origins set
    //     - A metrics source configured with the `native` or `native_json` codecs, where the upstream
    //       Angle instance enriched the EventMetadata with Origin metadata.
...
mod tests {
    ...
    #[test]
    fn encode_origin_metadata_angle_sourced() {
