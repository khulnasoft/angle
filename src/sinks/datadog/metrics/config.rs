...
use crate::{
    ...
    sinks::{
        ...
        Healthcheck, UriParseSnafu, AngleSink,
    },
...
impl SeriesApiVersion {
    ...
    fn get_api_version() -> Self {
        ...
        *API_VERSION.get_or_init(|| {
            match std::env::var("ANGLE_TEMP_USE_DD_METRICS_SERIES_V2_API") {
                Ok(_) => Self::V2,
                Err(_) => Self::V1,
            }
        })
...
impl DatadogMetricsConfig {
    ...
    /// domain to indicate that these metrics are being submitted by Angle, including the version,
    /// likely useful for detecting if a specific version of the agent (Angle, in this case) is
    ...
    fn build_sink(&self, client: HttpClient) -> crate::Result<AngleSink> {
