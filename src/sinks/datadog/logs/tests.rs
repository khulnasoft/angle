...
/// runs random lines through it, returning a angle of the random lines and a
...
async fn smoke() {
    ...
    for (i, val) in output.iter().enumerate() {
        ...
        assert_eq!(val.0.headers.get("DD-EVP-ORIGIN").unwrap(), "angle");
...
///
/// Angle allows for flagging a global 'enterprise' context that indicates
...
async fn enterprise_headers_inner(api_status: ApiStatus) {
    let (mut config, mut cx) = load_sink::<DatadogLogsConfig>(indoc! {r#"
        ...
        "#})
    .unwrap();
    cx.app_name = "Angle Enterprise".to_string();
    cx.app_name_slug = "angle-enterprise".to_string();
...
async fn no_enterprise_headers_inner(api_status: ApiStatus) {
    ...
    assert_eq!(parts.headers.get("DD-EVP-ORIGIN").unwrap(), "angle");
    assert!(parts.headers.get("DD-EVP-ORIGIN-VERSION").is_some());
}
