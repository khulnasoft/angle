...
use futures::stream;
use angle_lib::encode_logfmt;
use angle_lib::lookup::{owned_value_path, PathPrefix};
use angle_lib::{
...
use crate::{
    ...
    event::Value,
    schema,
    sinks::{util::test::load_sink, AngleSink},
    template::Template,
    test_util::{
        components::{
...
async fn build_sink(codec: &str, remove_timestamp: bool) -> (uuid::Uuid, AngleSink) {
...
async fn build_sink_with_compression(codec: &str, compression: &str) -> (uuid::Uuid, AngleSink) {
    let stream = uuid::Uuid::new_v4();
...
/// This field is given the meaning of `timestamp` to ensure that in the Angle namespace
...
fn namespaced_timestamp_generator(
    ...
    move |index: usize| {
        ...
        // We need angle metadata for it to pick up that it is in the Angle namespace.
        LogNamespace::Angle.insert_standard_angle_source_metadata(&mut log, "loki", Utc::now());
...
// https://github.com/khulnasoft/angle/issues/7815
#[tokio::test]
