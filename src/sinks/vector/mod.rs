...
/// Marker type for the version two of the configuration for the `angle` sink.
...
pub enum AngleSinkError {
    #[snafu(display("Request failed: {}", source))]
    Request { source: tonic::Status },

    #[snafu(display("Angle source unhealthy: {:?}", status))]
...
mod tests {
    ...
    #[test]
    fn generate_config() {
        crate::test_util::test_generate_config::<AngleConfig>();
    }
    ...
    async fn run_sink_test(test_type: TestType) {
        ...
        let config: AngleConfig = toml::from_str(&config).unwrap();
        ...
        let output_lines = get_received(rx, |parts| {
            ...
            assert_eq!("/angle.Angle/PushEvents", parts.uri.path());
            assert_eq!(
                "application/grpc",
