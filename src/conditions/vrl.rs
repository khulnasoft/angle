...
use angle_lib::{compile_vrl, emit, TimeZone};
...
/// A condition that uses the [Angle Remap Language](https://angle.khulnasoft.com/docs/reference/vrl) (VRL) [boolean expression](https://angle.khulnasoft.com/docs/reference/vrl#boolean-expressions) against an event.
...
impl ConditionalConfig for VrlConfig {
    fn build(
        ...
        let functions = vrl::stdlib::all()
            .into_iter()
            .chain(angle_lib::enrichment::vrl_functions())
            .chain(angle_vrl_functions::all())
            .collect::<Vec<_>>();
