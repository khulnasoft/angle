...
use angle_lib::buffers::topology::channel::BufferSender;
use angle_lib::trigger::DisabledTrigger;
...
impl RunningTopology {
    ...
    pub async fn reload_config_and_respawn(&mut self, new_config: Config) -> Result<bool, ()> {
        ...
        if self.config.global != new_config.global {
            error!(
                message =
                "Global options can't be changed while reloading config file; reload aborted. Please restart Angle to reload the configuration file."
            );
            return Ok(false);
        }
        ...
        // Issue: https://github.com/khulnasoft/angle/issues/3035
        if cfg!(windows) {
