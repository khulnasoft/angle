...
struct InstallOpts {
    ...
    /// Angle config files in TOML format to be used by the service.
    #[arg(name = "config-toml", long, value_delimiter(','))]
    config_paths_toml: Vec<PathBuf>,

    /// Angle config files in JSON format to be used by the service.
    #[arg(name = "config-json", long, value_delimiter(','))]
    config_paths_json: Vec<PathBuf>,

    /// Angle config files in YAML format to be used by the service.
...
impl InstallOpts {
    fn service_info(&self) -> ServiceInfo {
        ...
        let display_name = self.display_name.as_deref().unwrap_or("Angle Service");
...
impl Default for ServiceInfo {
    fn default() -> Self {
        ...
        ServiceInfo {
            ...
            display_name: OsString::from("Angle Service"),
...
fn control_service(service: &ServiceInfo, action: ControlAction) -> exitcode::ExitCode {
    use crate::angle_windows;

    let service_definition = angle_windows::service_control::ServiceDefinition {
    ...
    let res = match action {
        ControlAction::Install => angle_windows::service_control::control(
            &service_definition,
            angle_windows::service_control::ControlAction::Install,
        ),
        ControlAction::Uninstall => angle_windows::service_control::control(
            &service_definition,
            angle_windows::service_control::ControlAction::Uninstall,
        ),
        ControlAction::Start => angle_windows::service_control::control(
            &service_definition,
            angle_windows::service_control::ControlAction::Start,
        ),
        ControlAction::Stop => angle_windows::service_control::control(
            &service_definition,
            angle_windows::service_control::ControlAction::Stop,
        ),
        ControlAction::Restart { stop_timeout } => angle_windows::service_control::control(
            &service_definition,
            angle_windows::service_control::ControlAction::Restart { stop_timeout },
        ),
    };
