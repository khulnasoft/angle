...
use angle_lib::api_client::{connect_subscription_client, Client};
...
/// Delay (in milliseconds) before attempting to reconnect to the Angle API
const RECONNECT_DELAY: u64 = 5000;

/// CLI command func for displaying Angle components, and communicating with a local/remote
/// Angle API server via HTTP/WebSockets
pub async fn cmd(opts: &super::Opts) -> exitcode::ExitCode {
    // Exit early if the terminal is not a teletype
    if !is_tty() {
        #[allow(clippy::print_stderr)]
        {
            eprintln!("Terminal must be a teletype (TTY) to display a Angle dashboard.");
        }
        return exitcode::IOERR;
    }

    let url = opts.url();
    // Create a new API client for connecting to the local/remote Angle instance.
    let client = Client::new(url.clone());
    #[allow(clippy::print_stderr)]
    if client.healthcheck().await.is_err() {
        eprintln!(
            indoc::indoc! {"
            Angle API server isn't reachable ({}).

            Have you enabled the API?

            To enable the API, add the following to your Angle config file:
