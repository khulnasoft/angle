...
use angle_lib::prometheus::parser::proto;
use angle_lib::prometheus::parser::{GroupKind, MetricGroup, ParserError};
...
pub(super) fn parse_text(packet: &str) -> Result<Vec<Event>, ParserError> {
    angle_lib::prometheus::parser::parse_text(packet).map(reparse_groups)
}

#[cfg(feature = "sources-prometheus-remote-write")]
pub(super) fn parse_request(request: proto::WriteRequest) -> Result<Vec<Event>, ParserError> {
    angle_lib::prometheus::parser::parse_request(request).map(reparse_groups)
}
...
mod test {
    ...
    fn test_histogram_out_of_order() {
        ...
        assert_event_data_eq!(
            ...
            Ok(vec![Metric::new(
                ...
                MetricKind::Absolute,
                MetricValue::AggregatedHistogram {
                    buckets: angle_lib::buckets![1.0 => 133988],
                    count: 144320,
                    sum: 53423.0,
                },
            )
    ...
    fn test_histogram_backward_values() {
        ...
        assert_event_data_eq!(
            ...
            Ok(vec![Metric::new(
                ...
                MetricKind::Absolute,
                MetricValue::AggregatedHistogram {
                    buckets: angle_lib::buckets![1.0 => 2000, 10.0 => 0],
                    count: 2000,
                    sum: 2000.0,
                },
            )
    ...
    // https://github.com/khulnasoft/angle/issues/3276
    #[test]
    fn test_nginx() {
        let exp = r#"
