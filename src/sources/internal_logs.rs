...
use angle_lib::lookup::{owned_value_path, path, OwnedValuePath};
use angle_lib::{
...
#[configurable_component(source(
    "internal_logs",
    "Expose internal log messages emitted by the running Angle instance."
))]
#[derive(Clone, Debug)]
...
mod tests {
    ...
    async fn run_test() {
        ...
        for (i, event) in events.iter().enumerate() {
            ...
            if i == 0 {
                assert!(log.get("angle.component_id").is_none());
                assert!(log.get("angle.component_kind").is_none());
                assert!(log.get("angle.component_type").is_none());
            } else if i < 3 {
                assert_eq!(log["angle.component_id"], "foo".into());
                assert_eq!(log["angle.component_kind"], "source".into());
                assert_eq!(log["angle.component_type"], "internal_logs".into());
            } else {
                ...
                assert_eq!(log["angle.component_id"], "foo".into());
                assert_eq!(log["angle.component_kind"], "bar".into());
                assert_eq!(log["angle.component_type"], "internal_logs".into());
                assert_eq!(log["angle.component_new_field"], "baz".into());
                assert_eq!(log["angle.component_numerical_field"], 1.into());
                assert!(log.get("angle.ignored_field").is_none());
            }
        }
    }
