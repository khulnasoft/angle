...
use angle_lib::codecs::{decoding::BoxedFramingError, CharacterDelimitedDecoder};
use angle_lib::configurable::configurable_component;
use angle_lib::lookup::{metadata_path, owned_value_path, path};
use angle_lib::{
...
pub struct JournaldConfig {
    ...
    #[serde(default)]
    #[configurable(metadata(docs::examples = "/var/lib/angle"))]
...
impl JournaldConfig {
    ...
    fn schema_definition(&self, log_namespace: LogNamespace) -> Definition {
        let schema_definition = match log_namespace {
            LogNamespace::Angle => Definition::new_with_default_metadata(
                Kind::bytes().or_null(),
                [LogNamespace::Angle],
            ),
...
mod tests {
    ...
    fn output_schema_definition_angle_namespace() {
        ...
        let expected_definition =
            Definition::new_with_default_metadata(Kind::bytes().or_null(), [LogNamespace::Angle])
                .with_metadata_field(
