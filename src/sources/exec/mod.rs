...
use angle_lib::{config::LegacyKey, EstimatedJsonEncodedSizeOf};
...
use angle_lib::config::{log_schema, LogNamespace};
...
fn handle_event(
    ...
    if let Event::Log(log) = event {
        log_namespace.insert_standard_angle_source_metadata(log, ExecConfig::NAME, Utc::now());
