...
use tracing::Span;
use angle_lib::internal_event::{CountByteSize, InternalEventHandle as _, Registered};
use angle_lib::lookup::lookup_v2::OptionalValuePath;
use angle_lib::lookup::{self, event_path, owned_value_path};
use angle_lib::sensitive_string::SensitiveString;
use angle_lib::{
...
use angle_lib::{configurable::configurable_component, tls::MaybeTlsIncomingStream};
...
impl SourceConfig for SplunkConfig {
    ...
    fn outputs(&self, global_log_namespace: LogNamespace) -> Vec<SourceOutput> {
        ...
        let schema_definition = match log_namespace {
            LogNamespace::Legacy => {
                let definition = angle_lib::schema::Definition::empty_legacy_namespace()
                    .with_event_field(
            ...
            LogNamespace::Angle => angle_lib::schema::Definition::new_with_default_metadata(
...
impl<'de, R: JsonRead<'de>> EventIterator<'de, R> {
    ...
    fn build_event(&mut self, mut json: JsonValue) -> Result<Event, Rejection> {
        ...
        let mut log = match self.log_namespace {
            LogNamespace::Angle => self.build_log_angle(&mut json)?,
            LogNamespace::Legacy => self.build_log_legacy(&mut json)?,
        };

        // Add source type
        self.log_namespace.insert_angle_metadata(
            &mut log,
    ...
    /// Build the log event for the angle namespace.
    /// In this namespace the log event is created entirely from the event field.
    /// No renaming of the `line` field is done.
    fn build_log_angle(&mut self, json: &mut JsonValue) -> Result<LogEvent, Rejection> {
...
fn raw_event(
    ...
    let mut log = match log_namespace {
        LogNamespace::Angle => LogEvent::from(message),
        LogNamespace::Legacy => {
            let mut log = LogEvent::default();
    ...
    log_namespace.insert_standard_angle_source_metadata(&mut log, SplunkConfig::NAME, Utc::now());
...
mod tests {
    ...
    use angle_lib::codecs::{JsonSerializerConfig, TextSerializerConfig};
    use angle_lib::sensitive_string::SensitiveString;
    use angle_lib::{event::EventStatus, schema::Definition};
    ...
    async fn start(
        ...
        compression: Compression,
        acknowledgements: Option<HecAcknowledgementsConfig>,
    ) -> (AngleSink, impl Stream<Item = Event> + Unpin) {
...
{"event":"first"}
    ...
    /// modified to pass.
    /// https://github.com/khulnasoft/angle/issues/7097
