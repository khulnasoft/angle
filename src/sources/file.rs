...
use angle_lib::codecs::{BytesDeserializer, BytesDeserializerConfig};
use angle_lib::configurable::configurable_component;
use angle_lib::file_source::{
...
use angle_lib::finalizer::OrderedFinalizer;
use angle_lib::lookup::{lookup_v2::OptionalValuePath, owned_value_path, path, OwnedValuePath};
use angle_lib::{
...
pub struct FileConfig {
    /// Array of file patterns to include. [Globbing](https://angle.khulnasoft.com/docs/reference/configuration/sources/file/#globbing) is supported.
    #[configurable(metadata(docs::examples = "/var/log/**/*.log"))]
    pub include: Vec<PathBuf>,

    /// Array of file patterns to exclude. [Globbing](https://angle.khulnasoft.com/docs/reference/configuration/sources/file/#globbing) is supported.
    ///
    ...
    /// Set to `""` to suppress this key.
    ///
    /// [global_host_key]: https://angle.khulnasoft.com/docs/reference/configuration/global-options/#log_schema.host_key
    ...
    ///
    /// [global_data_dir]: https://angle.khulnasoft.com/docs/reference/configuration/global-options/#data_dir
    #[serde(default)]
    #[configurable(metadata(docs::examples = "/var/local/lib/angle/"))]
        ...
        match config {
            FingerprintConfig::Checksum {
                ...
                let bytes = match bytes {
                    Some(bytes) => {
                        warn!(message = "The `fingerprint.bytes` option will be used to convert old file fingerprints created by angle < v0.11.0, but are not supported for new file fingerprints. The first line will be used instead.");
                        bytes
                    }
                    None => 256,
                };
...
fn create_event(
    ...
    log_namespace.insert_angle_metadata(
        &mut event,
...
mod tests {
    ...
    fn output_schema_definition_angle_namespace() {
        ...
        assert_eq!(
            definitions,
            Some(
                Definition::new_with_default_metadata(Kind::bytes(), [LogNamespace::Angle])
    ...
    #[test]
    fn create_event_angle_namespace() {
        let line = Bytes::from("hello world");
        let file = "some_file.rs";
        let offset: u64 = 0;
        ...
        let log = create_event(line, offset, file, &meta, LogNamespace::Angle, false);
        ...
        assert!(log
            .metadata()
            .value()
            .get(path!("angle", "ingest_timestamp"))
            .unwrap()
            .is_timestamp());
    ...
    // https://github.com/khulnasoft/angle/issues/8363
    #[tokio::test]
    ...
    // Ignoring on mac: https://github.com/khulnasoft/angle/issues/8373
    ...
    use angle_lib::lookup::OwnedTargetPath;
    use AckingMode::*;
