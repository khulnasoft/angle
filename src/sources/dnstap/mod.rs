...
impl DnstapConfig {
    ...
    pub fn schema_definition(&self, log_namespace: LogNamespace) -> angle_lib::schema::Definition {
        let event_schema = DnstapEventSchema;

        match log_namespace {
            LogNamespace::Legacy => {
                let schema = angle_lib::schema::Definition::empty_legacy_namespace();
            ...
            LogNamespace::Angle => {
                let schema = angle_lib::schema::Definition::new_with_default_metadata(
...
impl FrameHandler for DnstapFrameHandler {
     ...
     * Takes a data frame from the unix socket and turns it into a Angle Event.
     **/
...
mod integration_tests {
    ...
    fn get_container() -> String {
        std::env::var("CONTAINER_NAME").unwrap_or_else(|_| "angle_dnstap".into())
    }
