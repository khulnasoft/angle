...
use angle_lib::codecs::decoding::DeserializerConfig;
use angle_lib::config::{log_schema, LegacyKey, LogNamespace};
use angle_lib::configurable::configurable_component;
use angle_lib::lookup::{lookup_v2::OptionalValuePath, owned_value_path};
...
mod test {
    ...
    use angle_lib::codecs::NewlineDelimitedDecoderConfig;
    #[cfg(unix)]
    use angle_lib::codecs::{
        decoding::CharacterDelimitedDecoderOptions, CharacterDelimitedDecoderConfig,
    };
    use angle_lib::event::EventContainer;
    use angle_lib::lookup::{lookup_v2::OptionalValuePath, owned_value_path, path};
    ...
    #[tokio::test]
    async fn tcp_it_includes_angle_namespaced_fields() {
        assert_source_compliance(&SOCKET_PUSH_SOURCE_TAGS, async {
            let (tx, mut rx) = SourceSender::new_test();
            let addr = next_addr();
            ...
            assert_eq!(
                event_meta.get(path!("angle", "source_type")).unwrap(),
    ...
    async fn tcp_with_tls() {
        assert_source_compliance(&SOCKET_PUSH_SOURCE_TAGS, async {
            ...
            let tls_meta: ObjectMap = btreemap!(
                "subject" => "CN=localhost,OU=Angle,O=Datadog,L=New York,ST=New York,C=US"
            );
    ...
    #[tokio::test]
    async fn tcp_with_tls_angle_namespace() {
        assert_source_compliance(&SOCKET_PUSH_SOURCE_TAGS, async {
            let (tx, mut rx) = SourceSender::new_test();
            let addr = next_addr();
    ...
    async fn tcp_shutdown_infinite_stream() {
        ...
        impl tokio_util::codec::Encoder<Event> for Serializer {
            type Error = angle_lib::codecs::encoding::Error;
    ...
    async fn init_udp_inner(
        ...
        let config = if use_angle_namespace {
    ...
    #[tokio::test]
    async fn udp_it_includes_angle_namespaced_fields() {
        assert_source_compliance(&SOCKET_PUSH_SOURCE_TAGS, async {
            let (tx, rx) = SourceSender::new_test();
    ...
    ////////////// UNIX TEST LIBS //////////////
    #[cfg(unix)]
    async fn init_unix(sender: SourceSender, stream: bool, use_angle_namespace: bool) -> PathBuf {
        let in_path = tempfile::tempdir().unwrap().into_path().join("unix_test");

        let mut config = UnixConfig::new(in_path.clone());
        if use_angle_namespace {
            config.log_namespace = Some(true);
        }
    ...
    async fn unix_message(
        ...
        let (tx, rx) = SourceSender::new_test();
        let path = init_unix(tx, stream, use_angle_namespace).await;
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn unix_datagram_message_with_angle_namespace() {
        assert_source_compliance(&SOCKET_PUSH_SOURCE_TAGS, async {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn unix_stream_message_with_angle_namespace() {
        assert_source_compliance(&SOCKET_PUSH_SOURCE_TAGS, async {
