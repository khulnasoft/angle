...
pub struct KafkaSourceConfig {
    ...
    ///
    /// When Angle shuts down or the Kafka consumer group revokes partitions from this
...
impl ReceivedMessage {
    ...
    fn apply(&self, keys: &Keys, event: &mut Event, log_namespace: LogNamespace) {
        if let Event::Log(ref mut log) = event {
            match log_namespace {
                LogNamespace::Angle => {
                    // We'll only use this function in Angle namespaces because we don't want
                    ...
                    log_namespace.insert_standard_angle_source_metadata(
                        log,
...
mod test {
    ...
    pub(super) fn make_config(
        ...
        KafkaSourceConfig {
            ...
            log_namespace: Some(log_namespace == LogNamespace::Angle),
            ..Default::default()
        }
    }
    ...
    fn test_output_schema_definition_angle_namespace() {
        let definitions = make_config("topic", "group", LogNamespace::Angle, None)
...
mod integration_test {
    ...
    #[tokio::test]
    async fn consumes_event_with_acknowledgements_angle_namespace() {
        send_receive(true, |_| false, 10, LogNamespace::Angle).await;
    }
    ...
    #[tokio::test]
    async fn consumes_event_without_acknowledgements_angle_namespace() {
        send_receive(false, |_| false, 10, LogNamespace::Angle).await;
    }
    ...
    #[tokio::test]
    async fn handles_one_negative_acknowledgement_angle_namespace() {
        send_receive(true, |n| n == 2, 10, LogNamespace::Angle).await;
    }
    ...
    #[tokio::test]
    async fn handles_permanent_negative_acknowledgement_angle_namespace() {
        send_receive(true, |n| n >= 2, 2, LogNamespace::Angle).await;
    }
    ...
    async fn send_receive(
        ...
        for (i, event) in events.into_iter().enumerate() {
            if let LogNamespace::Legacy = log_namespace {
                ...
                assert_eq!(
                    meta.get(path!("angle", "source_type")).unwrap(),
