...
use angle_lib::{config::LogNamespace, ByteSizeOf, EstimatedJsonEncodedSizeOf};
...
#[configurable_component(source(
    "internal_metrics",
    "Expose internal metrics emitted by the running Angle instance."
))]
#[derive(Clone, Debug)]
...
impl SourceConfig for InternalMetricsConfig {
    async fn build(&self, cx: SourceContext) -> crate::Result<super::Source> {
        ...
        // namespace for created metrics is already "angle" by default.
    ...
    async fn run(mut self) -> Result<(), ()> {
        ...
        while interval.next().await.is_some() {
            ...
            let batch = metrics.into_iter().map(|mut metric| {
                // A metric starts out with a default "angle" namespace, but will be overridden
                // if an explicit namespace is provided to this source.
                if self.namespace != "angle" {
...
mod tests {
    ...
    async fn default_namespace() {
        ...
        assert_eq!(event.as_metric().namespace(), Some("angle"));
    }
