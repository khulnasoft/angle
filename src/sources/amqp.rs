...
use angle_lib::codecs::decoding::{DeserializerConfig, FramingConfig};
use angle_lib::configurable::configurable_component;
use angle_lib::lookup::{lookup_v2::OptionalValuePath, metadata_path, owned_value_path, path};
use angle_lib::{
...
impl AmqpSourceConfig {
    fn decoder(&self, log_namespace: LogNamespace) -> angle_lib::Result<Decoder> {
    ...
    let stream = stream! {
        while let Some(result) = stream.next().await {
            match result {
                ...
                Err(error) => {
                    use angle_lib::codecs::StreamDecodingError as _;
