...
pub struct DockerLogsConfig {
    ...
    #[configurable(metadata(
        docs::examples = "org.opencontainers.image.vendor=Angle",
...
impl DockerLogsSource {
    fn new(
        ...
        // t3 -- list_containers
        // In that case, logs between [t1,t2] will be pulled to angle only on next start/unpause of that container.
    ...
    fn new_event(
        ...
        match log_namespace {
            LogNamespace::Angle => {
                ...
                log.insert(metadata_path!("angle", "ingest_timestamp"), Utc::now());
            }
            LogNamespace::Legacy => {
                if let Some(timestamp) = timestamp {
...
fn line_agg_adapter(
    ...
    line_agg_out.map(move |(_, message, mut log)| {
        match log_namespace {
            LogNamespace::Angle => log.insert(event_path!(), message),
