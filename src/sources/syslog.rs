...
use angle_lib::lookup::{lookup_v2::OptionalValuePath, path, OwnedValuePath};
use vrl::event_path;
...
impl TcpSource for SyslogTcpSource {
    type Error = angle_lib::codecs::decoding::Error;
...
fn enrich_syslog_event(
    ...
    log_namespace.insert_standard_angle_source_metadata(log, SyslogConfig::NAME, Utc::now());
...
mod test {
    ...
    use angle_lib::lookup::{event_path, owned_value_path, OwnedTargetPath};
    ...
    use angle_lib::codecs::decoding::format::Deserializer;
    use angle_lib::lookup::PathPrefix;
    use angle_lib::{config::ComponentKey, schema::Definition};
    ...
    async fn test_tcp_syslog() {
        assert_source_compliance(&SOCKET_PUSH_SOURCE_TAGS, async {
            ...
            let output_messages: Vec<SyslogMessageRfc5424> = output_events
                .into_iter()
                .map(|mut e| {
                    e.as_mut_log().remove("hostname"); // Angle adds this field which will cause a parse error.
                    e.as_mut_log().remove("source_ip"); // Angle adds this field which will cause a parse error.
                    e.into()
                })
                .collect();
