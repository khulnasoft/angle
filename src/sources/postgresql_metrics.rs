...
use angle_lib::{metric_tags, ByteSizeOf, EstimatedJsonEncodedSizeOf};
...
pub struct PostgresqlMetricsConfig {
    ...
    #[configurable(metadata(
        docs::examples = "postgresql://postgres:angle@localhost:5432/postgres"
    ))]
...
mod integration_tests {
    ...
    fn pg_url() -> String {
        std::env::var("PG_URL")
            .unwrap_or_else(|_| format!("postgres://angle:angle@{}/postgres", pg_host()))
    }
    ...
    async fn test_local() {
        let endpoint = format!(
            "postgresql:///postgres?host={}&user=angle&password=angle",
