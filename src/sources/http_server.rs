...
impl SourceConfig for HttpConfig {
    async fn build(&self, cx: SourceContext) -> angle_lib::Result<super::Source> {
...
pub struct SimpleHttpConfig {
    ...
    #[configurable(metadata(docs::examples = "angle_http_path"))]
...
mod tests {
    ...
    use angle_lib::event::LogEvent;
    use angle_lib::lookup::lookup_v2::OptionalValuePath;
    use angle_lib::lookup::{event_path, owned_value_path, OwnedTargetPath, PathPrefix};
    ...
    async fn http_path() {
...

        {
            ...
            let log = event.as_log();
            assert_eq!(log["key1"], "value1".into());
            assert_eq!(log["angle_http_path"], "/event/path".into());
            assert!(log.get_timestamp().is_some());
            assert_eq!(
                *log.get_source_type().unwrap(),
                SimpleHttpConfig::NAME.into()
    ...
    async fn http_path_no_restriction() {
...

        {
            ...
            let log = event.as_log();
            assert_eq!(log["key1"], "value1".into());
            assert_eq!(log["angle_http_path"], "/event/path1".into());
            assert!(log.get_timestamp().is_some());
            assert_eq!(
                *log.get_source_type().unwrap(),
                SimpleHttpConfig::NAME.into()
        ...
        }
        {
            ...
            let log = event.as_log();
            assert_eq!(log["key2"], "value2".into());
            assert_eq!(log["angle_http_path"], "/event/path2".into());
            assert!(log.get_timestamp().is_some());
            assert_eq!(
                *log.get_source_type().unwrap(),
                SimpleHttpConfig::NAME.into()
