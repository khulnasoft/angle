...
use angle_lib::lookup::{event_path, metadata_path, owned_value_path, path, OwnedValuePath};
use angle_lib::{
...
impl TcpSource for LogstashSource {
    ...
    fn handle_events(&self, events: &mut [Event], host: SocketAddr) {
        ...
        for event in events {
            ...
            // Angle: always insert `ingest_timestamp`. Insert `timestamp` if found in event.
            //
            ...
            match self.log_namespace {
                LogNamespace::Angle => {
                    ...
                    log.insert(metadata_path!("angle", "ingest_timestamp"), now);
                }
                LogNamespace::Legacy => {
