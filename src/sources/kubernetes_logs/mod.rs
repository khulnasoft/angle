...
//! at "/var/log/pods" on the host of the Kubernetes Node when Angle itself is
...
use angle_lib::lookup::{lookup_v2::OptionalTargetPath, owned_value_path, path, OwnedTargetPath};
use angle_lib::{config::LegacyKey, config::LogNamespace, EstimatedJsonEncodedSizeOf};
use angle_lib::{
...
const SELF_NODE_NAME_ENV_KEY: &str = "ANGLE_SELF_NODE_NAME";
...
pub struct Config {
    ...
    /// [exclude]: https://angle.khulnasoft.com/docs/reference/configuration/sources/kubernetes_logs/#pod-exclusion
    ...
    /// [exclude]: https://angle.khulnasoft.com/docs/reference/configuration/sources/kubernetes_logs/#namespace-exclusion
    #[configurable(metadata(docs::examples = "my_custom_label!=my_value"))]
    #[configurable(metadata(
        docs::examples = "my_custom_label!=my_value,my_other_custom_label=my_value"
    ))]
...
impl Source {
    ...
    async fn run(
        ...
        // Stop Kubernetes object reflectors to avoid their leak on angle reload.
...
fn create_event(
    ...
    match (log_namespace, ingestion_timestamp_field) {
        // When using LogNamespace::Angle always set the ingest_timestamp.
        (LogNamespace::Angle, _) => {
            log.metadata_mut()
                .value_mut()
                .insert(path!("angle", "ingest_timestamp"), Utc::now());
        }
...
const fn default_max_line_bytes() -> usize {
    ...
    // https://github.com/khulnasoft/angle/issues/6967
    //
...
fn prepare_label_selector(selector: &str) -> String {
    const BUILT_IN: &str = "angle.khulnasoft.com/exclude!=true";
...
mod tests {
    ...
    use angle_lib::lookup::{owned_value_path, OwnedTargetPath};
    ...
    fn prepare_label_selector() {
        let cases = vec![
            (
                ...
                "angle.khulnasoft.com/exclude!=true",
            ),
            (
            ...
            (
                ...
                .extra_label_selector,
                "angle.khulnasoft.com/exclude!=true,qwe",
            ),
            (
                Config {
    ...
    #[test]
    fn test_output_schema_definition_angle_namespace() {
