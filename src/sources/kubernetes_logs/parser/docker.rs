...
use angle_lib::lookup::{self, path, OwnedTargetPath};
...
fn normalize_event(
    ...
    let timestamp_key = match log_namespace {
        LogNamespace::Angle => Some(OwnedTargetPath::metadata(lookup::owned_value_path!(
            "kubernetes_logs",
            "timestamp"
        ))),
...
pub mod tests {
    ...
    #[test]
    fn test_parsing_valid_angle_namespace() {
        trace_init();

        test_util::test_parser(
            || Docker {
                log_namespace: LogNamespace::Angle,
            },
            |bytes| Event::Log(LogEvent::from(value!(bytes))),
            valid_cases(LogNamespace::Angle),
        );
    }
    ...
    #[test]
    fn test_parsing_invalid_angle_namespace() {
        trace_init();

        let cases = invalid_cases();

        for bytes in cases {
            let mut parser = Docker::new(LogNamespace::Angle);
