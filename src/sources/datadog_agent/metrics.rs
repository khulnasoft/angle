...
/// Builds Angle's `EventMetadata` from the received metadata. Currently this is only
...
pub(crate) fn decode_ddseries_v2(
    ...
    let decoded_metrics: Vec<Event> = payload
        ...
        .flat_map(|serie| {
            ...
            //
            // Note that because Angle does not yet have a specific Metric type to handle Rate,
...
fn decode_datadog_series_v1(
    ...
    let decoded_metrics: Vec<Event> = metrics
        .series
        .into_iter()
        .flat_map(|m| into_angle_metric(m, api_key.clone(), schema_definition))
        .collect();
