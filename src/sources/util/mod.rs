...
pub mod framestream;
#[cfg(any(feature = "sources-angle", feature = "sources-opentelemetry"))]
pub mod grpc;
#[cfg(any(
...
/// Thus Angle expects only one colon character to be present per chunk, so the find()
...
pub fn extract_tag_key_and_value<S: AsRef<str>>(
    tag_chunk: S,
) -> (String, angle_lib::event::metric::TagValue) {
    use angle_lib::event::metric::TagValue;
