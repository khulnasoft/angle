...
/// Function used by the `angle config` subcommand for outputting a normalized configuration.
...
mod tests {
    ...
    use serde_json::json;
    use angle_lib::configurable::component::{
    ...
    fn include_defaults_does_not_include_env_vars() {
        let env_var = "ANGLE_CONFIG_INCLUDE_DEFAULTS_TEST";
        let env_var_in_arr = "ANGLE_CONFIG_INCLUDE_DEFAULTS_TEST_IN_ARR";
    ...
    fn create_config_source(sources: &[&str], transforms: &[&str], sinks: &[&str]) -> String {
        // This creates a string in the syntax expected by the `angle generate`
        // command whose internal mechanics we are using to create valid Angle
        // configurations.
        //
        ...
        // sources depending on the ordering of the `angle generate` input.
        //
