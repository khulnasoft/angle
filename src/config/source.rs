...
impl Configurable for BoxedSource {
    fn referenceable_name() -> Option<&'static str> {
        Some("angle::sources::Sources")
    }

    fn metadata() -> Metadata {
        let mut metadata = Metadata::default();
        metadata.set_description("Configurable sources in Angle.");
    ...
    fn generate_schema(gen: &RefCell<SchemaGenerator>) -> Result<SchemaObject, GenerateError> {
        angle_lib::configurable::component::SourceDescription::generate_schemas(gen)
    }
}
...
pub trait SourceConfig: DynClone + NamedComponent + core::fmt::Debug + Send + Sync {
    ...
    ///
    /// Generally, Angle uses acknowledgements to track when an event has finally been processed,
