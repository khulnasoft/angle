...
pub use angle_lib::configurable::component::{
...
pub fn init_log_schema(config_paths: &[ConfigPath], deny_if_set: bool) -> Result<(), Vec<String>> {
    ...
    angle_lib::config::init_log_schema(builder.global.log_schema, deny_if_set);
    Ok(())
}
...
pub struct HealthcheckOptions {
    ...
    ///
    /// When enabled and a sink reports not being healthy, Angle will exit during start-up.
    ///
...
mod resource_config_tests {
    use indoc::indoc;
    use angle_lib::configurable::schema::generate_root_schema;
