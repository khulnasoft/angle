...
use angle_lib::config::proxy::ProxyConfig;
...
pub struct Options {
    ...
    /// The maximum number of retries to report Angle's configuration to Observability Pipelines at startup.
        ...
        Self {
            data: PipelinesData {
                attributes: PipelinesAttributes {
                    ...
                    angle_version: fields.angle_version,
                    config,
                },
    ...
    T: Future<Output = ()> + Send + 'static,
{
    ...
    pub fn send(&self, reporting_task: T) {
        if let Err(err) = self.reporting_tx.send(reporting_task) {
            error!(
                %err,
                "Unable to report configuration due to internal Angle issue.",
            );
        }
    }
}
...
fn setup_logs_reporting(
    ...
    let angle_version = crate::angle_version();
    let tag_logs = RemapConfig {
        source: Some(format!(
            r#"
            .ddsource = "angle"
            .angle.configuration_key = "{configuration_key}"
            .angle.configuration_version_hash = "{configuration_version_hash}"
            .angle.version = "{angle_version}"
            {}
        "#,
...
fn setup_metrics_reporting(
    ...
    let host_metrics = HostMetricsConfig {
        namespace: Some("angle.host".to_owned()),
    ...
    let tag_metrics = RemapConfig {
        source: Some(format!(
            ...
            .tags.angle_version = "{angle_version}"
            {}
        "#,
...
pub(crate) fn report_configuration(
    ...
    let fut = async move {
        ...
        // Get the Angle version. This is reported to Pipelines along with a config hash.
        let angle_version = crate::get_version();

        // We need to create a JSON representation of config, based on the original files
        // that Angle was spawned with.
        ...
        // Create a HTTP client for posting a Angle version to Datadog OP. This will
        ...
        .await
        {
            Ok(()) => {
                info!(
                    "Angle config {} successfully reported to {}.",
            ...
            Err(err) => {
                error!(
                    err = ?err.to_string(),
                    "Could not report Angle config to {}.", DATADOG_REPORTING_PRODUCT
                );
            }
        }
    };
...
/// Returns the full URL endpoint of where to POST a Datadog Angle configuration.
...
/// Build a POST request for reporting a Angle config to Datadog OP.
fn build_request<'a>(
    endpoint: &Url,
...
/// Reports a JSON serialized Angle config to Datadog, for use with Observability Pipelines.
