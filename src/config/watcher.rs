...
pub fn spawn_thread<'a>(
    ...
    Err("Reloading config on Windows isn't currently supported. Related issue https://github.com/khulnasoft/angle/issues/938 .".into())
}
...
fn raise_sighup() {
    ...
    _ = signal::raise(signal::Signal::SIGHUP).map_err(|error| {
        error!(message = "Unable to reload configuration file. Restart Angle to reload it.", cause = %error)
    });
}
...
#[cfg(all(test, unix, not(target_os = "macos")))] // https://github.com/khulnasoft/angle/issues/5000
mod tests {
    ...
    async fn file_directory_update() {
        ...
        let file_path = dir.join("angle.toml");
