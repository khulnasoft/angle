...
use angle_lib::internal_event::DEFAULT_OUTPUT;
...
pub async fn check_buffer_preconditions(config: &Config) -> Result<(), Vec<String>> {
    // We need to assert that Angle's data directory is located on a mountpoint that has enough
    ...
    //
    // Notably, this does *not* cover other data usage by Angle on the same mountpoint because we
