...
use serde::Serialize;
use angle_lib::buffers::{BufferConfig, BufferType};
use angle_lib::configurable::attributes::CustomAttribute;
use angle_lib::configurable::schema::{SchemaGenerator, SchemaObject};
use angle_lib::configurable::{
...
impl Configurable for BoxedSink {
    fn referenceable_name() -> Option<&'static str> {
        Some("angle::sinks::Sinks")
    }

    fn metadata() -> Metadata {
        let mut metadata = Metadata::default();
        metadata.set_description("Configurable sinks in Angle.");
    ...
    fn generate_schema(gen: &RefCell<SchemaGenerator>) -> Result<SchemaObject, GenerateError> {
        angle_lib::configurable::component::SinkDescription::generate_schemas(gen)
    }
}
    ...
    T: Configurable + Serialize + 'static,
{
    ...
    #[serde(
        default,
        skip_serializing_if = "angle_lib::serde::skip_serializing_if_default"
    )]
...
pub struct SinkHealthcheckOptions {
    /// Whether or not to check the health of the sink when Angle starts up.
    pub enabled: bool,
...
pub trait SinkConfig: DynClone + NamedComponent + core::fmt::Debug + Send + Sync {
    ...
    /// Angle to determine the correct order for rebuilding a topology during configuration reload
