...
impl Configurable for BoxedTransform {
    fn referenceable_name() -> Option<&'static str> {
        Some("angle::transforms::Transforms")
    }

    fn metadata() -> Metadata {
        let mut metadata = Metadata::default();
        metadata.set_description("Configurable transforms in Angle.");
    ...
    fn generate_schema(gen: &RefCell<SchemaGenerator>) -> Result<SchemaObject, GenerateError> {
        angle_lib::configurable::component::TransformDescription::generate_schemas(gen)
    }
}
...
pub struct TransformContext {
    ...
    pub enrichment_tables: angle_lib::enrichment::TableRegistry,
