...
pub struct Opts {
    ...
    /// Angle config files in TOML format to validate.
    #[arg(
        id = "config-toml",
        long,
    ...
    /// Angle config files in JSON format to validate.
    #[arg(
        id = "config-json",
        long,
    ...
    /// Angle config files in YAML format to validate.
    #[arg(
        id = "config-yaml",
        long,
    ...
    /// If none are specified, the default config path `/etc/angle/angle.yaml`
    /// is targeted.
    #[arg(env = "ANGLE_CONFIG", value_delimiter(','))]
