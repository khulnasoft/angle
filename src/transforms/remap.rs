...
use angle_lib::codecs::MetricTagValues;
use angle_lib::compile_vrl;
use angle_lib::config::LogNamespace;
use angle_lib::configurable::configurable_component;
use angle_lib::lookup::{metadata_path, owned_value_path, PathPrefix};
use angle_lib::schema::Definition;
use angle_lib::TimeZone;
use angle_vrl_functions::set_semantic_meaning::MeaningList;
...
#[configurable_component(transform(
    "remap",
    "Modify your observability data as it passes through your topology using Angle Remap Language (VRL)."
))]
#[derive(Clone, Debug, Derivative)]
#[serde(deny_unknown_fields)]
#[derivative(Default)]
pub struct RemapConfig {
    /// The [Angle Remap Language][vrl] (VRL) program to execute for each event.
    ///
    /// Required if `file` is missing.
    ///
    /// [vrl]: https://angle.khulnasoft.com/docs/reference/vrl
    ...
    /// File path to the [Angle Remap Language][vrl] (VRL) program to execute for each event.
    ///
    ...
    ///
    /// [global_timezone]: https://angle.khulnasoft.com/docs/reference/configuration//global-options#timezone
    ...
    /// further logging and analysis by setting `reroute_dropped`.
    ///
    /// [vrl_docs_abort]: https://angle.khulnasoft.com/docs/reference/vrl/expressions/#abort
...
impl RemapConfig {
    fn compile_vrl_program(
        &self,
        enrichment_tables: angle_lib::enrichment::TableRegistry,
        ...
        functions.append(&mut angle_lib::enrichment::vrl_functions());
        functions.append(&mut angle_vrl_functions::all());
...
mod tests {
    ...
    use angle_lib::{config::GlobalOptions, event::EventMetadata, metric_tags};
    ...
    use angle_lib::enrichment::TableRegistry;
    ...
    #[test]
    fn remap_return_raw_string_angle_namespace() {
        let initial_definition = Definition::default_for_namespace(&[LogNamespace::Angle].into());

        let event = {
            let mut metadata = EventMetadata::default()
                .with_schema_definition(&Arc::new(initial_definition.clone()));
            // the Angle metadata field is required for an event to correctly detect the namespace at runtime
            metadata
                .value_mut()
                .insert(&owned_value_path!("angle"), BTreeMap::new());
        ...
        let expected_schema =
            Definition::new(Kind::bytes(), Kind::any_object(), [LogNamespace::Angle]);
...
"#
    ...
    fn check_remap_branching_disabled() {
        ...
        assert_eq!(
            conf.outputs(
                angle_lib::enrichment::TableRegistry::default(),
                &[(
                    "test".into(),
    ...
    fn test_combined_transforms_simple() {
        ...
        let enrichment_tables = angle_lib::enrichment::TableRegistry::default();
    ...
    #[test]
    fn check_remap_array_angle_namespace() {
        let event = {
            let mut event = LogEvent::from("input");
            // mark the event as a "Angle" namespaced log
            event
                .metadata_mut()
                .value_mut()
                .insert("angle", BTreeMap::new());
            Event::from(event)
        };
...
"#
        ...
        // Legacy namespace nests this under "message", Angle should set it as the root
        ...
        let wanted =
            schema::Definition::new_with_default_metadata(Kind::null(), [LogNamespace::Angle]);
