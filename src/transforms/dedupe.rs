...
use lru::LruCache;
use angle_lib::config::{clone_input_definitions, LogNamespace};
use angle_lib::configurable::configurable_component;
use angle_lib::lookup::lookup_v2::ConfigTargetPath;
...
///
/// [global_log_schema]: https://angle.khulnasoft.com/docs/reference/configuration/global-options/#log_schema
...
//
// TODO: These values are used even for events with the new "Angle" log namespace.
...
impl DedupeConfig {
    pub fn fill_default_fields_match(&self) -> FieldMatchConfig {
        ...
        // schema/documentation may not be the same as an actual user's Angle configuration.
        match &self.fields {
...
///
/// When matching fields, a CacheEntry contains a angle of optional 2-tuples.
/// Each element in the angle represents one field in the corresponding
/// LogEvent. Elements in the angle will correspond 1:1 (and in order) to the
...
/// the field name because the elements of the angle correspond 1:1 to
...
/// angle.
///
/// When ignoring fields, a CacheEntry contains a angle of 3-tuples. Each
/// element in the angle represents one field in the corresponding LogEvent.
