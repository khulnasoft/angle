...
use angle_lib::lookup::{event_path, owned_value_path, path, PathPrefix};
...
pub struct MetricToLogConfig {
    ...
    ///
    /// [global_log_schema_host_key]: https://angle.khulnasoft.com/docs/reference/configuration//global-options#log_schema.host_key
    ...
    /// described by [the `native_json` codec][angle_native_json].
    ///
...
impl TransformConfig for MetricToLogConfig {
    ...
    fn outputs(
        &self,
        _: angle_lib::enrichment::TableRegistry,
        ...
        serde_json::to_value(&metric)
            ...
            .and_then(|value| match value {
                Value::Object(object) => {
                    ...
                    // converting all fields from serde `Value` to Angle `Value`
                    for (key, value) in object {
                        log.insert(event_path!(&key), value);
                    }

                    if self.log_namespace == LogNamespace::Legacy {
                        // "Angle" namespace just leaves the `timestamp` in place.
                    ...
                    if self.log_namespace == LogNamespace::Angle {
                        // Create angle metadata since this is used as a marker to see which namespace is used at runtime.
                        // This can be removed once metrics support namespacing.
                        log.insert(
                            (PathPrefix::Metadata, path!("angle")),
...
mod tests {
    ...
    use angle_lib::config::ComponentKey;
    use angle_lib::{event::EventMetadata, metric_tags};
    ...
    async fn transform_distribution() {
        let distro = Metric::new_with_metadata(
            ...
            MetricValue::Distribution {
                samples: angle_lib::samples![1.0 => 10, 2.0 => 20],
    ...
    async fn transform_histogram() {
        let histo = Metric::new_with_metadata(
            ...
            MetricValue::AggregatedHistogram {
                buckets: angle_lib::buckets![1.0 => 10, 2.0 => 20],
                count: 30,
                sum: 50.0,
            },
            event_metadata(),
    ...
    async fn transform_summary() {
        let summary = Metric::new_with_metadata(
            ...
            MetricValue::AggregatedSummary {
                quantiles: angle_lib::quantiles![50.0 => 10.0, 90.0 => 20.0],
                count: 30,
                sum: 50.0,
            },
            event_metadata(),
