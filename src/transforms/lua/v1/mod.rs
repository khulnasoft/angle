...
impl Lua {
    pub fn new(source: String, search_dirs: Vec<String>) -> crate::Result<Self> {
        ...
        let angle_func = lua.create_registry_value(func).context(InvalidLuaSnafu)?;
    ...
    fn process(&mut self, event: Event) -> Result<Option<Event>, mlua::Error> {
        ...
        let func = lua.registry_value::<mlua::Function<'_>>(&self.angle_func)?;
        func.call(())?;
