...
use crate::{
    ...
    heartbeat,
    internal_events::{AngleQuit, AngleStarted, AngleStopped},
        ...
        if self.api.enabled {
            match api::Server::start(
                ...
                Err(error) => {
                    ...
                    error!("An error occurred that Angle couldn't handle: {}.", error);
                    _ = self
                        .topology
                        .abort_tx
            ...
            info!(message="API is disabled, enable by setting `api.enabled` to `true` and use commands like `angle top`.");
            None
        }
    }
}
...
impl Application {
    ...
    pub fn prepare_from_opts(opts: Opts) -> Result<(Runtime, Self), ExitCode> {
        ...
        let runtime = build_runtime(opts.root.threads, "angle-worker")?;
...
fn get_log_levels(default: &str) -> String {
    std::env::var("ANGLE_LOG")
        .or_else(|_| {
            std::env::var("LOG").map(|log| {
                warn!(
                    message =
                        "DEPRECATED: Use of $LOG is deprecated. Please use $ANGLE_LOG instead."
                );
                log
            })
        })
