...
pub struct RootOpts {
    ...
    /// `/etc/angle/angle.yaml` is targeted.
    #[arg(
        id = "config",
        short,
        long,
    ...
    #[arg(long, default_value = "text", env = "ANGLE_LOG_FORMAT")]
    pub log_format: LogFormat,

    /// Control when ANSI terminal formatting is used.
    ///
    /// By default `angle` will try and detect if `stdout` is a terminal, if it is
    ...
    #[arg(long, default_value = "auto", env = "ANGLE_COLOR")]
    pub color: Color,

    /// Watch for changes in configuration file, and reload accordingly.
    #[arg(short, long, env = "ANGLE_WATCH_CONFIG")]
    ...
    /// This is useful when you would like for Angle to attempt to send data until terminated
    /// by a SIGKILL. Overrides/cannot be set with `--graceful-shutdown-limit-secs`.
    #[arg(
        long,
        default_value = "false",
        env = "ANGLE_NO_GRACEFUL_SHUTDOWN_LIMIT",
        group = "graceful-shutdown-limit"
    )]
    ...
    /// in the Angle process. This behavior can be problematic for users of the `exec` source, which by
    /// default inherits the environment of the Angle process.
    #[arg(long, env = "ANGLE_OPENSSL_NO_PROBE", default_value = "false")]
    ...
    /// `--watch-config`.
    #[arg(long, env = "ANGLE_ALLOW_EMPTY_CONFIG", default_value = "false")]
...
pub enum SubCommand {
    ...
    /// Generate a Angle configuration containing a list of components.
    Generate(generate::Opts),

    /// Generate the configuration schema for this version of Angle. (experimental)
    ///
    /// A JSON Schema document will be written to stdout that represents the valid schema for a
    /// Angle configuration. This schema is based on the "full" configuration, such that for usages
    /// where a configuration is split into multiple files, the schema would apply to those files
    /// only when concatenated together.
    GenerateSchema,

    /// Output a provided Angle configuration file/dir as a single JSON object, useful for checking in to version control.
    #[command(hide = true)]
    Config(config::Opts),
    ...
    /// Run Angle config unit tests, then exit. This command is experimental and therefore subject to change.
    /// For guidance on how to write unit tests check out <https://angle.khulnasoft.com/guides/level-up/unit-testing/>.
    Test(unit_test::Opts),

    /// Output the topology as visual representation using the DOT language which can be rendered by GraphViz
    Graph(graph::Opts),

    /// Display topology and metrics in the console, for a local or remote Angle instance
    #[cfg(feature = "api-client")]
    Top(top::Opts),
    ...
    /// Manage the angle service.
    #[cfg(windows)]
    Service(service::Opts),

    /// Angle Remap Language CLI
    Vrl(vrl::cli::Opts),
}
