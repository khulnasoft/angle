//! Functionality for managing template fields used by Angle's sinks.
...
use regex::Regex;
use snafu::Snafu;
use angle_lib::configurable::{configurable_component, ConfigurableString};
use angle_lib::lookup::lookup_v2::parse_target_path;
...
pub enum TemplateParseError {
    ...
    StrftimeError,
    #[snafu(display("Invalid field path in template {:?} (see https://angle.khulnasoft.com/docs/reference/configuration/template-syntax/)", path))]
...
fn parse_template(src: &str) -> Result<Vec<Part>, TemplateParseError> {
    ...
    for cap in RE.captures_iter(src) {
        ...
        // see: https://github.com/khulnasoft/angle/issues/14864
...
mod tests {
    ...
    use chrono_tz::Tz;
    use angle_lib::lookup::{metadata_path, PathPrefix};
    ...
    fn render_metric_with_namespace() {
        ...
        let metric = sample_metric().with_namespace(Some("angle-test"));
        assert_eq!(
            Ok(Bytes::from("namespace=angle-test name=a-counter")),
            template.render(&metric)
        );
    }
    ...
    fn render_log_with_timezone() {
        ...
        let template = Template::try_from("angle-%Y-%m-%d-%H.log").unwrap();
        let mut event = Event::Log(LogEvent::from("hello world"));
        event.as_mut_log().insert(
            (PathPrefix::Event, log_schema().timestamp_key().unwrap()),
            ts,
        ...
        assert_eq!(
            Ok(Bytes::from("angle-2001-02-03-12.log")),
