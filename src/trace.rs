...
/// BUFFER contains all of the internal log events generated by Angle between the initialization of `tracing` and early
...
/// angle of all early buffered events once early buffering has been stopped. Otherwise, if early buffering is already
...
/// A subscription to the log events flowing in via `tracing`, in the Angle native format.
///
/// Used to capture tracing events from internal log telemetry, via `tracing`, and convert them to native Angle events,
            ...
            if let Some(parent_span) = ctx.event_span(event) {
                for span in parent_span.scope().from_root() {
                    if let Some(fields) = span.extensions().get::<SpanFields>() {
                        for (k, v) in &fields.0 {
                            log.insert(event_path!("angle", *k), v.clone());
                        }
                    }
                }
            }
