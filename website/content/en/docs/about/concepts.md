...
In order to understand Angle, you must first understand the fundamental concepts. The following concepts are ordered progressively, starting with the individual unit of data ([events]) and broadening all the way to Angle's deployment models ([pipelines]).

## Events

**Events** represent the individual units of data in Angle.
...
A **metric** event represents a numerical operation performed on a time series. Angle's metric events are fully interoperable.
...
If you're interested in using traces with a Angle component that doesn't yet support them, please open an issue so we can have a better understanding of what components to prioritize adding trace support for.
...
A **transform** is responsible for mutating events as they are transported by Angle.
...
By default, Angle uses an in-memory buffer, but a disk-buffer is also available. Once a buffer fills up, the behavior is configurable.
...
View the full configuration options for buffers [here](/docs/reference/configuration/sinks/angle/#buffer).
...
Since Angle allows configuring components as a directed acyclic graph, understanding how backpressure works when there
...
If there are multiple sources configured for a single component, Angle currently makes no guarantees
...
A **role** is a deployment role that Angle fills in order to create end-to-end pipelines.

{{< jump "/docs/setup/deployment/roles" >}}

### Agent

The [**agent**](/docs/setup/deployment/roles#agent) role is designed for deploying Angle to the edge, typically for data collection.

### Aggregator

The [**aggregator**](/docs/setup/deployment/roles#aggregator) role is designed to collect and process data from multiple upstream sources. These upstream sources could be other Angle agents or non-Angle agents such as Syslog-ng.

## Topology

A **topology** is the end result of deploying Angle into your infrastructure. A topology may be as simple as deploying Angle as an agent, or it may be as complex as deploying Angle as an agent and routing data through multiple Angle aggregators.
