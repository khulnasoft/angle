...
Angle implements a buffering model that allows operators to choose whether to prioritize
...
While operators typically strive to ensure their Angle deployments are appropriately sized for the
...
Part of Angle's topology design involves propagating _backpressure_, which is a signal that events
...
of Angle. We want to avoid entirely slowing things down when a component just crosses over the
...
Buffering is the approach that Angle takes to solve these problems.

## Buffering between components

All components in a Angle topology have a small in-memory buffer between them. The primary purpose
...
When working with a Angle configuration, you'll be working with buffer configuration settings on
...
again, they are typically the primary source of backpressure in any given Angle topology.

Beyond the default buffer capacity being larger, you can also fully control the buffer configuration
as well.  Angle exposes two main settings for controlling buffering: the type of buffer to use, and
...
substantially if the data being processed by Angle changes upstream and grows in size unexpectedly.
The size of events is fluid, and based off the internal representation used by Angle. As a rule of
...
Additionally, in-memory buffers are not durable. While Angle provides features like
[end-to-end acknowledgements][e2e_acks] to ensure that sources don't acknowledge events until they
have been processed, any events sitting in an in-memory buffer would be lost if Angle, or the host
running Angle, crashed. While pull-based sources like S3 or Kafka would handle this by simply
reattempting to process the events, push-based sources may not be able to retransmit their messages.

### Disk buffers

When the durability of data is more important than the overall performance of Angle, disk buffers
can be used to persist buffered events while stopping and starting Angle, including if Angle
crashes. Disk buffers allow Angle to essentially pick up from where it left off when it starts back
up again.
...
write data to disk faster than Angle, we've chosen to make sure that events can be read as fast as
...
limited in terms of disk usage. This maximum size is adhered to rigidly, so you can depend on Angle
...
mistakenly deleted while Angle is running. Disk buffers automatically checksum all events being
...
safely on disk, Angle will **forcefully stop itself** when an I/O error occurs during flushing to
disk. An error message will be emitted before exiting that explains the underlying cause of the
error, such as "no storage space". Depending on the error, Angle can typically be safely restarted
...
As an operator, the main resource you'll need to monitor is _free storage space_. If Angle cannot
...
for Angle (located within the global [`data_dir`][global_data_dir]) is on a storage volume with
...
While Angle will exit at startup if it detects your disk buffers could grow to a size bigger than
...
have a major impact on how Angle as a system performs, and this behavior often need to be matched
to the configuration and workload itself.

### Blocking (`block`)

When configured to block, Angle will wait indefinitely to write to a buffer that is currently full.
...
processing observability data, in the order it was given to Angle. Additionally, blocking will
...
have them also blocked on waiting for a response that the data was accepted by Angle. We'll cover
...
dropped, even when Angle appears to have available processing capacity.
{{< /warning >}}

When configured to "drop newest", Angle will simply drop an event if the buffer is currently full.

This behavior can be useful when the data itself is idempotent (the same value is being sent
continually) or is generally not high-value, such as trace or debug logging. It allows Angle to
...
that Angle tries to buffer will go to in-memory buffer, even if there are still events in the disk
...
Below are a few common scenarios that Angle users often deal with and the recommended buffering
configurations to use.

**I can't provide any storage to Angle.**

You'll have to use in-memory buffers then. Angle does not support buffering events to external
storage systems.
