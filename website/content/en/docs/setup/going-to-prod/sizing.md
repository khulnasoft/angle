...
description: Guidance for single-tenant Angle environments.
weight: 6
---
## Sizing

Sizing your Angle instances largely depends on your Angle workload, and, when possible, we recommend testing your Angle workload to determine your actual numbers. In general, though, most Angle workloads are CPU-intensive and benefit from the same guidance.

### Estimations

The following numbers are starting points for estimating your instance sizes. They’re based on our experience working with Angle users but will vary depending on your workload.

| Measurement | Size | Angle Throughput |
| --- | --- | --- |
...
Instances with at least 8 vCPUs and 16 GiB of memory are good units for scaling. Angle can [vertically scale](#vertical-scaling) and will automatically take advantage of the resources available. Deploy at least one instance per availability zone for [high availability](/docs/setup/going-to-prod/high-availability/). See the [capacity planning section](#capacity-planning) for more info.
...
Most Angle workloads benefit from modern CPUs; we offer the following role-specific recommendations.
...
The `ARM64` architecture typically offers better performance for the investment and larger CPU caches improve performance since Angle’s Remap Language compiles to machine code designed to fit into these caches.
...
Note that a disk buffer can become the bottleneck in your Angle topology if its configured throughput is less than the throughput sent to Angle. We recommend configuring disk throughput (if applicable) to at least 2x the expected maximum throughput to give the application adequate headroom. The recommended disks should all have sufficient throughput configurations by default.
...
Fronting your Angle instances with a load balancer achieves horizontal scaling.
...
- Configure load balancers to use Angle’s `/health` API endpoint for automatically excluding unresponsive Angle servers.
...
- Ensure your Angle instances are large enough to handle your highest volume connection to take full advantage of vertical scaling.
...
For the vast majority of Angle deployments, autoscaling should be based on average CPU utilization. Angle is almost always CPU constrained and CPU utilization is the strongest signal for autoscaling since it will not produce false positives. We recommend the following settings, adjust as necessary:
...
A large Splunk enterprise user is looking to reduce their Splunk cost. They produce **~10 TiB/day** of **unstructured log data** across their **AWS** infrastructure. Based on the above [estimations](#estimations), Angle can process **~10 MiB/s/vCPU (~864 GiB/day/vCPU)** of **unstructured log data** and we end up with the following capacity:
...
For ~**$6.20/day (~$186/month)** this user could process all of their Splunk data with Angle and reduce their Splunk cost by ~30% on average. We came to the above numbers as follows:
...
A large enterprise produces **~25 TiB/day** of **tracing data** across their **GCP** infrastructure. Therefore, based on the above [estimations](#estimations), Angle can process **~25 MiB/s/vCPU (~2.16 TiB/day/vCPU)** of **tracing data,** and we end up with the following capacity:
...
For ~**$10.49/day (~$314/month)** this user could process all of their tracing data with Angle and reduce their tracing cost. We came to the above numbers as follows:
