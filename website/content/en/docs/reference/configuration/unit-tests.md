---
title: Unit testing Angle configurations
short: Unit tests
weight: 6
aliases: [
...
Angle enables you to [unit test] [transforms] in your processing [pipeline]. Unit tests in Angle
...
Unit tests can serve as a useful guardrail when running in Angle in production settings where you
need to ensure that your topology doesn't exhibit unexpected behavior and generally improve the
maintainability of your Angle pipelines, particularly in larger and more complex pipelines.

## Running unit tests

You can execute tests within a [configuration](#configuring) file using Angle's
[`test`][angle_test] subcommand:

```bash
angle test /etc/angle/angle.yaml
```

You can also specify multiple configuration files to test:

```bash
angle test /etc/angle/pipeline1.toml /etc/angle/pipeline2.toml
```

Specifying multiple files is useful if you want to, for example, keep your unit tests in a separate
file from your pipeline configuration. Angle always treats multiple files as a single, unified
configuration.
...
* [`assert`][assert] takes a [Boolean expression][boolean] as its first argument. If the Boolean
  resolves to `false`, the test fails and Angle logs an error.
* [`assert_eq`][assert_eq] takes any two values as its first two arguments. If those two values
  aren't equal, the test fails and Angle logs an error.
...
# The transform being tested is a Angle Remap Language (VRL) transform that
...
{{< info >}}
This unit involved only a single Angle transform. An example [multi-transform](#multiple) unit test
is provided below.
{{< /info >}}

### Real vs. test inputs

One important thing to note is that with this example configuration Angle is set up to pull in real
logs from Docker images using the [`docker_logs`][docker_logs] source. If Angle were running in
...
artificially inserts our test inputs into the `add_metadata` transform. You should think of Angle
...
The unit testing example above is in TOML but Angle also supports YAML and JSON as configuration
formats.
{{< /success >}}

## Configuring unit tests {#configuring}

Unit tests in Angle live alongside your topology configuration. You can specify your tests in the
...
[tests.inputs.log_fields]
message = "<102>1 2020-12-22T15:22:31.111Z angle-user.biz su 2666 ID389 - Something went wrong"
```
...
In this test configuration, Angle would expect that the `log_filter` and `metric_filter` transforms
...
Below is a full example of using `no_outputs_from` in a Angle unit test:
...
There are currently two event types that you can unit test in Angle:
...
[[tests.inputs]]
insert_at = "add_metadata"
value = "<102>1 2020-12-22T15:22:31.111Z angle-user.biz su 2666 ID389 - Something went wrong"
```
...
inputs = ["sanitize"]
source = '''
.tags.host = "web-backend1.angle-user.biz"
'''
...
assert_eq!(.tags.host, "web-backend1.angle-user.biz", "incorrect host tag")
...
[angle_test]: /docs/reference/cli#test
[angle_tests]: https://github.com/khulnasoft/angle/tree/master/tests/behavior/transforms
