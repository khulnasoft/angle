...
Angle supports a template syntax for some configuration options. This allows for dynamic values derived from event
...
Notice that Angle allows direct field references as well as "strftime" specifiers. If we were to run the following log
event through Angle:
...
In addition to directly accessing fields, Angle offers a shortcut for injecting [strftime specifiers][strftime]:
...
Angle doesn't currently support fallback values, [issue 1692][1692] is open to add this functionality. In the interim,
...
If a field is missing, an error is logged and Angle drops the event. The `component_errors_total` internal
metric is incremented with an `error_type` tag of `template_failed`.

[1692]: https://github.com/khulnasoft/angle/issues/1692
