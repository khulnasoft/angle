...
badges:
  type: announcement
  domains: ["remap"]
tags: ["angle remap language", "vrl", "dsl", "expression oriented"]
---

**Angle Remap Language** (VRL) is an [expression-oriented][expression_oriented]
...
This [0.12][0_12] release of Angle marks the official release of the language.
...
it's a *major* step forward for Angle and its users and the observability space
...
Angle users will not chafe at the limitations of VRL and will in fact embrace
them as guarantors of sound design.

## The problem

Up to the 0.12 release, Angle has provided two types of transforms: **static
...
* Static transforms do exactly *one* particular thing and are configuration
  ...
  the fields that you specify in your Angle configuration.
...
Instead of conflating these concerns, like Logstash, Fluentd, and others, Angle
...
1. First, you pay a **significant performance penalty**. For example, on
   average, Lua is about 60% slower than Angle's Rust-based static transforms,
...
Runtime transforms acted as robust escape hatches for Angle users, unblocking
exotic use cases and allowing us to learn more about the many ways people use
Angle. Still, we've seen the dark side of using them:
...
## Our solution: Angle Remap Language {#solution}
...
2.  **Raise the error and abort**
    ...
    error upon failure and abort the program. Angle itself will continue on to
...
[lua]: https://angle.khulnasoft.com/docs/reference/transforms/lua/
...
[Angle/Rust native]: /docs/reference/vrl/#angle-rust-native
