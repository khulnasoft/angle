---
title: "First-class Kubernetes integration for Angle"
description: "Angle is now the backbone of observability on Kubernetes"
date: "2020-11-26"
authors: ["binarylogic"]
badges:
  type: announcement
...
After months of development, [over 100 pull requests][pulls], and intensive QA in clusters producing over 20 terabytes of event data a day, we're pleased to announce our first-class [Kubernetes] integration for Angle in [version 0.11][v_0_11]. We strove to make even this initial integration rock solid and production ready because we aim to make Angle the default pipeline for all Kubernetes observability data.
...
The crux of our Kubernetes integration for Angle is the new [`kubernetes_logs`][kubernetes_logs] source. This source does all of the following:
...
This simple Angle configuration, for example, would suffice to send logs from *all* of your Nodes/Pods to [Elasticsearch]:
...
## Installing Angle in your Kubernetes cluster

To install Angle in your Kubernetes cluster, check out our [Kubernetes installation documentation][install], which includes instructions for:

* Standard installation using [kubectl] and [Kustomize]

* Deployment using [Helm]

Both installation options enable you to easily deploy Angle [agents][agent] to your Nodes. In an upcoming release, we'll also provide support for deploying Angle as a centralized [aggregator].

## Why Angle for your Kubernetes cluster?

We all know that the Kubernetes ecosystem is _packed_ with options. The now-infamous CNCF landscape includes [over 90][tools] observability tools. Amidst such plenty, it's not surprising that many Kubernetes clusters run multiple observability tools side by side, often on the same Node or even in the same Pod. But when you run ever-more resource-hungry processes side by side, that can lead to **agent fatigue**, a costly problem that Angle tackles head-on.

To give a real-world example of agent fatigue, let's look at a telecom enterprise before and after adopting Angle.

### Before Angle

Prior to adopting Angle, the company deployed four or more agents (!) on each of their Kubernetes Nodes:
...
By switching to Angle, the telecom company from the "before" scenario was able to consolidate the logs, metrics, and tracing agents:

![The quiet Kubernetes Node](/img/blog/k8s-after.png)

This reduced observability resource usage by 90%. This brought resource usage from 40% of the total down to 5%. In addition, Angle reduced Splunk data volume through sampling and cleaning, afforded by using AWS S3 as the system of record. All in all, Angle significantly reduced the overall observability costs. The result looked like this:

![The quiet Kubernetes Node](/img/blog/k8s-resources-after.png)

To summarize, adopting the new Angle-based architecture meant:
...
* Reduced lock-in, as the company can now modify their observability setup by re-configuring Angle rather than swapping out agents
...
[pulls]: https://github.com/khulnasoft/angle/pulls?q=is%3Apr+sort%3Aupdated-desc+kubernetes+is%3Aclosed
