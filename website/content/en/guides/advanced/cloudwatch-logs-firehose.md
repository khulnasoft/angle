...
* You have logs in [AWS CloudWatch Logs][AWS CloudWatch Logs] that you'd like to
  consume.
* You are able to deploy Angle with a publicly exposed HTTPS endpoint.

[AWS CloudWatch Logs]: https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/WhatIsCloudWatchLogs.html
{{< /requirement >}}

If you use [AWS CloudWatch Logs][AWS CloudWatch Logs] on Amazon Web Services
(AWS), you may be wondering how to ingest these logs with Angle so you can
...
running Angle instances over HTTPS.

You will learn how to:

* Configure Angle to consume AWS CloudWatch Log events via the
...
* Configure [AWS Kinesis Firehose][AWS Kinesis Firehose] to forward events to
  a remotely running Angle instance (or instances)
...
adapted to send them to any other [destination of your choice](https://angle.khulnasoft.com/docs/reference/sinks/).
...
and [AWS Kinesis Firehose][AWS Kinesis Firehose] to forward logs to Angle? This
pipeline:
...
* Allows you to load balance the log ingestion
  * You can put multiple `angle` instances behind a load balancer
...
export ANGLE_ENDPOINT="https://example.com" # the endpoint where angle is deployed with the aws_kinesis_firehose source (see Configuring Angle below)

# Update these if needed
export LOG_GROUP="/test/angle" # the log group you want to consume with angle
export FIREHOSE_DELIVERY_STREAM="angle-stream"
export FIREHOSE_LOG_GROUP="/aws/kinesisfirehose/angle-stream" # log group for Kinesis Firehose to log messages to
...
This will configure `angle` to listen for Firehose messages on the configured
...
`angle` instance to handle TLS termination or configure the `tls` parameters of
...
First, deploy angle with the [`aws_kinesis_firehose`][aws_kinesis_firehose] source. See example
configuration above.
...
```bash
$ aws iam put-role-policy --role-name FirehoseAngle \
    --policy-name FirehoseAngleS3 \
    ...
    --http-endpoint-destination file://<(cat <<EOF
{
  ...
  "RoleARN": "arn:aws:iam::${AWS_ACCOUNT_ID}:role/FirehoseAngle",
  "RetryOptions": {
...
This will configure Firehose to ship all events to `$ANGLE_ENDPOINT` using the
...
We've got our delivery stream setup and pointing at our Angle instance(s). Let
...
group we've setup. We can use Angle for this too!
...
AWS credentials Angle will need to write to AWS.
...
```bash
flog -f json | angle --config config.toml
```

This will send some logs to your log group. Within 300 seconds (the default
batch interval) these logs should be forwarded to your running `angle`
instance(s).

## Wrap Up

Congratulations, now you are ingesting logs from AWS CloudWatch Logs in a robust
manner. You can now use Angle to transform and forward these logs to your
destinations of choice.

Any troubles getting this to work? Let us know via [GitHub
issues](https://github.com/khulnasoft/angle/issues/new/choose) or [drop into the
`#aws` channel on our discord server.](https://chat.angle.khulnasoft.com)
...
group, `/test/angle` that we had setup:
