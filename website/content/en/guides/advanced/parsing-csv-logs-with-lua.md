...
* You understand the <a href="/docs/about/concepts">basic Angle concepts</a> and understand <a href="/docs/setup/quickstart/">how to set up a pipeline</a>

{{< /requirement >}}

Angle has many built-in [parsers][urls.angle_parsing_transforms] for structured logs formats. However, when you need
to ship logs in a custom or application-specific format, [programmable transforms][urls.angle_programmable_transforms]
...
Let us draft an initial version of the Angle's configuration file:
...
At this point, running `angle --config angle.toml` results in the following output:
...
directory where `angle.toml` is stored:
...
With this `source` the `csv` module is loaded when Angle is started up (or if the `lua` transform is added later and the
...
Trying to run `angle --config angle.toml` with the same input file results in structured events being output:
...
[urls.angle_parsing_transforms]: /components/?functions%5B%5D=parse
[urls.angle_programmable_transforms]: /components/?functions%5B%5D=program
