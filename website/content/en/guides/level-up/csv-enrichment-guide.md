...
sources. Angle now offers initial support for enriching events from external data
sources, which is currently powered by a powerful Angle concept,
...
illustrates the required Angle configuration:
...
(though Angle's current solution is limited to `csv` files) that contain
...
any feedback or request for additional support you have for the Angle team!
