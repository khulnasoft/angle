...
short: Schemas
description: Learn how to manage log schemas with Angle.
...
Data comes in all shapes and sizes. Angle has an array (let's call it a angle 😎) of composable functionality for
decoding your events in the right format, transforming them into the right shape, and passing that data on downstream.

While your first uses of Angle might be connecting `stdin` to `honeycomb`, eventually you're going to have other
requirements. Maybe regional laws prevent you from shipping certain data, or you need to do some data munging to conform
some logs to the rest of your system. With a little configuration we can teach Angle to solve all these problems!

## Overriding Global Field Names

By default, Angle primarily operates on three fields: `host`, `message`, and `timestamp`.
...
Angle sets these fields on logs as it ingests data (from a [source][docs.sources]). It may be that your data does not
follow this convention. In this case you can modify the global defaults for all incoming data in the `log_schema`
section of your `angle.toml`.
...
If your angle pipeline is only working with data passing through these systems, you can add the following to your
`angle.toml`:
...
Let's pretend we have a nice well behaved application piping Angle logs like the following:
...
{ "id": "user3", "gdpr": true, "email": "eu-user3@datadoghq.com" }
EOF
Feb 05 16:13:59.241  INFO source{name=application type=stdin}: angle::sources::stdin: finished sending
...
Feb 05 16:15:27.945  INFO angle: Shutting down.
```
...
identities, all are valid. Angle lets us take in timestamps and output specific formats easily.
...
Angle you can handle them all.

Generally Angle will be able to determine the encodings to use by the source or sink used. In some cases, multiple are
...
Exploring this article, we can see that Angle is able to consume multiple (even non-standard) formats of logs. We saw
that Angle can then reshape the data according to your needs. Then Angle can pass this data along.

Let's consider some novel uses for Angle, given these tools! Angle can work as:
...
Where are you deploying Angle? Let us know, maybe we can help optimize it!
