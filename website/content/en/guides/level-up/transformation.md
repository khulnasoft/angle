...
short: Transforming
description: Use Angle to parse, structure, shape, and transform observability data
...
Angle provides multiple [transforms][docs.transforms] that you can use to
modify your observability data as it passes through your Angle
...
[Angle Remap Language][docs.vrl] (VRL for short) to define event
transformation logic. VRL has several features that should make it your first
choice for transforming data in Angle:
...
* It's built for the very specific use case of working with Angle logs and
  metrics, which means that it has no extraneous functionality, its data model
  maps directly to Angle's internal data model, and its performance is comparable
  to native [Rust][urls.rust] performance.
* The VRL compiler built into Angle performs several compile-time checks to
  ensure that your VRL code is sound, meaning no dead code, no unhandled errors,
  and no type mismatches.

In cases where VRL doesn't fit your use case, Angle also offers a [Lua runtime
...
{{< info >}}
Although we're using [YAML][urls.yaml] for the configuration here, Angle also
...
To start Angle using this topology:
...
So far, we've gotten Angle to *parse* the Syslog data but we're not yet
...
* Any errors thrown by VRL functions must be handled. Were we to neglect to
  ...
  the VRL compiler would provide a very specific warning and Angle wouldn't
  start up.
...
If you stop and restart Angle, you should see log lines like this (again
...
And that's it! We've successfully created a Angle topology that transforms
...
If VRL doesn't cover your use case—and that should happen rarely—Angle also
...
your Angle configuration

The `lua` transform provides maximal flexibility because they enable you to use
a full-fledged programming language right inside of Angle. But we recommend
...
2. It requires you to add a coding/testing/debugging workflow to using Angle,
