---
title: Using Angle `tap`
short: Angle tap
description: Learn how to use the Angle `tap` CLI command to examine events as they flow through your pipeline and troubleshoot issues.
...
* You understand the [basic Angle concepts][concepts]
...
Angle's `tap` CLI command allows you to observe events as they flow to and from
...
right place. This guide walks you through how `angle tap` can be used to level
up your troubleshooting experience.

## Getting Started

To start, we'll reference the following base configuration, but feel free to
substitute your own. Just note that the [Angle API] must be enabled for `angle
...
Run Angle with this configuration and watch the configuration for changes.

```console
angle --config path/to/config.toml -w
```

Now run `angle tap`! You should start seeing a stream of
...
Running the bare `angle tap` command invokes sensible defaults and is
equivalent to specifying `--outputs-of "*"` where `"*"` matches any component
ID.

Try running `angle tap --inputs-of "out"`. The events should be the same
...
Try running `angle tap --quiet --format logfmt`. You'll now see no
...
With `angle tap` running, add the following `demo_logs` source to the base
configuration.
...
You can read more about all available options in the [Angle tap docs] or by
running `angle tap --help`.
...
We'll use the following Angle configuration.
...
We can verify this by examining the inputs of the `store` sink with `angle tap
...
`angle tap --outputs-of "picky"` (which, in this case, is effectively the same
as inspecting the inputs of `store`) shows that events are not flowing.

Are all the events being dropped instead? A quick glance with `angle tap
...
they're structured. We hope `angle tap` eases the troubleshooting burden
especially in larger, more complex setups where mistakes are not easy to find
by simply reading a configuration file.

Ultimately, `angle tap` is one tool in the wide range of Angle features that
lets you safely and reliably set up your observability pipelines. Be sure to
also check out [Angle unit testing] to verify the expected behavior of your
transforms and [Angle internal observability] for more insight into Angle
itself.

## Under the hood

Under the hood, `angle tap` is powered by the [Angle API], specifically by the
...
{{< info >}}
Note that as long as your Angle instance has its API enabled and exposed,
`angle tap` will work! Simply use the `--url` option to specify a non-default
API address. This allows you to troubleshoot remote Angle instances.
{{< /info >}}

We encourage contributions and suggestions for improving `angle tap`!

[Angle API]: /docs/reference/api
[Angle tap docs]: /docs/reference/cli/#tap
[Angle unit testing]: /docs/reference/configuration/unit-tests
[Angle internal observability]: /docs/administration/monitoring
