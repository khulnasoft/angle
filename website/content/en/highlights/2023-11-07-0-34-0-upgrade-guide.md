...
Angle's 0.34.0 release includes **breaking changes**:
...
`--openssl-legacy-provider` (and its environment variable: `ANGLE_OPENSSL_LEGACY_PROVIDER`). This
...
The deprecated name of the `armv7` RPM package, `angle-<version>-1.armv7.rpm`, is no longer
published. Instead, it is published as `angle-<version>-1.armv7hl.rpm` to better follow RPM
...
The default config location is now `/etc/angle/angle.yaml` rather than `/etc/angle/angle.toml`.
This is part of the migration to YAML as Angle's preferred configuration language (though TOML and
JSON will still be supported for the forseeable future). If you need Angle to load
`/etc/angle/angle.toml` you must now specify `--config /etc/angle/angle.toml` or
`ANGLE_CONFIG=/etc/angle/angle.toml`
...
`repositories.timber.io` to `apt.angle.khulnasoft.com` and `yum.angle.khulnasoft.com`. This is the last point release
