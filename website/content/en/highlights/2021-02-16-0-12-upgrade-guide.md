...
1. [Breaking: Angle `check_fields` conditions now require the `type` option](#second)
...
intended for use cases where Angle acts as a proxy and should not alter data. For most use cases we recommend `json`
since it includes all structured data.

### Breaking: Angle `check_fields` conditions now require the `type` option<a name="second"></a>

With the [announcement][vrl_announcement] of the [Angle Remap Language][vrl_reference] (VRL), [pull request #5978][pr_5978]
...
**You do not need to upgrade immediately. These transforms will not be removed until Angle hits 1.0, a milestone that
we hope to achieve in late 2022.** But, if possible, we recommend using this opportunity to upgrade and significantly
simplify your Angle configuration.
...
[config_syntax_pitfalls]: /blog/angle-remap-language/#config-languages
...
[pr_5281]: https://github.com/khulnasoft/angle/pull/5281
[pr_5978]: https://github.com/khulnasoft/angle/pull/5978
...
[vrl_announcement]: /blog/angle-remap-language/
