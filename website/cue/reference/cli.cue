...
cli: {
	...
	flags: _default_flags & {
		...
		"require-healthy": {
			_short:      "r"
			description: env_vars.ANGLE_REQUIRE_HEALTHY.description
			env_var:     "ANGLE_REQUIRE_HEALTHY"
		}
		"verbose": {
			_short:      "v"
		...
		"watch-config": {
			_short:      "w"
			description: env_vars.ANGLE_WATCH_CONFIG.description
			env_var:     "ANGLE_WATCH_CONFIG"
		}
		"no-graceful-shutdown-limit": {
			description: env_vars.ANGLE_NO_GRACEFUL_SHUTDOWN_LIMIT.description
			env_var:     "ANGLE_NO_GRACEFUL_SHUTDOWN_LIMIT"
		}
		"openssl-no-probe": {
			description: env_vars.ANGLE_OPENSSL_NO_PROBE.description
			env_var:     "ANGLE_OPENSSL_NO_PROBE"
		}
		"allow-empty-config": {
			description: env_vars.ANGLE_ALLOW_EMPTY_CONFIG.description
			env_var:     "ANGLE_ALLOW_EMPTY_CONFIG"
		}
	}
	...
	_core_config_options: {
		"config": {
			_short:      "c"
			description: env_vars.ANGLE_CONFIG.description
			type:        "string"
			default:     env_vars.ANGLE_CONFIG.type.string.default
			env_var:     "ANGLE_CONFIG"
		}
		"config-dir": {
			description: env_vars.ANGLE_CONFIG_DIR.description
			type:        "string"
			env_var:     "ANGLE_CONFIG_DIR"
		}
		"config-yaml": {
			description: env_vars.ANGLE_CONFIG_YAML.description
			type:        "string"
			env_var:     "ANGLE_CONFIG_YAML"
		}
		"config-toml": {
			description: env_vars.ANGLE_CONFIG_TOML.description
			type:        "string"
			env_var:     "ANGLE_CONFIG_TOML"
		}
		"config-json": {
			description: env_vars.ANGLE_CONFIG_JSON.description
			type:        "string"
			env_var:     "ANGLE_CONFIG_JSON"
		}
		"graceful-shutdown-limit-secs": {
			description: env_vars.ANGLE_GRACEFUL_SHUTDOWN_LIMIT_SECS.description
			default:     env_vars.ANGLE_GRACEFUL_SHUTDOWN_LIMIT_SECS.type.uint.default
			env_var:     "ANGLE_GRACEFUL_SHUTDOWN_LIMIT_SECS"
			type:        "integer"
		}
	}
	...
	_core_options: _core_config_options & {
		"color": {
			description: env_vars.ANGLE_COLOR.description
			default:     env_vars.ANGLE_COLOR.type.string.default
			enum:        env_vars.ANGLE_COLOR.type.string.enum
			env_var:     "ANGLE_COLOR"
		}
		"log-format": {
			description: env_vars.ANGLE_LOG_FORMAT.description
			default:     env_vars.ANGLE_LOG_FORMAT.type.string.default
			enum:        env_vars.ANGLE_LOG_FORMAT.type.string.enum
			env_var:     "ANGLE_LOG_FORMAT"
		}

		"threads": {
			_short:      "t"
			description: env_vars.ANGLE_THREADS.description
			type:        "integer"
			env_var:     "ANGLE_THREADS"
		}
		"internal-log-rate-limit": {
			_short:      "i"
			description: env_vars.ANGLE_INTERNAL_LOG_RATE_LIMIT.description
			default:     env_vars.ANGLE_INTERNAL_LOG_RATE_LIMIT.type.uint.default
			type:        "integer"
			env_var:     "ANGLE_INTERNAL_LOG_RATE_LIMIT"
		}
	}
	...
	commands: {
		"graph": {
			...
			example: "angle graph --config /etc/angle/angle.yaml | dot -Tsvg > graph.svg"

			options: _core_options
		}
		"generate": {
			description: "Generate a Angle configuration containing a list of components"
			...
			options: {
				"file": {
					...
					type:        "string"
					example:     "/etc/angle/my-config.toml"
				}
			}
		...
		"test": {
			description: """
				Run Angle config unit tests, then exit. This command is experimental and
				therefore subject to change. For guidance on how to write unit tests check
				out the [unit testing documentation](\(urls.angle_unit_tests)).
				"""
			...
			args: {
				paths: _paths_arg & {
					description: """
						Any number of Angle config files to test. If none are specified
						the default config path `/etc/angle/angle.yaml` will be targeted
						"""
				}
			}
		}
		...
		"tap": {
			...
			flags: _default_flags & {
				...
				"no-reconnect": {
					_short:      "n"
					description: "Whether to reconnect if the underlying Angle API connection drops. By default, tap will attempt to reconnect if the connection drops."
				}
			}
			...
			options: {
				...
				"url": {
					_short:      "u"
					description: "Angle GraphQL API server endpoint"
					type:        "string"
				}
				"limit": {
					_short:      "l"
		...
		"top": {
			description: """
				Display topology and metrics in the console, for a local or remote Angle
				instance
				"""
			...
			flags: _default_flags & {
				...
				"no-reconnect": {
					_short:      "n"
					description: "Whether to reconnect if the underlying Angle API connection drops. By default, top will attempt to reconnect if the connection drops."
				}
			}
			...
			options: {
				...
				"url": {
					_short:      "u"
					description: "The URL for the GraphQL endpoint of the running Angle instance"
					type:        "string"
				}
			}
		}
		...
		"validate": {
			...
			options: {
				"config-yaml": {
					description: """
						Any number of Angle config files to validate.
			...
			args: {
				paths: _paths_arg & {
					description: """
						Any number of Angle config files to validate. If none are specified
						the default config path `/etc/angle/angle.yaml` will be targeted
						"""
				}
			}
		...
		"vrl": {
			description: "Angle Remap Language CLI"
	...
	env_vars: {
		PROCFS_ROOT: {
			description: """
				...
				used to expose host metrics from within a container. Angle uses the system's
				`/proc` by default.
				"""
		...
		RUST_BACKTRACE: {
			description: """
				...
				this only when debugging, as it can degrade Angle's performance.
				"""
		...
		SYSFS_ROOT: {
			description: """
				...
				to expose host metrics from within a container. Angle uses the system's `/sys` by
				default.
				"""
			type: string: {
				default: null
		...
		ANGLE_CONFIG: {
			description: """
				...
				specified the default config path `/etc/angle/angle.yaml` is targeted. TOML, YAML and
				JSON file formats are supported. The format to interpret the file with is determined from
				the file extension (`.yaml`, `.toml`, `.json`). Angle falls back to YAML if it can't
				detect a supported format.
				"""
			type: string: {
				default: "/etc/angle/angle.yaml"
			}
		}
		ANGLE_CONFIG_DIR: {
			description: """
		...
		ANGLE_LOG: {
			description: "Angle's log level. Each log level includes messages from higher priority levels."
			type: string: {
				default: "INFO"
				enum: {
					ERROR: "Only show error logs. The same as `-qq`"
					WARN:  "Include warnings. The same as `-q`"
					INFO:  "Include logs about Angle's operation. This is the default."
					DEBUG: "Includes logs useful for debugging or troubleshooting Angle. The same as `-v`"
					TRACE: "Most verbose log level. Can be used for troubleshooting Angle. The same as `-vv`"
				}
				examples: ["DEBUG", "INFO"]
			}
		}
		ANGLE_LOG_FORMAT: {
			...
			type: string: {
				default: "text"
				enum: {
					json: "Output Angle's logs as JSON."
					text: "Output Angle's logs as text."
				}
			}
		}
		ANGLE_REQUIRE_HEALTHY: {
		...
		ANGLE_INTERNAL_LOG_RATE_LIMIT: {
			description: "Set the internal log rate limit. This limits Angle from emitting identical logs more than once over the given number of seconds."
			type: uint: {
				default: 10
				unit:    null
			}
		}
		ANGLE_GRACEFUL_SHUTDOWN_LIMIT_SECS: {
			description: "Set the duration in seconds to wait for graceful shutdown after SIGINT or SIGTERM are received. After the duration has passed, Angle will force shutdown. To never force shutdown, use `--no-graceful-shutdown-limit`."
			type: uint: {
				default: 60
				unit:    "seconds"
			}
		}
		ANGLE_NO_GRACEFUL_SHUTDOWN_LIMIT: {
			description: "Never time out while waiting for graceful shutdown after SIGINT or SIGTERM received. This is useful when you would like for Angle to attempt to send data until terminated by a SIGKILL. Overrides/cannot be set with `--graceful-shutdown-limit-secs`."
			type: bool: default: false
		}
		ANGLE_OPENSSL_NO_PROBE: {
			description: """
				Disable probing and configuration of root certificate locations on the system for OpenSSL.

				The probe functionality manipulates the `SSL_CERT_FILE` and `SSL_CERT_DIR` environment variables in the Angle process. This behavior can be problematic for users of the `exec` source, which by default inherits the environment of the Angle process.
				"""
			type: bool: default: false
		}
		ANGLE_ALLOW_EMPTY_CONFIG: {
			description: """
