...
releases: "0.24.0": {
	...
	known_issues: [
		"""
			...
			""",
		"""
			`angle top` and some sinks like `file` incorrectly report metrics from the
	...
	description: """
		The Angle team is pleased to announce version 0.24.0!
		...
		- Support for expiring high cardinality internal metrics through the global `expire_metrics`
		  ...
		  Angle using increased memory over time. For now it is opt-in, but we may make this the
		  default in the future.

		Note that this release has a backwards incompatible data model change that users of the
		`angle` sink and disk buffers should be aware of while upgrading. See the [note in the
	...
	changelog: [
		...
		},
		{
			...
			description: """
				...
				[GELF](https://docs.graylog.org/docs/gelf) data in Angle's sources and sinks. It
		...
		},
		{
			...
			scopes: ["reload"]
			description: """
				Angle no longer panics when reloading a config where a component is deleted
				and then re-added.
				"""
		...
		},
		{
			...
			description: """
				Angle now has a concept of "rate" metrics which are a counter with an additional
		...
		},
		{
			...
			scopes: ["enrichment tables"]
			description: """
				Angle no longer panics after reloading an existing enrichment table that had index
				updates.
				"""
			pr_numbers: [13704]
		},
		...
		},
		{
			...
			description: """
				...
				a histogram of the time difference of when Angle ingests an event and the timestamp
		...
		},
		{
			...
			description: """
				Angle's internal metrics store, which is exposed via the `internal_metrics`
	...
	commits: [
		...
		{sha: "e14b2c1796d35118eef967bce1a94d38df9958f6", date: "2022-07-05 02:53:44 UTC", description: "Fix `cargo test --lib --package angle_common`", pr_number:                                   13430, scopes: ["tests"], type:                                                      "chore", breaking_change:       false, author: "Bruce Guenter", files_count:          1, insertions_count:    4, deletions_count:    0},
		...
		{sha: "18432ef398d5c3179db96edf2a5b7824cdedadda", date: "2022-07-08 03:16:54 UTC", description: "Fully qualify angle.khulnasoft.com docker image", pr_number:                                            13469, scopes: [], type:                                                             "chore", breaking_change:       false, author: "Spencer Gilbert", files_count:        1, insertions_count:    1, deletions_count:    1},
		...
		{sha: "238d3a1f3ae7ce7f0b92a29ecc85522b4bfa92d4", date: "2022-07-27 00:01:16 UTC", description: "fetch indexes if angle config is modified", pr_number:                                       13704, scopes: ["enriching"], type:                                                  "fix", breaking_change:         false, author: "Stephen Wakely", files_count:         2, insertions_count:    6, deletions_count:    1},
