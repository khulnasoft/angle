...
releases: "0.19.0": {
	...
	whats_next: [
		{
			...
			description: """
				...
				implementation specific to Angle's needs. The end result is
				faster disk buffers.
				"""
		},
		{
			...
			description: """
				We are in the process of ensuring that all Angle components
				report a consistent set of metrics to make it easier to monitor
				the performance of Angle.  These metrics are outlined in this
				new [instrumentation
	...
	known_issues: [
		"A regression was introduced that changed the name of the data directory for sinks using a disk buffer. This means, when upgrading from `v0.18.0`, if there was any data in existing disk buffers, it will not be sent. Angle's starting with clean or empty disk buffers are unaffected. Fixed in v0.19.1. See [#10430](https://github.com/khulnasoft/angle/issues/10430) for more details.",
		"Presence of `framing.character_delimited.delimiter` in configs causes Angle to fail to start with `invalid type: string`. Fixed in v0.19.1.",
	...
	description: """
		The Angle team is pleased to announce version 0.19.0!

		In addition to the below features, enhancements, and fixes, we've been
		hard at work improving Angle's performance and were able to move the
	...
	changelog: [
		...
		},
		{
			...
			description: """
				...
				sink to modify how Angle encodes events.

				A `measurement` config field was added to allow overriding the
				measurement name (previously hardcoded to `angle`). Along with
				...
				```text
				angle-logs,metric_type=logs,host=example.com message="hello world",size=10 {timestamp}
				```
				...
				```text
				ns.angle,metric_type=foo,host=example.com message="hello world",size=10 {timestamp}
				```
				"""
		...
		},
		{
			...
			description: """
				...
				logs first. This should result in better behavior with Angle
		...
		},
		{
			...
			scopes: ["config"]
			description: """
				Angle's support for environment variable expansion in
		...
		},
		{
			...
			description: """
				...
				able to send metrics to Angle. We expect this to be released in
		...
		},
		{
			...
			description: """
				...
				sink. This makes it easier to use Angle in-between a Splunk
		...
		},
		{
			...
			description: """
				Angle's CPU utilization has improved by running eligible
				transforms on multiple cores when possible. Previously,
				a transform could be a significant bottleneck since only one
				copy of it was ran which would result Angle under-utilizing
		...
		},
		{
			...
			description: """
				...
				to allow suppressing the `type` field from being sent by Angle.
		...
		},
		{
			...
			description: """
				...
				not logs while Angle was running.
				"""
			pr_numbers: [10425]
		},
		{
			...
			scopes: ["performance"]
			description: """
				We have improved Angle's performance for most use-cases by
				...
				see if they are a better fit for Angle's allocation patterns.
				"""
			pr_numbers: [10459]
		},
		{
			...
			scopes: ["sinks"]
			description: """
				Fix a panic that could occur during when reloading Angle config
		...
		},
		{
			...
			scopes: ["config"]
			description: """
				Angle, when using `--config-dir`, no longer tries to load
		...
		},
		{
			...
			description: """
				...
				sources previously caused Angle to stop decoding a given input
		...
		},
		{
			...
			description: """
				...
				one byte. Angle now correctly errors if a delimiter that is
	...
	commits: [
		...
		{sha: "664dac0ac13fb5bd8d69ac824f9c056f42857f57", date: "2021-11-18 15:52:34 UTC", description: "Add additional label so Angle service works in soak tests", pr_number:                       10094, scopes: [], type:                                              "chore", breaking_change:       false, author: "Spencer Gilbert", files_count:      1, insertions_count:   9, deletions_count:    5},
		...
		{sha: "00777ee3aa06c96482715c85f18aaefcecc69465", date: "2021-11-24 16:09:21 UTC", description: "rewrite angle sink (v2) in the new style", pr_number:                                        10148, scopes: ["angle sink"], type:                                 "chore", breaking_change:       false, author: "Nathan Fox", files_count:           20, insertions_count:  1104, deletions_count: 807},
		...
		{sha: "31883416edcc58a8bb3ffda3013e806d3012ff77", date: "2021-12-03 02:53:06 UTC", description: "Remove references to playground.angle.khulnasoft.com", pr_number:                                       10242, scopes: [], type:                                              "docs", breaking_change:        false, author: "Jesse Szwedko", files_count:        5, insertions_count:   21, deletions_count:   22},
		...
		{sha: "877d22090ff56b0e79e0649ab00c3771fb52c1de", date: "2021-12-09 05:59:24 UTC", description: "Clarify that `/etc/angle` just includes configuration", pr_number:                           10361, scopes: [], type:                                              "docs", breaking_change:        false, author: "Jesse Szwedko", files_count:        1, insertions_count:   3, deletions_count:    0},
