...
releases: "0.22.0": {
	...
	known_issues: [
		"The `journald` source deadlocks almost immediately ([#12966](https://github.com/khulnasoft/angle/issues/12966)). Fixed in v0.22.1.",
		"The `kubernetes_logs` source does not work with k3s/k3d ([#12989](https://github.com/khulnasoft/angle/issues/12989)). Fixed in v0.22.1.",
		"Angle would panic when reloading configuration using the `compression` or `concurrency` options due to a deserialization failure ([#12919](https://github.com/khulnasoft/angle/issues/12919)). Fixed in v0.22.1.",
		"When using a component that creates a unix socket, `angle validate` no longer creates the socket ([#13018](https://github.com/khulnasoft/angle/issues/13018)). This causes the default SystemD unit file to fail to start Angle since it runs `angle validate` before starting Angle. Fixed in v0.22.1.",
		"VRL sometimes miscalculates type definitions when conditionals are used causing later usages of values assigned in conditionals to not require type coercion as they should ([#12948](https://github.com/khulnasoft/angle/issues/12948)). Fixed in v0.22.1.",
		...
		"Angle shuts down when a configured source codec (`decoding.codec`) receives invalid data. Fixed in v0.23.1.",
	]

	description: """
		The Angle team is pleased to announce version 0.22.0!
		...
		**Important**: as part of this release, we have promoted the new implementation of disk buffers (`buffer.type
		...
		a back up of the disk buffers (in the configured `data_dir`, typically in `/var/lib/angle`) to roll back if
		...
		- [Support for iteration has landed in VRL](/highlights/2022-05-18-vrl-iteration-support). Now you can
		  ...
		  the `lua` transform with the much more performant [`remap`](\(urls.angle_remap_transform)) transform.
		- [New native event codecs](/highlights/2022-03-31-native-event-codecs) for Angle. We are still rolling out the
		  new codec support to all sinks, but this will allow sending events (logs, metrics, and traces) between Angle
		  instances via transports like `kafka` rather than being limited to the gRPC `angle` source and sink.
	...
	changelog: [
		...
		},
		{
			...
			scopes: ["buffers"]
			description: """
				Angle now automatically migrates disk buffers from `disk_v1` to `disk_v2` as described in [Switching
		...
		},
		{
			...
			description: """
				...
				generate initialization angles for the `encrypt` function.
		...
		},
		{
			...
			description: """
				...
				for read data to be processed by Angle.
				"""
			pr_numbers: [12209]
		},
		{
			...
			scopes: ["config"]
			description: """
				Angle's configuration interpolation of environment variables has been enhanced to both allow setting of
		...
		},
		{
			...
			scopes: ["config"]
			description: """
				When using Angle's ability to load configuration from a directory (`--config-dir`), Angle now ignores
		...
		},
		{
			...
			scopes: ["codecs"]
			description: """
				Angle has two new codecs that can be used on sources and sinks to encode as Angle's native
				representation: `native` and `native_json`. This makes it easier to send events between Angle instances
				on transports like `kafka`. It also makes it possible to send metrics to Angle from an external process
		...
		},
		{
			...
			description: """
				The `internal_logs` source occasionally missed some events generated early in Angle's start-up, before
		...
		},
		{
			...
			scopes: ["releasing"]
			description: """
				The RPM package now adds the created `angle` user to the `systemd-journal-remote` group to be able to
		...
		},
		{
			...
			description: """
				The `kubernetes_logs` source now allows configuration of `extra_namespace_label_selector` which Angle
		...
		},
		{
			...
			description: """
				...
				Angle could start reading the new file before it finished processing the previous one, resulting in the
		...
		},
		{
			...
			description: """
				...
				passed around in Angle's internal topology. Note that this is different than sink-level batching. It is
				mostly useful for debugging performance issues in Angle due to low internal batching.
				"""
			pr_numbers: [11290]
		},
		{
		...
		},
		{
			...
			description: """
				The `aws_s3` source now handles S3 object keys that contain spaces. Previously Angle would encounter
		...
		},
		{
			...
			scopes: ["observability"]
			description: """
				All components now emit consistent metrics in accordance with Angle's [component
				specification](https://github.com/khulnasoft/angle/blob/927fd2eeb4ee15b10f8c046f0f9347789ca1c356/docs/specs/component.md).
				"""
	...
	commits: [
		...
		{sha: "3eadc96742a33754a5859203b58249f6a806972a", date: "2022-04-28 03:48:45 UTC", description: "Rename Angle development image", pr_number:                                                 12439, scopes: [], type:                                      "chore", breaking_change:       false, author: "Jesse Szwedko", files_count:      1, insertions_count:    3, deletions_count:    3},
		...
		{sha: "e222294361b92aa2495b4423ebb9115ce6b7e613", date: "2022-05-03 22:49:54 UTC", description: "Document angle config subcommand", pr_number:                                               12500, scopes: ["cli"], type:                                 "docs", breaking_change:        false, author: "Will", files_count:               1, insertions_count:    34, deletions_count:   12},
		...
		{sha: "2fa5495be993d6756f72aedaf4d58867c4865a0b", date: "2022-05-05 21:37:55 UTC", description: "Allow stdout/stderr in angle config tests", pr_number:                                      12600, scopes: [], type:                                      "chore", breaking_change:       false, author: "Jesse Szwedko", files_count:      1, insertions_count:    2, deletions_count:    0},
		...
		{sha: "58abdce840f05069221ef342a52ccf0b0baff14a", date: "2022-05-16 23:13:44 UTC", description: "Hide angle config subcommand", pr_number:                                                   12713, scopes: ["cli"], type:                                 "chore", breaking_change:       false, author: "Will", files_count:               4, insertions_count:    319, deletions_count:  66},
		...
		{sha: "41bf25ad43869448c28f8e8abaa840f5536f2314", date: "2022-05-19 08:53:22 UTC", description: "Add angle version and target triple info to enterprise logs", pr_number:                    12774, scopes: ["observability"], type:                       "chore", breaking_change:       false, author: "Will", files_count:               2, insertions_count:    30, deletions_count:   1},
