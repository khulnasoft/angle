...
releases: "0.21.0": {
	...
	known_issues: [
		"The `kubernetes_logs` source can panic when while processing Kubernetes watcher events when there is an error. [#12245](https://github.com/khulnasoft/angle/issues/12245). Fixed in `0.21.1`.",
		"The `elasticsearch` sink fails to include the security token when signing requests for AWS authentication to OpenSearch. [#12249](https://github.com/khulnasoft/angle/issues/12249). Fixed in `0.21.1`.",
		"The `nats` source and sink authentication options were not configurable. [#12262](https://github.com/khulnasoft/angle/issues/12262). Fixed in `0.21.1`.",
		"The `internal_logs` source includes excess trace logs whenever `angle top` is used. [#12251](https://github.com/khulnasoft/angle/issues/12251). Fixed in `0.21.1`.",
		"The `aws_cloudwatch_logs` source does not handle throttle responses from AWS. [#12253](https://github.com/khulnasoft/angle/issues/12253). Fixed in `0.21.1`.",
		"Angle panics when loading configuration that includes event paths like `encoding.only_fields`. [#12256](https://github.com/khulnasoft/angle/issues/12256). Fixed in `0.21.1`.",
		"Angle panicked when reloading configuration that added components to a running topology. [#12273](https://github.com/khulnasoft/angle/issues/12273). Fixed in `0.21.1`.",
		"Using `assume_role` on AWS components did not function correctly. [#12314](https://github.com/khulnasoft/angle/issues/12314). Fixed in `0.21.1`.",
		"The Angle VRL REPL loses variable assignments if the expression being evaluated errors. [#12400](https://github.com/khulnasoft/angle/issues/12400). Fixed in `0.21.2`.",
		"Angle docker images require a volume to be mounted at `/var/lib/angle` to start correctly when the default `data_dir` of is used. [#12413](https://github.com/khulnasoft/angle/issues/12413). Fixed in `0.21.2`.",
		"For AWS components, the timeout for loading credentials was dropped from 30 seconds to 5 seconds. [#12421](https://github.com/khulnasoft/angle/issues/12421). `0.21.2` adds a new option, `load_timeout_secs` that can be configured to a higher value.",
		"`angle generate` works again with the `datadog_agent` source. [#12469](https://github.com/khulnasoft/angle/issues/12469). Fixed in `0.21.2`.",
	...
	whats_next: [
		{
			...
			description: """
				We are [in the process](https://github.com/khulnasoft/angle/pull/11802) of adding a source for
				...
				We'll also be adding an OpenTelemetry sink for forwarding data from Angle to OpenTelemetry-compatible APIs.
				"""
		},
		{
			title:       "Component metric standardization"
			description: """
				We continue to be in the process of ensuring that all Angle components report a consistent set of
				metrics to make it easier to monitor the performance of Angle.  These metrics are outlined in this new
		...
		},
		{
			...
			description: """
				...
				See the
				[RFC](https://github.com/khulnasoft/angle/blob/master/rfcs/2021-08-29-8381-vrl-iteration-support.md)
	...
	description: """
		The Angle team is pleased to announce version 0.21.0!
		...
		- A new implementation of the VRL runtime as a Virtual Machine (VM). This new implementation improves
		  ...
		  [report](https://github.com/khulnasoft/angle/issues/new?assignees=&labels=type%3A+bug&template=bug.yml) any
		  issues you find.
		...
		- The `kubernetes_logs` source has been rewritten to use the community supported
		  ...
		  outstanding bugs with Angle ceasing to process container logs. It also adds support for Kubernetes
		...
		framework](https://github.com/khulnasoft/angle/tree/master/soaks/tests)).

		Also, check out our [new guide](/guides/level-up/angle-tap-guide) on using `angle tap` for observing events
		running through Angle instances
		"""
	...
	changelog: [
		...
		},
		{
			...
			description: """
				We are in the process of updating all Angle components with consistent instrumentation as described in
				[Angle's component
		...
		},
		{
			...
			description: """
				Angle's `/health` endpoint (mounted when `api.enabled` is `true`) now returns a 503 when Angle is
		...
		},
		{
			...
			scopes: ["observability"]
			description: """
				`angle tap` and `angle top` have had a few enhancements.
				...
				* A `--meta` flag was added to include metadata about which component the output events came from.
				...
				Both `angle top` and `angle tap` now automatically reconnect if the remote Angle instance goes away.
		...
		},
		{
			...
			description: """
				`docker_logs` source now exits, shutting down Angle, if it hits
		...
		},
		{
			...
			description: """
				Initial support was added for ingesting traces from the Datadog Agent into Angle (via the
				...
				forward traces to it through Angle. We will be following up to add support APM metrics to Angle.

				Datadog docs are forthcoming but the Agent configuration option, `apm_config.apm_dd_url`, can be used to
				forward traces from the Datadog Agent to Angle.
				"""
		...
		},
		{
			...
			scopes: ["reload"]
			description: """
				A case where Angle would panic during reload was fixed that would occur whenever a component has
		...
		},
		{
			...
			scopes: ["releasing"]
			description: """
				For Debian packages, the created `angle` user is now added to the `systemd-journal-remote` group, if it
				exists, to facilitate Angle being used to collect remote journald logs.
				"""
			pr_numbers: [11713]
		},
		{
		...
		},
		{
			...
			scopes: ["loki sink"]
			description: """
				The `loki` sink now supports setting `out_of_order_action` to `accept` to instruct Angle to not modify
				event timestamps. Angle would previously modify timestamps to attempt to satisfy Loki's ordering
				constraints, but these constraints were relaxed in Loki 2.4. If you are running Loki >= 2.4 it is
				recommended to set `out_of_order_action` to `accept` to enable Angle to send data concurrently.
				"""
		...
		},
		{
			...
			scopes: ["journald source"]
			description: """
				The `journald` source now supports a `since_now` option to instruct Angle to only fetch journal entries
				that occur after Angle starts.
				"""
			pr_numbers: [11799]
		},
		{
			...
			description: """
				...
				behavior. We have an [open issue](https://github.com/khulnasoft/angle/issues/11817) for reloading the
				database from disk during Angle's reload process.
				"""
			pr_numbers: [11831]
		},
		{
			...
			scopes: ["observability"]
			description: """
				Ensure that instrumentation labels on internal logs and metrics are not lost when Angle is run in quiet
		...
		},
		{
			...
			description: """
				...
				`angle-${ANGLE_VERSION}-${PLATFORM}.deb` to `angle_${ANGLE_VERSION}_${PLATFORM}-${REV}.deb`. `REV`
				is typically `1`.
				"""
			pr_numbers: [11887]
		},
		...
		},
		{
			...
			scopes: ["observability"]
			description: """
				`angle top` now reports error metrics correctly again.
				"""
			pr_numbers: [11973]
		},
		{
		...
		},
		{
			...
			description: """
				...
				- `visibility_timeout_secs` to control how long messages are locked for
				  ...
				  a message will be "sent" if a Angle instance crashes before deleting the message.
		...
		},
		{
			...
			breaking: true
			description: """
				Angle's published docker images no longer include `VOLUME` declarations. Instead, users should provide
				a volume at runtime if they require one. This avoids the behavior of Angle creating a volume for its
		...
		},
		{
			...
			description: """
				...
				configured `endpoint`s via the new `query` option. This is useful when using Angle with a federated
				Prometheus endpoint.
				"""
			pr_numbers: [12033]
		},
		{
			...
			scopes: ["angle sink"]
			description: """
				The `angle sink` can now enable gzip compression by setting `compression` to `true`.
				"""
			pr_numbers: [12059]
		},
		{
	...
	commits: [
		...
		{sha: "16cf27b6bb8d229d27f54ea887b9600c928bf65b", date: "2022-02-12 05:57:08 UTC", description: "Allow health endpoint to return 503s when Angle is shutting down", pr_number:                   11183, scopes: ["api"], type:                                    "feat", breaking_change:        false, author: "Spencer Gilbert", files_count:      6, insertions_count:    108, deletions_count:  11},
		...
		{sha: "30b89abdccc206d11cac80bd81670de9a54763dc", date: "2022-02-17 07:07:19 UTC", description: "fix angle-core feature flags", pr_number:                                                       11421, scopes: [], type:                                         "chore", breaking_change:       false, author: "Luke Steensen", files_count:        4, insertions_count:    9, deletions_count:    9},
		...
		{sha: "fb445804144109ab693d65bdd367cd054a1cd39f", date: "2022-02-22 05:43:31 UTC", description: "Support tapping component inputs with angle tap", pr_number:                                    11293, scopes: ["api"], type:                                    "enhancement", breaking_change: false, author: "Will", files_count:                 11, insertions_count:   798, deletions_count:  63},
		{sha: "51ee56ea2577a0078effa82b79947fbc5eb1b855", date: "2022-02-22 17:37:21 UTC", description: "Add --quiet option to angle tap", pr_number:                                                    11321, scopes: ["api"], type:                                    "chore", breaking_change:       false, author: "Will", files_count:                 5, insertions_count:    46, deletions_count:   22},
		...
		{sha: "9855ea6a2ae9066cdbfa1fb3e9294b3a6f1a9176", date: "2022-02-26 05:14:11 UTC", description: "Add angle tap guide", pr_number:                                                                11517, scopes: ["guides website"], type:                         "docs", breaking_change:        false, author: "Will", files_count:                 2, insertions_count:    261, deletions_count:  1},
		...
		{sha: "258c49aabfc093563f6c31a76d92e9556d5bf4bf", date: "2022-02-26 15:25:11 UTC", description: "Change angle top --human_metrics short flag", pr_number:                                        11581, scopes: ["api"], type:                                    "fix", breaking_change:         false, author: "Will", files_count:                 3, insertions_count:    11, deletions_count:   2},
		...
		{sha: "82264fa6c280fcee6cdaf0c95074f8c6ddaf3f8f", date: "2022-03-01 10:06:29 UTC", description: "Add --meta flag to angle tap", pr_number:                                                       11531, scopes: ["api"], type:                                    "enhancement", breaking_change: false, author: "Will", files_count:                 12, insertions_count:   339, deletions_count:  67},
		...
		{sha: "8d54ee33f17f0f8374a181494e75ca9f3b0ef8ff", date: "2022-03-04 02:16:27 UTC", description: "Warn on invalid matches in angle tap", pr_number:                                               11411, scopes: ["api"], type:                                    "chore", breaking_change:       false, author: "Will", files_count:                 9, insertions_count:    442, deletions_count:  118},
		...
		{sha: "7372b4270208e827446be4f4f187d74813172eed", date: "2022-03-05 06:13:06 UTC", description: "Add `angle config` subcommand to output a normalized configuration", pr_number:                 11442, scopes: ["cli"], type:                                    "feat", breaking_change:        false, author: "Lee Benson", files_count:           10, insertions_count:   596, deletions_count:  467},
		...
		{sha: "1a3abf6f0e941b94e5f1d9858d882460fa90479f", date: "2022-03-08 16:56:08 UTC", description: "Add angle user to systemd-journal-remote group", pr_number:                                     11713, scopes: ["releasing"], type:                              "enhancement", breaking_change: false, author: "Priit Laes", files_count:           1, insertions_count:    8, deletions_count:    3},
		...
		{sha: "f662074d41ed1d69d5f639361f1b89dfefbcd315", date: "2022-03-26 00:12:47 UTC", description: "Update error metrics in angle top", pr_number:                                                  11973, scopes: ["api"], type:                                    "fix", breaking_change:         false, author: "Will", files_count:                 5, insertions_count:    60, deletions_count:   27},
		...
		{sha: "7bf62b8d0e9e7d66371367d9d08420f323aca88d", date: "2022-04-06 18:58:40 UTC", description: "compression support for angle v2", pr_number:                                                   12059, scopes: ["angle sink"], type:                            "feat", breaking_change:        false, author: "Mathew Heard", files_count:         6, insertions_count:    58, deletions_count:   6},
