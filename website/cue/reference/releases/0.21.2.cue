...
releases: "0.21.2": {
	...
	changelog: [
		{
			...
			scopes: ["vrl"]
			description: """
				Angle's VRL REPL no longer loses variable assignments when the expression errors.
				"""
			pr_numbers: [12405]
		},
		{
			...
			description: """
				Angle's docker images no longer need a volume mounted at `/var/lib/docker` to run correctly with the
		...
		},
		{
			...
			scopes: ["config"]
			description: """
				`angle generate` now works correctly again when `datadog_agent` is used as a source rather than logging
				an error.
				"""
			pr_numbers: [12470]
		},
		{
			...
			scopes: ["aws provider"]
			description: """
				AWS components now check for a region at Angle start-up time so that they return an error rather than
	...
	commits: [
		...
		{sha: "f3b75a6a8523d1b1708c91eae6ce608c02e2eb5d", date: "2022-04-27 07:26:29 UTC", description: "add empty `/var/lib/angle` directory to Docker images", pr_number: 12414, scopes: ["docker platform"], type:      "fix", breaking_change:         false, author: "Hugo Hromic", files_count:     6, insertions_count:  14, deletions_count: 0},
