...
releases: "0.23.3": {
	...
	description: """
		...
		mismatch during release](https://github.com/khulnasoft/angle/pull/13930).
	...
	changelog: [
		{
			...
			scopes: ["codec"]
			description: """
				Angle no longer shutsdown when `encoding.codec` is configured on a source and the
		...
		},
		{
			...
			description: """
				...
				expected behavior as of Angle v0.23.0 when the `remove_empty` parameter was
				dropped.
				"""
			pr_numbers: [13721]
		},
	...
	commits: [
		...
		{sha: "b6deeffa654a61aebbcc79dabaf7b75ff85dcb13", date: "2022-07-30 14:03:59 UTC", description: "Fix bug where Angle shuts down when a decoding error is returned to `FramedRead`", pr_number: 13737, scopes: ["codecs"], type:             "fix", breaking_change:         false, author: "Pablo Sichert", files_count:   3, insertions_count:  44, deletions_count:  2},
