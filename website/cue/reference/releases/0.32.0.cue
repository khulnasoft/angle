...
releases: "0.32.0": {
	...
	description: """
		The Angle team is pleased to announce version 0.32.0!
	...
	changelog: [
		...
		},
		{
			...
			scopes: ["angle sink"]
			description: """
				The `angle` sink now considers `DataLoss` responses to be hard errors at indicates
				the a sink in the downstream `angle` source rejected the data. The `angle` sink
		...
		},
		{
			...
			scopes: ["angle sink"]
			description: """
				The `angle` sink now correctly applies configured HTTPS proxy settings. Previously
		...
		},
		{
			...
			description: """
				A new `greptimedb` sink was added allowing Angle to send metrics to
				[GreptimeDB](https://github.com/greptimeteam/greptimedb).
				"""
		...
		},
		{
			...
			scopes: ["reload"]
			description: """
				The Angle API can now correctly be disabled during reload by setting `api.enabled`
				to `false`.
				"""
		...
		},
		{
			...
			description: """
				...
				how long Angle should wait for each request.
				"""
			contributors: ["nullren"]
			pr_numbers: [18021]
		},
		{
			...
			scopes: ["releasing"]
			description: """
				Angle's `debian` Docker images are now based on Debian 12 (Bookworm).
				"""
			pr_numbers: [18057]
		},
		{
		...
		},
		{
			...
			scopes: ["codecs"]
			description: """
				Angle sources that support codecs now support `protobuf` as an option. A Protobuf
		...
		},
		{
			...
			scopes: ["config"]
			description: """
				Numeric compression levels can now be set when using TOML. Previously Angle would
		...
		},
		{
			...
			scopes: ["angle source"]
			description: """
				The `angle` source no longer fails to decode large payloads. This was a regression
	...
	commits: [
		...
		{sha: "9c0d2f2a9bd0b50c5e1c703f4087b1e297c8ece6", date: "2023-07-06 23:38:33 UTC", description: "Bump Angle to 0.32.0", pr_number:                                                          17887, scopes: ["releasing"], type:                                                  "chore", breaking_change:       false, author: "Jesse Szwedko", files_count:      2, insertions_count:  2, deletions_count:    2},
		...
		{sha: "4ef0b1778923567c8aa755e28d9419c52b6bc97c", date: "2023-07-08 03:25:09 UTC", description: "Add DataLoss error code as non-retryable", pr_number:                                       17904, scopes: ["angle sink"], type:                                                "fix", breaking_change:         false, author: "Scott Balmos", files_count:       1, insertions_count:  1, deletions_count:    0},
		...
		{sha: "45e24c73e78d3daf609103635950245dcc715444", date: "2023-07-08 03:24:29 UTC", description: "cert verification with proxy enabled", pr_number:                                           17651, scopes: ["angle sink"], type:                                                "fix", breaking_change:         false, author: "joe miller", files_count:         1, insertions_count:  3, deletions_count:    18},
		...
		{sha: "4cc9cdf04cbd2e25426ca3283b76c5b3eee93565", date: "2023-08-09 03:48:13 UTC", description: "Remove the 4MB default for requests", pr_number:                                            18186, scopes: ["angle source"], type:                                              "fix", breaking_change:         false, author: "Jesse Szwedko", files_count:      1, insertions_count:  3, deletions_count:    1},
