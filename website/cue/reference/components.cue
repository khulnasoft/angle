	...
	{[Kind=string]: [Name=string]: {
		...
		configuration: {
			...
			_gcp_credentials_path: {
				common:      true
				description: "The filename for a Google Cloud service account credentials JSON file used to authenticate access to the pubsub project and topic. If this is unset, Angle checks the `GOOGLE_APPLICATION_CREDENTIALS` environment variable for a filename.\n\nIf no filename is named, Angle will attempt to fetch an instance service account for the compute instance the program is running on. If Angle is not running on a GCE instance, you must define a credentials file as above."
				required:    false
				type: string: {
					default: null
			...
			_timezone: {
				...
				description: """
					...
					zone. This overrides the global [`timezone` option](\(urls.angle_configuration)/global-options#timezone).
			...
			"type": {
				description: "The component type. This is a required field for all components and tells Angle which component to use."
				required:    true
				sort:        -2
				"type": string: {
					enum: #Enum | *{
