...
components: _aws: {
	...
	how_it_works: {
		aws_authentication: {
			...
			body:  """
				Angle checks for AWS credentials in the following order:
				...
				If no credentials are found, Angle's health check fails and an error is [logged](\(urls.angle_monitoring)).
				"""
			sub_sections: [
				{
				...
				},
				{
					...
					body: """
						Angle can assume an AWS IAM role via the [`auth.assume_role`](#auth.assume_role) option. This is an
