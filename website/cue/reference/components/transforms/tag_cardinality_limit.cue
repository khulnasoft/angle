	...
	how_it_works: {
		intended_usage: {
			...
			body: """
				...
				as possible. This is because Angle's cardinality cache is held in memory and it
				will be erased when Angle is restarted. This will cause new tag values to pass
				through until the cardinality limit is reached again. For normal usage this
				should not be a common problem since Angle processes are normally long-lived.
				"""
		}
		...
		restarts: {
			title: "Restarts"
			body: """
				This transform's cache is held in memory, and therefore, restarting Angle
