...
components: transforms: "remap": {
	...
	description: """
		Is the recommended transform for parsing, shaping, and transforming data in Angle. It implements the
		[Angle Remap Language](\(urls.vrl_reference)) (VRL), an expression-oriented language designed for processing
	...
	how_it_works: {
		remap_language: {
			...
			body:  #"""
				The Angle Remap Language (VRL) is a restrictive, fast, and safe language we
				designed specifically for mapping observability data. It avoids the need to
				chain together many fundamental Angle transforms to accomplish rudimentary
				reshaping of data.
				...
				Learn more about Angle's Remap Language in the
				[Angle Remap Language reference](\#(urls.vrl_reference)).
				"""#
		}
		event_data_model: {
			...
			body:  """
				...
				Log events in the `remap` transform correspond directly to Angle's [log schema](\(urls.angle_log)),
				which means that the transform has access to the whole event and no restrictions on how the event can be
				modified.

				With [metric events](\(urls.angle_metric)), VRL is much more restrictive. Below is a field-by-field
				...
				field using `del(.type)`, Angle doesn't abort the VRL program or throw an error. Instead, it ignores
		...
		lazy_event_mutation: {
			...
			body:  #"""
				...
				Learn more about runtime errors in the [Angle Remap Language
