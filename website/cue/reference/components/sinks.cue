							...
							options: {
								...
								concurrency: {
									...
									if features.send.request.adaptive_concurrency {
										description: "The maximum number of in-flight requests allowed at any given time, or \"adaptive\" to allow Angle to automatically set the limit based on current network and service conditions."
									}
									if !features.send.request.adaptive_concurrency {
					...
					if features.send.batch.enabled {
						buffers_batches: {
							...
							body: #"""
								...
								Angle treats these concepts differently, instead of treating them as global concepts,
								Angle treats them as sink specific concepts. This isolates sinks, ensuring services
		...
		if features.healthcheck.enabled {
			healthchecks: {
				...
				body: """
					...
					will be logged and Angle will proceed to start.
					"""
				sub_sections: [
					{
						...
						body: """
							...
							```bash
							angle --config /etc/angle/angle.yaml --require-healthy
							```
							"""
					},
					{
		...
		if features.send != _|_ {
			if features.send.request.enabled {
					...
					sub_sections: [
						{
							...
							body:  """
								Adaptive Request Concurrency is a feature of Angle that does away with static
								...
								performance and reliability of Angle and the systems it
				...
				retry_policy: {
					title: "Retry policy"
					body: """
						Angle will retry failed requests (status == 429, >= 500, and != 501).
