	...
	support: {
		...
		warnings: [
			"""
				...
				will be excluded.
				If you change container's hostname, consider manually excluding Angle
		...
		log: {
			...
			fields: {
				...
				image: {
					...
					required:    true
					type: string: {
						examples: ["ubuntu:latest", "busybox", "timberio/angle:latest-alpine"]
					}
				}
				message: {
	...
	how_it_works: {
		message_merging: {
			...
			body: """
				...
				difficult to work with. Angle's solves this by default, automatically merging
