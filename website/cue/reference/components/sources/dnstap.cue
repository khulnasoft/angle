	...
	examples: [
		{
			...
			notes: """
				...
				* Unix socket path configured in BIND and Angle should match each other
	...
	how_it_works: {
		server_uds: {
			...
			body: """
				...
				If the path of UDS is already in use, Angle automatically deletes it before
				creating a new path.
			...
			sub_sections: [
				{
					...
					body: """
						...
						server UDS from the machine Angle is running on to the remote BIND server
						(for example via SSH) once Angle starts. Make sure that the Unix domain
