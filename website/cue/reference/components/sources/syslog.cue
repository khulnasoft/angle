...
components: sources: syslog: {
	...
	output: logs: line: {
		...
		fields: {
			...
			timestamp: {
				description: "The time extracted from the Syslog formatted line. If parsing fails, then the exact time the event was ingested into Angle is used."
				required:    true
				type: timestamp: {}
			}
			version: {
	...
	how_it_works: {
		...
		parsing: {
			title: "Parsing"
			body:  """
				Angle makes a _best effort_ to parse the various Syslog formats out in the wild.
				...
				accurately followed, but we hope that Angle insulates you from these deviations.

				If parsing fails, Angle will raise an error. If you find this happening often,
				we recommend using the [`socket` source](\(urls.angle_socket_source)) combined with
