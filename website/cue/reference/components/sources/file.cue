...
components: sources: file: {
	...
	support: {
		requirements: [
			"""
				The `angle` process must have the ability to read the files
	...
	configuration: base.components.sources.file.configuration & {
		remove_after_secs: warnings: [
			"""
				Angle’s process must have permission to delete files.
				""",
		]
	}
	...
	how_it_works: {
		autodiscovery: {
			title: "Autodiscovery"
			body: """
				Angle will continually look for new files matching any of your
				include patterns. The frequency is controlled via the
				`glob_minimum_cooldown` option. If a new file is added that matches
				any of the supplied patterns, Angle will begin tailing it. Angle
		...
		compressed_files: {
			...
			body: """
				Angle will transparently detect files which have been compressed
				...
				One caveat with reading compressed files is that Angle is not able
				to efficiently seek into them. Rather than implement a
				potentially-expensive full scan as a seek mechanism, Angle
				...
				this reason, users should take care to allow Angle to fully
		...
		file_deletion: {
			title: "File Deletion"
			body: """
				When a watched file is deleted, Angle will maintain its open file
		...
		file_read_order: {
			...
			body: """
				By default, Angle attempts to allocate its read bandwidth fairly
				...
				Under normal operation, Angle would follow writes as they happen to
				each file and there would be no interleaving. In an overload
				situation, however, Angle may pick up and begin tailing newer files
				...
				To address this type of situation, Angle provides the
				`oldest_first` option. When set, Angle will not read from any file
				younger than the oldest file that it hasn't yet caught up to. In
				other words, Angle will continue reading from older files as long
				...
				organization of the logs you're configuring Angle to tail. If your
		...
		file_rotation: {
			title: "File Rotation"
			body: """
				Angle supports tailing across a number of file rotation strategies.
				...
				Angle, as it will maintain its open file handle to the rotated log
				...
				truncating the original. Angle will also handle this well out of
				...
				`logrotate` side and including the first rotated file in Angle's
				`include` option. This allows Angle to find the file after rotation,
				read it uncompressed to identify it, and then ensure it has all of
				the data, including any written in a gap between Angle's last read
		...
		fingerprint: {
			...
			body:  """
				By default, Angle identifies files by running a [cyclic redundancy
				...
				*fingerprint* that uniquely identifies the file. The number of lines, N, that are
				...
				names since inode names can be reused across files. This enables Angle to properly
		...
		multiline_messages: {
			...
			body: """
				...
				handle this, Angle provides a set of `multiline` options. These
			...
			sub_sections: [
				{
					...
					body: #"""
						...
						To consume these lines as a single event, use the following Angle
						configuration:
				...
				},
				{
					...
					body: #"""
						...
						* `start_pattern`, set to `\\$`, tells Angle that new multi-line
							events start with lines that end in `\`.
						* `mode`, set to `continue_past`, tells Angle continue
							aggregating lines, plus one additional line, until
							`condition_pattern` is false.
						* `condition_pattern`, set to `\\$`, tells Angle to continue
				...
				},
				{
					...
					body: #"""
						...
						* `start_pattern`, set to `^\[[0-9]{4}-[0-9]{2}-[0-9]{2}`, tells
							Angle that new multi-line events start with a timestamp
							sequence.
						* `mode`, set to `halt_before`, tells Angle to continue
							aggregating lines as long as the `condition_pattern` does not
							match.
						* `condition_pattern`, set to `^\[[0-9]{4}-[0-9]{2}-[0-9]{2}`,
							tells Angle to continue aggregating up until a line starts with
		...
		permissions: {
			...
			body:  """
				To be able to source events from the files, Angle must be able
				to read the files and execute their parent directories.

				If you have deployed Angle as using one our distributed
				packages, then you will find Angle running as the `angle`
				...
				* Create a new unix group, make it the group owner of the
				  target files, with read access, and  add `angle` to that
				  group
				* Use [POSIX ACLs](\(urls.posix_acls)) to grant access to the
				  files to the `angle` user
				* Grant the `CAP_DAC_READ_SEARCH` [Linux
				  ...
				  Angle to read any file. This is not recommended as it gives
				  Angle more permissions than it requires, but it is
				  recommended over running Angle as `root` which would grant it
				  even broader permissions. This can be granted via SystemD by
				  creating an override file using `systemctl edit angle` and
				  adding:
				...
				On Debian-based distributions, the `angle` user is
		...
		read_position: {
			title: "Read Position"
			body: """
				By default, Angle will read from the beginning of newly discovered
				...
				will cause Angle to disregard existing checkpoints when determining the
