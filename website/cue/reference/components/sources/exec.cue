...
components: sources: exec: {
	...
	how_it_works: {
		...
		shutdown: {
			title: "Shutting Down"
			body: """
				When Angle begins shutting down (typically due to a SIGTERM), this source will
				signal to the child process to terminate, if it is running, to shut down.

				On *nix platforms, Angle will issue a SIGTERM to the child process, allowing it to
				gracefully shutdown, and the source will continue reading until the process exits or
				Angle's shutdown grace period expires. The duration of the grace period can be
