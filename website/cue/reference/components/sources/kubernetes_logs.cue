...
components: sources: kubernetes_logs: {
	...
	how_it_works: {
		enrichment: {
			title: "Enrichment"
			body:  """
					Angle will enrich data with Kubernetes context. A comprehensive
					list of fields can be found in the
					[`kubernetes_logs` source output docs](\(urls.angle_kubernetes_logs_source)#output-data).
					"""
		}
		...
		filtering: {
			...
			body: """
				...
				* The `extra_label_selector` option specifies the label selector to
				  ...
				  `angle.khulnasoft.com/exclude` filter](#pod-exclusion).
				"""
		}
		...
		globbing: {
			title: "Globbing"
			body:  """
				By default, the [`kubernetes_logs` source](\(urls.angle_kubernetes_logs_source))
				ignores compressed and temporary files. This behavior can be configured with the
				[`exclude_paths_glob_patterns`](\(urls.angle_kubernetes_logs_source)#configuration) option.
		...
		namespace_exclusion: {
			...
			body:  """
					By default, the [`kubernetes_logs` source](\(urls.angle_kubernetes_logs_source))
					will skip logs from the Namespaces that have a `angle.khulnasoft.com/exclude: "true"` **label**.
					You can configure additional exclusion rules via label selectors,
					see [the available options](\(urls.angle_kubernetes_logs_source)#configuration).
					"""
		}
		...
		pod_exclusion: {
			...
			body:  """
					By default, the [`kubernetes_logs` source](\(urls.angle_kubernetes_logs_source))
					will skip logs from the Pods that have a `angle.khulnasoft.com/exclude: "true"` **label**.
		...
		container_exclusion: {
			...
			body:  """
					The [`kubernetes_logs` source](\(urls.angle_kubernetes_logs_source))
					can skip the logs from the individual Containers of a particular
					Pod. Add an **annotation** `angle.khulnasoft.com/exclude-containers` to the
					...
					This annotation will make Angle skip logs originating from the
		...
		kubernetes_api_communication: {
			...
			body:  """
					Angle communicates with the Kubernetes API to enrich the data it collects with
					Kubernetes context. Therefore, Angle must have access to communicate with the
					[Kubernetes API server](\(urls.kubernetes_api_server)). If Angle is running in
					a Kubernetes cluster then Angle will connect to that cluster using the
					[Kubernetes provided access information](\(urls.kubernetes_accessing_api_from_pod)).

					In addition to access, Angle implements proper desync handling to ensure
					communication is safe and reliable. This ensures that Angle will not overwhelm
		...
		partial_message_merging: {
			...
			body:  """
					Angle, by default, will merge partial messages that are
					split due to the Docker size limit. For everything else, it
					is recommended to use the [`reduce`
					transform](\(urls.angle_reduce_transform)) which offers
		...
		pod_removal: {
			title: "Pod removal"
			body: """
				To ensure all data is collected, Angle will continue to collect logs from the
				Pod for some time after its removal. This ensures that Angle obtains some of
		...
		resource_limits: {
			...
			body: """
				Angle recommends the following resource limits.
				"""
			sub_sections: [
				{
					title: "Agent resource limits"
					body: """
						If deploy Angle as an agent (collecting data for each of your
						...
						Angle pipeline is complex, you may need more resources; if you
			...
			sub_sections: [
				{
					...
					body: """
						For the agent role, Angle stores its state at the host-mapped dir with a static
		...
		testing_and_reliability: {
			...
			body: """
				Angle is tested extensively against Kubernetes. In addition to Kubernetes
				being Angle's most popular installation method, Angle implements a
		...
		kubernetes_api_access_control: {
			...
			body:  """
				Angle requires access to the Kubernetes API.
				Specifically, the [`kubernetes_logs` source](\(urls.angle_kubernetes_logs_source))
				...
				scheme. RBAC-enabled clusters require some configuration to grant Angle
				...
				configuration - Angle will just work.

				Clusters using legacy ABAC scheme are not officially supported
				(although Angle might work if you configure access properly) -
				we encourage switching to RBAC. If you use a custom access control
				scheme - make sure Angle's Pod/ServiceAccount is granted `list` and `watch` access
				to the `/api/v1/pods`, `/api/v1/namespaces`, and `/api/v1/nodes` resources.

				[serviceaccount]: https://github.com/khulnasoft/angle/blob/master/distribution/kubernetes/angle-agent/serviceaccount.yaml
				[rbac]: https://github.com/khulnasoft/angle/blob/master/distribution/kubernetes/angle-agent/rbac.yaml
				[rbac_helm]: https://github.com/khulnasoft/helm-charts/blob/develop/charts/angle/templates/rbac.yaml
				"""
		}
	}
