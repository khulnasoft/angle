...
components: sources: fluent: {
	...
	how_it_works: {
		aggregator: {
			title: "Sending data from fluent agents to Angle aggregators"
			body: """
				If you are already running fluent agents (Fluentd or Fluent Bit) in your infrastructure, this source can
				make it easy to start getting that data into Angle.
				"""
		}
		...
		fluentd_configuration: {
			...
			body: """
				To configure Fluentd to forward to a Angle instance, you can use the following output configuration:
		...
		fluentbit_configuration: {
			...
			body: """
				To configure Fluent Bit to forward to a Angle instance, you can use the following output configuration:
		...
		secure_mode: {
			...
			body:  """
				...
				If you would find this useful, [please let us know](\(urls.angle_repo)/issues/7532).
				"""
		}
	}
}
