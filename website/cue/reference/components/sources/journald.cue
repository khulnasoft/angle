...
components: sources: journald: {
	...
	how_it_works: {
		communication_strategy: {
			...
			body:  """
				To ensure the `journald` source works across all platforms, Angle interacts
				with the systemd journal via the `journalctl` command. This is accomplished by
				spawning a [subprocess](\(urls.rust_subprocess)) that Angle interacts
				...
				[issue #1473](\(urls.angle_issues)/1473).
				"""
		}
		non_ascii: {
			...
			body: """
				...
				loss. Angle handles this alternate format by translating such messages
