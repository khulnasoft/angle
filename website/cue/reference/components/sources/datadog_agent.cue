...
components: sources: datadog_agent: {
	...
	how_it_works: {
		decompression: {
			...
			body:  """
				Sending logs or metrics to Angle requires the [Datadog Agent](\(urls.datadog_agent_doc)) v7.35/6.35 or greater.
				...
				angle:
					logs.enabled: true
					logs.url: http://"<ANGLE_HOST>:<SOURCE_PORT>" # Use https if SSL is enabled in Angle source configuration
				```
				...
				angle:
					...
					metrics.url: http://"<ANGLE_HOST>:<SOURCE_PORT>" # Use https if SSL is enabled in Angle source configuration
				```
				...
				angle:
					traces.enabled: true
					traces.url: http://"<ANGLE_HOST>:<SOURCE_PORT>" # Use https if SSL is enabled in Angle source configuration
				```
				"""
		}
		trace_support: {
			...
			body: """
				...
				disable any sampling of traces within the Datadog Agent or client SDKs as Angle
