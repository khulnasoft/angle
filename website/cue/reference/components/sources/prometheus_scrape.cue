...
components: sources: prometheus_scrape: {
	...
	configuration: base.components.sources.prometheus_scrape.configuration & {
		endpoints: warnings: ["You must explicitly add the path to your endpoints. Angle will _not_ automatically add `/metrics`."]
	}
	...
	how_it_works: {
		duplicate_tag_names: {
			...
			body: """
				...
				itself will reject a metric with duplicate tags. Angle will accept the metric,
