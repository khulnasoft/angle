...
components: sources: logstash: {
	...
	how_it_works: {
		aggregator: {
			title: "Sending data from logstash agents to Angle aggregators"
			body: """
				If you are already running an Elastic agent (Logstash or Elastic Beats) in your infrastructure, this
				source can make it easy to start getting that data into Angle.
				"""
		}
		...
		logstash_configuration: {
			...
			body: """
				To configure Logstash to forward to a Angle instance, you can use the following output configuration:

				```text
				output {
						lumberjack {
								# update these to point to your angle instance
								hosts => ["127.0.0.1"]
								port => 5044
		...
		beats_configuration: {
			...
			body: """
				To configure one of the Elastic Beats agents to forward to a Angle instance, you can use the following
		...
		acking: {
			...
			body: """
				...
				component in the topology. In the future, this source will utilize Angle's support for end-to-end
				acknowledgements.
				"""
		}
	}
