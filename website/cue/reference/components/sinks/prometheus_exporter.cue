	...
	support: {
		...
		warnings: [
			"""
				...
				and reliability problems. You should consider alternative strategies to reduce the cardinality. Angle
				offers a [`tag_cardinality_limit` transform](\(urls.angle_transforms)/tag_cardinality_limit) as a way
	...
	how_it_works: {
		...
		memory_usage: {
			...
			body: """
				...
				fails to scrape the Angle instance over an extended period of time. The
				downside is that data will be lost if Angle is restarted. This is by design of
				Prometheus' pull model approach, but is worth noting if restart Angle
				frequently.
				"""
		}
		...
		duplicate_tag_names: {
			...
			body: """
				...
				values for each name, Angle will only send the last value specified.
				"""
		}
	}
