		...
		how_it_works: {
			gcp_authentication: {
				...
				body:  """
						...
						Angle is concerned with the [server to server methods](\(urls.gcp_authentication_server_to_server))
						...
						1. Finally, Angle will check for an [instance service account](\(urls.gcp_authentication_service_account)).

						If credentials aren't found, Angle's health checks fail and an error is
						[logged](\(urls.angle_monitoring)).
						"""
			}
		}
	}
