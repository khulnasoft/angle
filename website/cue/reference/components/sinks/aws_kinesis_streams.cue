...
components: sinks: aws_kinesis_streams: components._aws & {
	...
	how_it_works: {
		partitioning: {
			title: "Partitioning"
			body:  """
				By default, Angle issues random 16 byte values for each
				...
				nicely with the [`remap` transform](\(urls.angle_remap_transform)), which enables you
			...
			sub_sections: [
				{
					...
					body:  """
						...
						[`warning`-level log event](\(urls.angle_monitoring)) is logged. The field
				...
				},
				{
					...
					body: """
						...
						Angle will slice the value and use the first 256 characters.
						"""
				},
				{
					title: "Non-string partition keys"
					body: """
						Angle will coerce the value into a string.
						"""
				},
			]
		}
