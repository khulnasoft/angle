...
components: sinks: aws_s3: components._aws & {
	...
	how_it_works: {
		cross_account: {
			...
			body:  """
				If you're using Angle to write objects across AWS accounts then you should
		...
		object_acl: {
			...
			body:  """
				...
				objects. In the context of Angle, only object ACLs are relevant (Angle does
		...
		object_naming: {
			title: "Object naming"
			body:  """
				Angle uses two different naming schemes for S3 objects. If you set the
				[`compression`](#compression) parameter to `true` (this is the default), Angle uses
				this scheme:
				...
				If compression isn't enabled, Angle uses this scheme (only the file extension
				is different):
				...
				Angle appends a [UUIDV4](\(urls.uuidv4)) token to ensure there are no naming
				conflicts in the unlikely event that two Angle instances are writing data at the
				same time.
		...
		object_tags_and_metadata: {
			...
			body:  """
				Angle currently only supports [AWS S3 object tags](\(urls.aws_s3_tags)) and does
		...
		server_side_encryption: {
			...
			body:  """
				...
				of Angle only the object level is relevant (Angle does not create or modify
		...
		storage_class: {
			...
			body:  """
				...
				level. In the context of Angle only the object level is relevant (Angle does
