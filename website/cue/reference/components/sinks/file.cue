...
components: sinks: file: {
	...
	how_it_works: {
		dir_and_file_creation: {
			...
			body: """
				Angle will attempt to create the entire directory structure
				and the file when emitting events to the file sink. This
				requires that the Angle agent have the correct permissions
		...
		durability: {
			...
			body: """
				Angle makes no attempt to ensure the files output by
