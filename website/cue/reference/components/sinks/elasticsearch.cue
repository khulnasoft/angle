...
components: sinks: elasticsearch: {
	...
	support: {
		requirements: [
			#"""
				Elasticsearch's Data streams feature requires Angle to be configured with the `create` `bulk.action`.
	...
	how_it_works: {
		conflicts: {
			title: "Conflicts"
			body:  """
				Angle [batches](#buffers-and-batches) data and flushes it to Elasticsearch's
		...
		data_streams: {
			title: "Data streams"
			body:  """
				By default, Angle uses the `index` action with Elasticsearch's Bulk API.
		...
		distribution: {
			...
			body: """
				...
				Health of endpoints is actively monitored and if an endpoint is deemed unhealthy, Angle will stop sending events to it
