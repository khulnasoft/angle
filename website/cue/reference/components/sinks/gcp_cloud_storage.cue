	...
	how_it_works: {
		object_access_control_list: {
			...
			body:  """
					...
					objects. In the context of Angle, only object ACLs are relevant (Angle
		...
		object_naming: {
			title: "Object naming"
			body:  """
				By default, Angle names your GCS objects in accordance with one of two formats.
				...
				Angle appends a [UUIDV4](\(urls.uuidv4)) token to ensure there are no name
				conflicts in the unlikely event that two Angle instances are writing data at the same
				time.
		...
		storage_class: {
			...
			body:  """
					...
					object level. In the context of Angle only the object level is relevant
					(Angle does not create or modify buckets). You can set the storage
		...
		tags_and_metadata: {
			title: "Tags & Metadata"
			body:  """
					Angle supports adding [custom metadata](\(urls.gcs_custom_metadata)) to
