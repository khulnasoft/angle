...
components: sources: [Name=string]: {
		...
		if !features.auto_generated {
			if features.collect != _|_ {
				if features.collect.checkpoint.enabled {
					data_dir: {
						common:      false
						description: "The directory used to persist file checkpoint positions. By default, the global `data_dir` option is used. Please make sure the Angle project has write permissions to this dir."
						required:    false
						type: string: {
							default: null
							examples: ["/var/lib/angle"]
								...
								type: string: {
									...
									enum: {
										...
										syslog: """
											...
											"""
										native: """
											Decodes the raw bytes as Angle’s [native Protocol Buffers format][angle_native_protobuf].

											This codec is **[experimental][experimental]**.

											[angle_native_protobuf]: https://github.com/khulnasoft/angle/blob/master/lib/angle-core/proto/event.proto
											[experimental]: https://angle.khulnasoft.com/highlights/2022-03-31-native-event-codecs
											"""
										native_json: """
											Decodes the raw bytes as Angle’s [native JSON format][angle_native_json].

											This codec is **[experimental][experimental]**.

											[angle_native_json]: https://github.com/khulnasoft/angle/blob/master/lib/codecs/tests/data/native_encoding/schema.cue
											[experimental]: https://angle.khulnasoft.com/highlights/2022-03-31-native-event-codecs
											"""
									}
								}
				...
				if features.encoding.enabled {
					encoding: {
						...
						type: object: options: {
							charset: {
								common:      false
								description: "Encoding of the source messages. Takes one of the encoding [label strings](\(urls.encoding_charset_labels)) defined as part of the [Encoding Standard](\(urls.encoding_standard)). When set, the messages are transcoded from the specified encoding to UTF-8, which is the encoding angle assumes internally for string-like data. Enable this transcoding operation if you need your data to be in UTF-8 for further processing. At the time of transcoding, any malformed sequences (that can't be mapped to UTF-8) will be replaced with [replacement character](\(urls.unicode_replacement_character)) and warnings will be logged."
								required:    false
								type: string: {
									default: null
	...
	output: {
		logs?: [Name=string]: {
			fields: {
				_current_timestamp: {
					description: string | *"The exact time the event was ingested into Angle."
					required:    true
					type: timestamp: {}
				}
					...
					type: object: {
						options: {
							subject: {
								...
								type: string: {
									default: null
									examples: [ "CN=localhost,OU=Angle,O=Datadog,L=New York,ST=New York,C=US"]
								}
							}
						}
					}
	...
	how_it_works: {
		_tls: {
			...
			body:  """
				Angle uses [OpenSSL](\(urls.openssl)) for TLS protocols due to OpenSSL's maturity. You can
		...
		if features.collect != _|_ {
			if features.collect.checkpoint.enabled {
				checkpointing: {
					title: "Checkpointing"
					body: """
						Angle checkpoints the current read position after each
						successful read. This ensures that Angle resumes where it left
