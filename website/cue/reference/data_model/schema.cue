...
data_model: schema: {
	log: {
		common: true
		description: """
			A Angle log event is a structured representation of a
			point-in-time event. It contains an arbitrary set of
			fields that describe the event.
			A key tenet of Angle is to remain schema neutral. This
			ensures that Angle can work with any schema, supporting
			legacy and future schemas as your needs evolve. Angle
	...
	metric: {
		common: true
		description: """
			A Angle metric event represents a numerical operation
			performed on a time series. Unlike other tools, metrics
			in Angle are first class citizens, they are not represented
			as structured logs. This makes them interoperable with
			various metrics services without the need for any
			transformation.

			Angle's metric data model favors accuracy and correctness over
			ideological purity. Therefore, Angle's metric types are a
			...
			options: {
				...
				gauge: {
					...
					description: """
						...
						that can increase and decrease. Angle's
	...
	trace: {
		common: true
		description: """
			A Angle trace event is a vendor agnostic trace representation.
			It is similar to a Angle log event but it contains a list of spans
			"""
		required: false
		type: object: {
			examples: [
