remap: features: compilation: {
	...
	characteristics: {
		fail_safety_checks: {
			...
			description: """
				At compile time, Angle performs [fail safety](#fail-safety) checks to ensure that
				...
				malformed data—a superior alternative to being surprised by such issues when Angle is already handling
		...
		type_safety_checks: {
			...
			description: """
				At compile time, Angle performs [type safety](#type-safety)) checks to catch runtime
