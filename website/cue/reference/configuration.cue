...
configuration: {
	configuration: {
		...
		data_dir: {
			common: false
			description: """
				The directory used for persisting Angle state, such
				as on-disk buffers, file checkpoints, and more.
				Please make sure the Angle project has write
				permissions to this directory.
				"""
			required: false
			type: string: {
				default: "/var/lib/angle/"
				examples: ["/var/lib/angle", "/var/local/lib/angle/", "/home/user/angle/"]
			}
		}
		...
		expire_metrics: {
			common: false
			description: """
				If set, Angle will configure the internal metrics system to automatically
		...
		enrichment_tables: {
			...
			description: """
				...
				[`remap`](\(urls.angle_remap_transform)) transform. Currently supported are:
					...
					type: object: options: {
						path: {
							description: """
								...
								"""
							warnings: [
								"In order to be used by Angle, you need to assign read access to the enrichment table file.",
							]
							required: true
							type: string: {
								examples: [
		...
		schema: {
			common: false
			description: """
				Configures options for how Angle handles event schema.
				"""
			required: false
			type: object: {
				examples: []
		...
		telemetry: {
			common: false
			description: """
				Configures options for how Angle emits telemetry.
				"""
			required: false
			type: object: {
				examples: []
				options: {
					tags: {
						required: false
						description: """
							Controls which tags should be included with the `angle_component_sent_events_total` and
							`angle_component_sent_event_bytes_total` metrics.
							"""
						type: object: {
							examples: []
							options: {
		...
		log_schema: {
			...
			description: """
				...
				Angle components to assign the fields on incoming
				events.
		...
		secret: {
			...
			description: """
				...
				in Angle config. Currently, only the exec backend is supported. Multiple backends can be configured. To signify
				Angle that it should look for a secret to retrieve use the `SECRET[<backend_name>.<secret_key>]`. This placeholder
			...
			type: object: options: {
				exec: {
					...
					description: """
						...
						Angle will log the errors and exit.

						Secrets will be loaded when Angle starts or if Angle receives a `SIGHUP` signal triggering its
					...
					type: object: options: {
						...
						timeout: {
							description: "The amount of time Angle will wait for the command to complete."
							required:    false
							common:      false
							type: uint: {
								default: 5
		...
		proxy: {
			common:      false
			description: "Configures an HTTP(S) proxy for Angle to use."
			required:    false
			type: object: options: {
				enabled: {
					common:      false
	...
	how_it_works: {
		environment_variables: {
			...
			body: """
				Angle interpolates environment variables within your configuration file
				with the following syntax:

				```toml title="angle.yaml"
				transforms:
					add_host:
						inputs: ["apache_logs"]
						type: "remap"
		...
		secrets_management: {
			...
			body: """
				Angle can retrieve secrets like a password or token by querying an external system in order to
				avoid storing sensitive information in Angle configuration files. Secret backends used to retrieve
				...
				environment variables interpolation. While Angle supports multiple commands to retrieve secrets, a
				...
				In that example Angle will retrieve the `dd_api_key` from `backen_1` and `splunk_token` from `backend_2`.
				"""
			...
			sub_sections: [
				{
					...
					body:  """
						When using the `exec` type for a secret backend Angle and the external command are communicating using
						the standard input and output. The communication is using plain text JSON. Angle spawns the specified
						...
						logged by Angle and can be used for troubleshooting.

						The JSON object used for communication between an `exec` backend and Angle shall comply with the
						...
						given secret Angle will refuse to load the configuration.

						Currently Angle will always query backend with `"version": "1.0"`.
						"""
				},
			]
		}
		formats: {
			title: "Formats"
			body:  """
				Angle supports [YAML](\(urls.yaml)), [TOML](\(urls.toml)), and [JSON](\(urls.json)) to
				ensure Angle fits into your workflow. A side benefit of supporting YAML and JSON is that they
		...
		location: {
			title: "Location"
			body: """
				The location of your Angle configuration file depends on your installation method. For most Linux
				based systems, the file can be found at `/etc/angle/angle.yaml`.

				All files in `/etc/angle` are user configuration files and can be safely overridden to craft your
				desired Angle configuration.
				"""
		}
		multiple: {
		...
		automatic_namespacing: {
			...
			body: """
				...
				For example, you can create the sink `foo` in the folder `/path/to/angle/config/sinks/foo.toml` and
				...
				For angle to find and load the different configuration files, you need to load your configuration with
				the `--config-dir` argument.

				```bash
				angle --config-dir /path/to/angle/config
				```
				"""
		}
		wildcards: {
			...
			body: """
				Angle supports wildcard characters (`*`) in component names when building your topology.
