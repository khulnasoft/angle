...
administration: {
	#Download: {
		...
		#Option: {
			...
			if _file_type != "deb" {
				download_url: "\(urls.angle_packages_root)/angle/{v1}/angle-{v2}-\(_version_postfix)\(filename)"
			}
			if _file_type == "deb" {
				download_url: "\(urls.angle_packages_root)/angle/{v1}/angle_{v2}-1_\(_version_postfix)\(filename)"
			}
