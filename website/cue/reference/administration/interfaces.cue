...
administration: {
	#Interface: {
		...
		role_implementations: {
			_systemd_commands: {
				logs:    "sudo journalctl -fu angle"
				reload:  "systemctl kill -s HUP --kill-who=main angle.service"
				restart: "sudo systemctl restart angle"
				start:   "sudo systemctl start angle"
				stop:    "sudo systemctl stop angle"
			}
		}
