...
administration: topologies: centralized: {
	...
	description: """
		...
		without the management overheard incurred by a stream-based setup, which often involves using Angle in
	...
	pros: [
		{
			...
			description: """
				Centralized topologies are typically more efficient for client nodes and downstream services. Angle
				[agents](\(urls.angle_agent_role)) do less work and thus use fewer resources. In addition, in this
				topology the centralized Angle service buffers data, provides better compression, and sends optimized
				requests downstream.
				"""
		},
		{
			title: "More reliable"
			description: """
				Angle protects downstream services from volume spikes by buffering and flushing data at smoothed-out
				intervals.
				"""
		},
		{
	...
	cons: [
		{
			title:       "More complex"
			description: """
				A centralized topology has more moving parts, as you need to run Angle in both the
				[agent](\(urls.angle_agent_role)) and [aggregator](\(urls.angle_aggregator_role)) roles.
				"""
		},
		{
			title:       "Less durable"
			description: """
				[Agent](\(urls.angle_agent_role)) nodes are designed to get data off of the machine as quickly as
				possible. While this is fine for some use cases, it does bear the possibility of data loss since the
				central Angle service could go down and thus lose any buffered data. If this type of outage is
