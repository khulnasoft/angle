...
//
// * `daemon` - Angle is installed as a single process on the host.
// * `sidecar` - Angle is installed alongside each process it is
//   monitoring. Therefore, there might be multiple Angle processes
//   on the host.
// * `aggregator` - Angle receives data from one or more upstream
...
// `#EventType` represents one of Angle's supported event types.
//
// * `log` - log event
// * `metric` - metric event
...
#TypeCondition: {
	...
	syntaxes: [#Syntax, ...#Syntax] & [
			...
			name:        "vrl"
			description: """
				A [Angle Remap Language](\(urls.vrl_reference)) (VRL) [Boolean
