...
/// Read the first line from angle logs and assert that it matches the expected
/// one.
...
pub async fn smoke_check_first_line(log_reader: &mut Reader) {
    ...
    let expected_pat = "INFO angle::app:";
    assert!(
        first_line.contains(expected_pat),
        "Expected a line ending with {:?} but got {:?}; angle might be malfunctioning",
        expected_pat,
        first_line
    );
}
