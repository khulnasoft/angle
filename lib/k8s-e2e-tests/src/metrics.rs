...
/// This helper function validates the presence of `angle_started`-ish metric.
pub fn extract_angle_started(metrics: &str) -> bool {
    metrics_regex().captures_iter(metrics).any(|captures| {
        let metric_name = &captures["name"];
        let value = &captures["value"];
        metric_name.contains("angle_started") && value == "1"
    })
}
...
/// This helper function performs an HTTP request to the specified URL and
/// validates the presence of `angle_started`-ish metric.
pub async fn assert_angle_started(url: &str) -> Result<(), Box<dyn std::error::Error>> {
    let metrics = load(url).await?;
    if !extract_angle_started(&metrics) {
        return Err(format!("`angle_started`-ish metric was not found:\n{}", metrics).into());
    }
    Ok(())
}

/// This helper function performs HTTP requests to the specified URL and
/// waits for the presence of `angle_started`-ish metric until the deadline
/// with even delays between attempts.
pub async fn wait_for_angle_started(
    url: &str,
    next_attempt_delay: std::time::Duration,
    deadline: std::time::Instant,
) -> Result<(), Box<dyn std::error::Error>> {
    loop {
        let err = match assert_angle_started(url).await {
            Ok(()) => break,
            Err(err) => err,
        };
        ...
        eprintln!(
            "Waiting for `angle_started`-ish metric to be available, next poll in {} sec, deadline in {} sec",
...
pub const SOURCE_COMPLIANCE_METRICS: &[&str] = &[
    "angle_component_received_events_total",
    "angle_component_received_event_bytes_total",
    "angle_component_sent_events_total",
    "angle_component_sent_event_bytes_total",
];
...
mod tests {
        ...
        let cases = vec![
            ...
            (
                vec![
                    ...
                    r#"angle_component_sent_events_total 3"#,
                ],
                1 + 3,
            ),
            ...
            (
                vec![
                    ...
                    r#"angle_component_sent_events_total 3 1607985729161"#,
                ],
                1 + 3,
            ),
        ];
    ...
    #[test]
    fn test_extract_angle_started() {
        let cases = vec![
            (vec![r#"angle_started 1"#], true),
            (vec![r#"angle_started_total 1"#], true),
            (vec![r#"angle_angle_started_total 1"#], true),
            (vec![r#""#], false),
            (vec![r#"other{} 1"#], false),
            // Real-world example.
            (
                vec![
                    r#"# HELP angle_started_total angle_started_total"#,
                    r#"# TYPE angle_started_total counter"#,
                    r#"angle_started_total 1"#,
                ],
                true,
            ),
            ...
            (
                vec![
                    r#"# HELP angle_started_total started_total"#,
                    r#"# TYPE angle_started_total counter"#,
                    r#"angle_started_total 1 1607985729161"#,
                ],
                true,
            ),
        ];
        ...
        for (input, expected_value) in cases {
            let input = input.join("\n");
            let actual_value = extract_angle_started(&input);
