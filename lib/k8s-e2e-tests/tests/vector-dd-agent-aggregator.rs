use indoc::indoc;
use k8s_e2e_tests::*;
use k8s_test_framework::{lock, namespace, test_pod, angle::Config as AngleConfig};
use serde_json::Value;

/// This test validates that angle-aggregator can deploy with the default
/// settings and a dummy topology.
#[tokio::test]
async fn datadog_to_angle() -> Result<(), Box<dyn std::error::Error>> {
    let _guard = lock();
    let namespace = get_namespace();
    let override_name = get_override_name(&namespace, "angle-aggregator");
    let angle_endpoint = &format!("{}.{}.svc.cluster.local", override_name, namespace);
...
"#},
    ...
    let _datadog_agent = framework
        .helm_chart(
            ...
            // AngleConfig is a generic config container
            AngleConfig {
