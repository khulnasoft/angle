...
const CUSTOM_RESOURCE_ANGLE_CONFIG: &str = indoc! {r#"
    apiVersion: v1
    kind: ConfigMap
    metadata:
      name: angle-agent-config
    data:
      angle.yaml: |
...
/// This test validates that angle picks up logs at the simplest case
...
async fn default_agent() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let override_name = get_override_name(&namespace, "angle-agent");
    ...
    // Wait that `angle_started`-ish metric is present.
    ...
    let angle_pod = framework
        .get_angle_pod_with_pod(&pod_namespace, "test-pod", &namespace, &override_name)
        .await?;

    let mut log_reader = framework.logs(&namespace, &format!("pod/{}", angle_pod))?;
    ...
    metrics::assert_metrics_present(&angle_metrics_url, metrics::SOURCE_COMPLIANCE_METRICS)
        .await?;
...
/// This test validates that angle properly merges a log message that
...
/// This test validates that angle picks up preexisting logs - logs that
/// existed before angle was deployed.
#[tokio::test]
...
/// This test validates that angle picks up multiple log lines, and that
...
/// This test validates that angle properly annotates log events with pod
...
/// This test validates that angle properly filters out the logs that are
...
async fn pod_filtering() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let excluded_test_pod = framework
        .test_pod(test_pod::Config::from_pod(&make_test_pod_with_affinity(
            ...
            vec![("angle.khulnasoft.com/exclude", "true")],
            vec![],
    ...
    // to be confident that angle should've picked up the excluded message
    ...
    loop {
        ...
        if !line.starts_with('{') {
            // This isn't a json, must be an entry from Angle's own log stream.
            continue;
        }
        ...
        tokio::spawn(async move {
            // Wait for two minutes - a reasonable time for angle internals to
            ...
            // logs to appear to have high confidence that Angle has enough
...
/// This test validates that angle properly filters out the logs by the
...
async fn custom_selectors() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let label_sets = vec![
        ("test-pod-excluded-1", vec![("angle.khulnasoft.com/exclude", "true")]),
        (
            "test-pod-excluded-2",
...
/// This test validates that angle properly filters out the logs from
...
async fn container_filtering() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_pod = framework
        .test_pod(test_pod::Config::from_pod(&make_test_pod_with_containers(
            ...
            "test-pod",
            vec![],
            vec![("angle.khulnasoft.com/exclude-containers", "excluded")],
            None,
            vec![
    ...
    loop {
        ...
        let mut stop_tx = stop_tx.clone();
        tokio::spawn(async move {
            // Wait for 30 seconds - a reasonable time for angle internals to
...
/// This test validates that angle properly filters out the logs matching
...
/// This test validates that angle properly collects logs from multiple
...
/// This test validates that angle helm chart properly allows
    ...
    let angle = framework
        .helm_chart(
            ...
            AngleConfig {
                ...
                custom_resource: CUSTOM_RESOURCE_ANGLE_CONFIG,
            },
        )
        .await?;
    framework
...
/// This test validates that angle properly exposes metrics in
...
async fn metrics_pipeline() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // We give Angle some reasonable time to perform this initial bootstrap,
    // and capture the `processed_events` value afterwards.
    debug!("Waiting for Angle bootstrap");
    tokio::time::sleep(std::time::Duration::from_secs(30)).await;
    debug!("Done waiting for Angle bootstrap");

    // Capture events processed before deploying the test pod.
    let processed_events_before =
        metrics::get_component_sent_events_total(&angle_metrics_url).await?;
...
/// This test validates that angle chart properly exposes host metrics
/// out of the box.
#[tokio::test]
async fn host_metrics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    metrics::assert_metrics_present(&angle_metrics_url, metrics::HOST_METRICS).await?;
...
async fn simple_checkpoint() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let angle = framework
        ...
        .await?;
    framework
        .wait_for_rollout("test-angle", "daemonset/angle", vec!["--timeout=60s"])
        .await?;

    let test_namespace = framework
        .namespace(namespace::Config::from_namespace(
            &namespace::make_namespace("test-angle-test-pod".to_string(), None),
        )?)
        .await?;
    ...
    let test_pod = framework
        .test_pod(test_pod::Config::from_pod(&make_test_pod(
            ...
            // then ensure we just read the new marker after restarting Angle
    ...
    let mut log_reader = framework.logs("test-angle", "daemonset/angle")?;
    smoke_check_first_line(&mut log_reader).await;

    // Read the rest of the log lines.
    let mut got_marker = false;
    look_for_log_line(&mut log_reader, |val| {
        if val["kubernetes"]["pod_namespace"] != "test-angle-test-pod" {
            // A log from something other than our test pod, pretend we don't
            // see it.
            return FlowControlCommand::GoOn;
        }
    ...
    framework
        .restart_rollout("test-angle", "daemonset/angle", vec![])
        .await?;
