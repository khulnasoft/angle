...
fn get_angle_toml_path() -> PathBuf {
    let path = fs::canonicalize(env::var("CARGO_MANIFEST_DIR").unwrap()).unwrap();

    // Remove the "lib/angle-vrl/web-playground" suffix
    let parent_path = path
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent());
    parent_path
        .expect("Failed to find angle repo root")
        .join("Cargo.toml")
        .to_path_buf()
}
...
fn write_angle_constants(output_file: &mut File) {
    // TODO: For releases, we should use the manifest.package().version().
    // https://github.com/khulnasoft/angle/issues/18425
    let angle_git_sha = get_git_hash();
    let angle_version_statement = create_const_statement("ANGLE_VERSION", &angle_git_sha);
    output_file
        .write_all(angle_version_statement.as_bytes())
        .expect("Failed to write Angle version constant");

    let angle_link = format!("https://github.com/khulnasoft/angle/tree/{angle_git_sha}");
    let angle_link_statement = create_const_statement("ANGLE_LINK", &angle_link);
    output_file
        .write_all(angle_link_statement.as_bytes())
        .expect("Failed to write Angle version constant");
}
...
fn write_build_constants(manifest: &Manifest, dest_path: &Path) -> io::Result<()> {
    ...
    write_angle_constants(&mut output_file);
...
fn main() {
    let manifest =
        Manifest::from_path(get_angle_toml_path()).expect("Failed to load Angle Cargo.toml");
