...
pub struct ProtobufDeserializerConfig {
    ...
    #[serde(
        default,
        skip_serializing_if = "angle_core::serde::skip_serializing_if_default"
    )]
...
impl ProtobufDeserializerConfig {
    ...
    pub fn build(&self) -> angle_common::Result<ProtobufDeserializer> {
...
impl TryFrom<&ProtobufDeserializerConfig> for ProtobufDeserializer {
    ...
    fn try_from(config: &ProtobufDeserializerConfig) -> angle_common::Result<Self> {
...
) -> angle_common::Result<vrl::value::Value> {
    let vrl_value = match prost_reflect_value {
        ...
        prost_reflect::Value::List(v) => {
            let vec = v
                ...
                .collect::<Result<Vec<_>, angle_common::Error>>()?;
            ...
            if let Some(field_descriptor) = field_descriptor {
                ...
                vrl::value::Value::from(
                    v.iter()
                        ...
                        .collect::<angle_common::Result<ObjectMap>>()?,
                )
            } else {
                Err("Expected valid field descriptor")?
            }
...
mod tests {
    ...
    use std::path::PathBuf;
    use std::{env, fs};
    use angle_core::config::log_schema;
    ...
    fn parse_and_validate(
        ...
        for namespace in [LogNamespace::Legacy, LogNamespace::Angle] {
