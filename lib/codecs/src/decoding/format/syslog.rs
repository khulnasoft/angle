...
use angle_core::config::{LegacyKey, LogNamespace};
use angle_core::{
...
impl SyslogDeserializerConfig {
    ...
    pub fn schema_definition(&self, log_namespace: LogNamespace) -> schema::Definition {
        match (log_namespace, self.source) {
            ...
            (LogNamespace::Angle, Some(source)) => {
...
pub struct SyslogDeserializer {
    ...
    /// differently when using the Angle lognamespace.
...
impl Deserializer for SyslogDeserializer {
    fn parse(
        ...
        let log = match (self.source, log_namespace) {
            (Some(source), LogNamespace::Angle) => {
...
mod tests {
    use super::*;
    use angle_core::config::{init_log_schema, log_schema, LogSchema};
    ...
    #[test]
    fn deserialize_syslog_angle_namespace() {
        init();
        ...
        let events = deserializer.parse(input, LogNamespace::Angle).unwrap();
        assert_eq!(events.len(), 1);
