...
/// Deserializer that builds `Event`s from a byte frame containing Angle's native JSON
/// representation.
...
impl Deserializer for NativeJsonDeserializer {
    fn parse(
        &self,
        bytes: Bytes,
        // LogNamespace is ignored because Angle owns the data format being consumed and as such there
