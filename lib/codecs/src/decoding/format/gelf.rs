...
//   of angle will still work with the new relaxed decoding.
...
impl GelfDeserializer {
    ...
    fn message_to_event(&self, parsed: &GelfMessage) -> angle_common::Result<Event> {
        ...
        if let Some(add) = &parsed.additional_fields {
            for (key, val) in add.iter() {
                ...
                if val.is_string() || val.is_number() {
                    let angle_val: Value = val.into();
                    log.insert(event_path!(key.as_str()), angle_val);
                } else {
                    let type_ = match val {
...
mod tests {
    ...
    use angle_core::{config::log_schema, event::Event};
    use vrl::value::Value;
