...
pub struct ProtobufSerializerOptions {
    ...
    #[configurable(metadata(docs::examples = "/etc/angle/protobuf_descriptor_set.desc"))]
...
fn convert_value_raw(
    ...
    match (value, kind) {
        ...
        _ => Err(format!("Cannot encode angle `{kind_str}` into protobuf `{kind:?}`",).into()),
    }
}

/// Convert a angle `Value` into a protobuf `Value`.
fn convert_value(
    ...
    if let Value::Array(a) = value {
        if field_descriptor.cardinality() == prost_reflect::Cardinality::Repeated {
            let repeated: Result<Vec<prost_reflect::Value>, angle_common::Error> = a
                .into_iter()
                .map(|v| convert_value_raw(v, &field_descriptor.kind()))
                .collect();
            Ok(prost_reflect::Value::List(repeated?))
        } else {
            Err("Cannot encode angle array into a non-repeated protobuf field".into())
        }
    } else {
...
/// Convert a angle object (`Value`) into a protobuf message.
///
/// This function can only operate on `Value::Object`s,
/// since they are the only field-based angle Value
/// and protobuf messages are defined as a collection of fields and values.
fn encode_message(
    message_descriptor: &MessageDescriptor,
    value: Value,
) -> Result<DynamicMessage, angle_common::Error> {
...
mod tests {
    ...
    fn test_encode_bytes() {
        ...
        let message = encode_message(
            ...
            Value::Object(BTreeMap::from([
                ("text".into(), Value::Bytes(Bytes::from("angle"))),
                ("binary".into(), Value::Bytes(bytes.clone())),
            ])),
        )
        .unwrap();
        assert_eq!(Some("angle"), mfield!(message, "text").as_str());
