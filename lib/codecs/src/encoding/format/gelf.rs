...
fn coerce_required_fields(mut log: LogEvent) -> angle_common::Result<LogEvent> {
    // returns Error for missing field
    fn err_missing_field(field: &str) -> angle_common::Result<()> {
...
fn coerce_field_names_and_values(
    mut log: LogEvent,
) -> angle_common::Result<(LogEvent, Vec<String>)> {
        ...
        for (field, value) in event_data.iter_mut() {
            match field.as_str() {
                ...
                _ => {
                    ...
                    // Prepending the underscore since angle adds fields such as 'source_type'
...
fn to_gelf_event(log: LogEvent) -> angle_common::Result<LogEvent> {
...
mod tests {
    ...
    use angle_core::event::{Event, EventMetadata};
    use vrl::btreemap;
