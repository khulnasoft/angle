...
use angle_core::event::Event;

/// Serialize a structured event into a byte frame.
pub trait Serializer:
    tokio_util::codec::Encoder<Event, Error = angle_common::Error> + DynClone + Debug + Send + Sync
{
}
...
impl<Encoder> Serializer for Encoder where
    Encoder: tokio_util::codec::Encoder<Event, Error = angle_common::Error>
        + Clone
        + Debug
        + Send
        + Sync
