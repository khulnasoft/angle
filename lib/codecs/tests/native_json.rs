...
use angle_core::buckets;
use angle_core::config::LogNamespace;
use angle_core::event::{Event, Metric};
use angle_core::event::{MetricKind, MetricValue};

fn assert_roundtrip(
    input_event: Event,
    serializer: &mut dyn Serializer<Error = angle_common::Error>,
    ...
    let events = deserializer.parse(bytes, LogNamespace::Angle).unwrap();
