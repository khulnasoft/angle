...
impl ToTokens for Format {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let format_tokens = match self {
            Format::Date => quote! { #angle_config::validation::Format::Date },
            Format::Time => quote! { #angle_config::validation::Format::Time },
            Format::DateTime => quote! { #angle_config::validation::Format::DateTime },
            Format::Duration => quote! { #angle_config::validation::Format::Duration },
            Format::Email => quote! { #angle_config::validation::Format::Email },
            Format::Hostname => quote! { #angle_config::validation::Format::Hostname },
            Format::Uri => quote! { #angle_config::validation::Format::Uri },
            Format::IPv4 => quote! { #angle_config::validation::Format::IPv4 },
            Format::IPv6 => quote! { #angle_config::validation::Format::IPv6 },
            Format::Uuid => quote! { #angle_config::validation::Format::Uuid },
            Format::Regex => quote! { #angle_config::validation::Format::Regex },
        };
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let validation_tokens = match self {
            Validation::KnownFormat(format) => {
                quote! { #angle_config::validation::Validation::KnownFormat(#format) }
            }
            Validation::Length { minimum, maximum } => {
                let min_tokens = option_as_token(*minimum);
                let max_tokens = option_as_token(*maximum);

                quote! { #angle_config::validation::Validation::Length { minimum: #min_tokens, maximum: #max_tokens } }
            }
            Validation::Range { minimum, maximum } => {
                let min_tokens = option_as_token(*minimum);
                let max_tokens = option_as_token(*maximum);

                quote! { #angle_config::validation::Validation::Range { minimum: #min_tokens, maximum: #max_tokens } }
            }
            Validation::Pattern(pattern) => {
                quote! { #angle_config::validation::Validation::Pattern(#pattern.to_string()) }
            }
        };
