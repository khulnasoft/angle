...
/// Generate the package name to reach `angle_config` in the output of the macros. This should be
/// `angle_lib::configurable` in all packages that can import `angle_lib` to allow for a single
/// import interface, but it needs to explicitly name `angle_config` in all packages that
/// themselves import `angle_lib`.
pub fn configurable_package_name_hack() -> proc_macro2::TokenStream {
    ...
    let running_in_lib = *RUNNING_IN_LIB.get_or_init(|| {
        ...
        package.starts_with("angle-") || package == "file-source" || package == "codecs"
    });
    if running_in_lib {
        syn::parse_quote! { ::angle_config }
    } else {
        syn::parse_quote! { ::angle_lib::configurable }
    }
}
