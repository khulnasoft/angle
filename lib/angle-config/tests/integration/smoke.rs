...
pub mod angle_v2 {
    ...
    use angle_config::{component::GenerateConfig, configurable_component};

    /// Configuration for version two of the `angle` source.
    #[configurable_component]
    #[derive(Clone, Debug)]
...
/// Marker type for the version two of the configuration for the `angle` source.
#[configurable_component]
#[derive(Clone, Debug)]
enum V2 {
...
#[configurable_component]
#[derive(Clone, Debug)]
pub struct AngleConfigV2 {
    /// Version of the configuration.
    version: Option<V2>,

    #[serde(flatten)]
    config: self::angle_v2::AngleConfig,
}

/// Configurable for the `angle` source.
#[configurable_component(source("angle"))]
#[derive(Clone, Debug)]
#[serde(untagged)]
pub enum AngleSourceConfig {
    /// Configuration for version two.
    V2(AngleConfigV2),
}

impl GenerateConfig for AngleSourceConfig {
    fn generate_config() -> toml::Value {
        let config = toml::Value::try_into::<self::angle_v2::AngleConfig>(
            self::angle_v2::AngleConfig::generate_config(),
        )
        .unwrap();
        toml::Value::try_from(AngleConfigV2 {
            version: Some(V2::V2),
            config,
        })
        .unwrap()
...
/// Collection of various sources available in Angle.
#[derive(Clone)]
#[configurable_component]
#[serde(tag = "type")]
pub enum SourceConfig {
    ...
    /// Angle source.
    Angle(AngleSourceConfig),
}

/// Collection of various sinks available in Angle.
#[derive(Clone)]
#[configurable_component]
#[serde(tag = "type")]
pub enum SinkConfig {
...
#[derive(Clone)]
#[configurable_component]
#[configurable(description = "Global options for configuring Angle.")]
pub struct GlobalOptions {
    /// The data directory where Angle will store state.
...
/// The overall configuration for Angle.
#[derive(Clone)]
#[configurable_component]
pub struct AngleConfig {
    #[configurable(derived)]
...
fn generate_semi_real_schema() {
    match generate_root_schema::<AngleConfig>() {
        Ok(schema) => {
