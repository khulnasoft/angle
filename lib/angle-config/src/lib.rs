...
// have fields with lifetime bounds in any of our configuration types in `angle`? :thinking:
//
...
pub use angle_config_macros::*;

// Re-export of both `Format` and `Validation` from `angle_config_common`.
//
// The crate exists so that both `angle_config_macros` and `angle_config` can import the types and work with them
// natively, but from a codegen and usage perspective, it's much cleaner to export everything needed to use
// `Configurable` from `angle_config` itself, and not leak out the crate arrangement as an impl detail.
pub use angle_config_common::{attributes, validation};
