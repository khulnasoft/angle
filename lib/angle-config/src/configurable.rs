...
/// A type that can be represented in a Angle configuration.
///
/// In Angle, we want to be able to generate a schema for our configuration such that we can have a Rust-agnostic
...
/// A pseudo-reference to a type that can be represented in a Angle configuration. This is
