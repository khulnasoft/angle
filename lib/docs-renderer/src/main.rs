...
use tracing::debug;
use angle_config::schema::parser::{component::ComponentSchema, query::SchemaQuerier};
use angle_config_common::constants::{self, ComponentType};

fn main() -> Result<()> {
    let querier = SchemaQuerier::from_schema("/tmp/angle-config-schema.json")
