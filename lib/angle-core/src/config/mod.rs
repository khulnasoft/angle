...
pub use angle_common::config::ComponentKey;
use angle_config::configurable_component;
use vrl::value::Value;

use crate::schema;

pub const MEMORY_BUFFER_DEFAULT_MAX_EVENTS: NonZeroUsize =
    angle_buffers::config::memory_buffer_default_max_events();
...
impl SourceOutput {
    ...
    /// A simple definition is just the default for the namespace. For the Angle namespace the
...
[global_acks]: https://angle.khulnasoft.com/docs/reference/configuration/global-options/#acknowledgements
[e2e_acks]: https://angle.khulnasoft.com/docs/about/under-the-hood/architecture/end-to-end-acknowledgements/"
)]
...
impl LogNamespace {
    /// Angle: This is added to "event metadata", nested under the source name.
    ///
    ...
    /// Angle: This is retrieved from the "event metadata", nested under the source name.
    ///
    ...
    /// Angle: The `ingest_timestamp`, and `source_type` fields are added to "event metadata", nested
    /// under the name "angle". This data will be marked as read-only in VRL.
    ///
    ...
    pub fn insert_standard_angle_source_metadata(
        &self,
        log: &mut LogEvent,
        source_name: &'static str,
        now: DateTime<Utc>,
    ) {
        self.insert_angle_metadata(
            log,
    ...
    /// Angle: This is added to the "event metadata", nested under the name "angle". This data
    /// will be marked as read-only in VRL.
    ///
    /// Legacy: This is stored on the event root, only if a field with that name doesn't already exist.
    pub fn insert_angle_metadata<'a>(
        &self,
        log: &mut LogEvent,
        ...
        match self {
            LogNamespace::Angle => {
                log.metadata_mut()
                    .value_mut()
                    .insert(path!("angle").concat(metadata_key), value);
            }
            LogNamespace::Legacy => {
    ...
    /// Angle: This is retrieved from the "event metadata", nested under the name "angle".
    ///
    /// Legacy: This is retrieved from the event.
    pub fn get_angle_metadata<'a, 'b>(
        &self,
        log: &'b LogEvent,
        legacy_key: impl ValuePath<'a>,
        metadata_key: impl ValuePath<'a>,
        ...
        match self {
            LogNamespace::Angle => log
                .metadata()
                .value()
                .get(path!("angle").concat(metadata_key)),
            LogNamespace::Legacy => log.get((PathPrefix::Event, legacy_key)),
        }
    }
    ...
    pub fn new_log_from_data(&self, value: impl Into<Value>) -> LogEvent {
        match self {
            LogNamespace::Angle | LogNamespace::Legacy => LogEvent::from(value.into()),
        }
    }
...
mod test {
    ...
    use angle_common::btreemap;
    use vrl::value::Kind;

    #[test]
    fn test_insert_standard_angle_source_metadata() {
        ...
        namespace.insert_standard_angle_source_metadata(&mut event, "source", Utc::now());
    ...
    #[test]
    fn test_source_definitons_angle() {
        let definition = schema::Definition::default_for_namespace(&[LogNamespace::Angle].into())
            .with_metadata_field(
                &owned_value_path!("angle", "zork"),
                Kind::integer(),
                Some("zork"),
            )
            .with_event_field(&owned_value_path!("nork"), Kind::integer(), None);
        ...
        valid_event
            .metadata_mut()
            .value_mut()
            .insert(path!("angle").concat("zork"), 32);
        ...
        invalid_event
            .metadata_mut()
            .value_mut()
            .insert(path!("angle").concat("zork"), "noog");
