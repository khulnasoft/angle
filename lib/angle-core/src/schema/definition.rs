...
impl Definition {
    ...
    pub fn any() -> Self {
        Self {
            ...
            log_namespaces: [LogNamespace::Legacy, LogNamespace::Angle].into(),
        }
    }
    ...
    pub fn default_for_namespace(log_namespaces: &BTreeSet<LogNamespace>) -> Self {
        ...
        let is_angle = log_namespaces.contains(&LogNamespace::Angle);
        match (is_legacy, is_angle) {
            (false, false) => Self::new_with_default_metadata(Kind::any(), []),
            (true, false) => Self::default_legacy_namespace(),
            (false, true) => Self::new_with_default_metadata(Kind::any(), [LogNamespace::Angle]),
            (true, true) => Self::any(),
        }
    }
    ...
    /// Adds the `source_type` and `ingest_timestamp` metadata fields, which are added to every Angle source.
    /// This function should be called in the same order as the values are actually inserted into the event.
    #[must_use]
    pub fn with_standard_angle_source_metadata(self) -> Self {
        self.with_angle_metadata(
    ...
    pub fn with_source_metadata(
        ...
        angle_path: &OwnedValuePath,
        kind: Kind,
        meaning: Option<&str>,
    ) -> Self {
        self.with_namespaced_metadata(source_name, legacy_path, angle_path, kind, meaning)
    }

    /// This should be used wherever `LogNamespace::insert_angle_metadata` is used to insert metadata.
    /// This automatically detects which log namespaces are used, and also automatically
    /// determines if there are possible conflicts from existing field names (usually from the selected decoder).
    /// This function should be called in the same order as the values are actually inserted into the event.
    #[must_use]
    pub fn with_angle_metadata(
        self,
    ...
    fn with_namespaced_metadata(
        ...
        let angle_definition = if self.log_namespaces.contains(&LogNamespace::Angle) {
            Some(self.clone().with_metadata_field(
                &angle_path.with_field_prefix(prefix),
                kind,
                meaning,
            ))
        } else {
        ...
        match (legacy_definition, angle_definition) {
    ...
    pub fn combine_log_namespaces(
        ...
        if log_namespaces.contains(&LogNamespace::Angle) {
            combined = combined.merge(angle);
        }
        combined
    }
...
mod tests {
    ...
    fn test_empty_legacy_field() {
        let definition = Definition::default_legacy_namespace().with_angle_metadata(
