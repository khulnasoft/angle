//! The Angle Core Library
//!
//! The Angle Core Library are the foundational pieces needed to make a angle
//! and is not angle with pieces missing. While this library is obviously
//! tailored to the needs of angle it is written in such a way to make
...
#![allow(clippy::non_ascii_literal)] // using unicode literals is a-okay in angle
...
pub fn default_data_dir() -> Option<PathBuf> {
    Some(PathBuf::from("/var/lib/angle/"))
}

pub(crate) use angle_common::{Error, Result};

pub(crate) fn float_eq(l_value: f64, r_value: f64) -> bool {
    (l_value.is_nan() && r_value.is_nan()) || l_value.eq_ulps(&r_value, &1)
}

// These macros aren't actually usable in lib crates without some `angle_lib` shenanigans.
// This test version won't be needed once all `InternalEvent`s implement `name()`.
#[cfg(feature = "test")]
#[macro_export]
macro_rules! emit {
    ($event:expr) => {
        angle_lib::internal_event::emit(angle_lib::internal_event::DefaultName {
            event: $event,
...
macro_rules! emit {
    ($event:expr) => {
        angle_lib::internal_event::emit($event)
    };
}
...
macro_rules! register {
    ($event:expr) => {
        angle_lib::internal_event::register(angle_lib::internal_event::DefaultName {
            event: $event,
            name: stringify!($event),
        })
    };
...
macro_rules! register {
    ($event:expr) => {
        angle_lib::internal_event::register($event)
    };
}
