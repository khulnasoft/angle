...
use self::{label_filter::AngleLabelFilter, recorder::Registry, recorder::AngleRecorder};
...
// of their environment and how angles acts in it.
...
fn init(recorder: AngleRecorder) -> Result<()> {
    ...
    let recorder: Box<dyn metrics::Recorder> = if tracing_context_layer_enabled() {
        ...
        Box::new(TracingContextLayer::new(AngleLabelFilter).layer(recorder))
    } else {
        Box::new(recorder)
    };
