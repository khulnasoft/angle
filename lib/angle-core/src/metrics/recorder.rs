...
use super::storage::AngleStorage;
...
pub(super) struct Registry {
    registry: MetricsRegistry<Key, GenerationalStorage<AngleStorage>>,
...
impl Registry {
    fn new() -> Self {
        Self {
            registry: MetricsRegistry::new(GenerationalStorage::new(AngleStorage)),
...
/// [`AngleRecorder`] is a [`metrics::Recorder`] implementation that's suitable
/// for the advanced usage that we have in Angle.
#[derive(Clone)]
pub(super) enum AngleRecorder {
    Global(Arc<Registry>),
    ThreadLocal,
}
...
impl Recorder for AngleRecorder {
