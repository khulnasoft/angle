...
/// Angle metadata is set to read-only to prevent it from being mutated
///
/// # Errors
...
pub fn compile_vrl(
    ...
    // Prevent mutating anything under the "angle" path in metadata.
    //
    ...
    config.set_read_only_path(OwnedTargetPath::metadata(owned_value_path!("angle")), true);
