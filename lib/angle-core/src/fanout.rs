...
use angle_buffers::topology::channel::BufferSender;
...
impl Fanout {
    ...
    async fn wait_for_replacements(&mut self) {
        while self.senders.values().any(Option::is_none) {
            if let Some(msg) = self.control_channel.recv().await {
                ...
                // sinks if Angle is stopping normally/gracefully, so like... we'd only get
    ...
    /// Angle's config reloading logic.
    ///
    /// # Errors
    ///
            ...
            tokio::select! {
                ...
                maybe_msg = self.control_channel.recv(), if control_channel_open => {
                    ...
                    match maybe_msg {
                        ...
                        None => {
                            // Control channel is closed, which means Angle is shutting down.
