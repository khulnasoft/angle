...
static ANGLE_SOURCE_TYPE_PATH: Lazy<Option<OwnedTargetPath>> = Lazy::new(|| {
...
impl LogEvent {
    ...
    /// of the read-only "angle" metadata, which only exists (and is required to exist)
    /// with the `Angle` log namespace.
    pub fn namespace(&self) -> LogNamespace {
        if self.contains((PathPrefix::Metadata, path!("angle"))) {
            LogNamespace::Angle
        } else {
            LogNamespace::Legacy
        }
...
impl LogEvent {
    /// Fetches the "message" path of the event. This is either from the "message" semantic meaning (Angle namespace)
    /// or from the message key set on the "Global Log Schema" (Legacy namespace).
    pub fn message_path(&self) -> Option<&OwnedTargetPath> {
        match self.namespace() {
            LogNamespace::Angle => self.find_key_by_meaning("message"),
            LogNamespace::Legacy => log_schema().message_key_target_path(),
        }
    }

    /// Fetches the "timestamp" path of the event. This is either from the "timestamp" semantic meaning (Angle namespace)
    /// or from the timestamp key set on the "Global Log Schema" (Legacy namespace).
    pub fn timestamp_path(&self) -> Option<&OwnedTargetPath> {
        match self.namespace() {
            LogNamespace::Angle => self.find_key_by_meaning("timestamp"),
            LogNamespace::Legacy => log_schema().timestamp_key_target_path(),
        }
    }

    /// Fetches the `host` path of the event. This is either from the "host" semantic meaning (Angle namespace)
    /// or from the host key set on the "Global Log Schema" (Legacy namespace).
    pub fn host_path(&self) -> Option<&OwnedTargetPath> {
        match self.namespace() {
            LogNamespace::Angle => self.find_key_by_meaning("host"),
            LogNamespace::Legacy => log_schema().host_key_target_path(),
        }
    }

    /// Fetches the `source_type` path of the event. This is either from the `source_type` Angle metadata field (Angle namespace)
    ...
    pub fn source_type_path(&self) -> Option<&OwnedTargetPath> {
        match self.namespace() {
            LogNamespace::Angle => ANGLE_SOURCE_TYPE_PATH.as_ref(),
            LogNamespace::Legacy => log_schema().source_type_key_target_path(),
        }
    }

    /// Fetches the `message` of the event. This is either from the "message" semantic meaning (Angle namespace)
    /// or from the message key set on the "Global Log Schema" (Legacy namespace).
    pub fn get_message(&self) -> Option<&Value> {
        match self.namespace() {
            LogNamespace::Angle => self.get_by_meaning("message"),
    ...
    /// Fetches the `timestamp` of the event. This is either from the "timestamp" semantic meaning (Angle namespace)
    /// or from the timestamp key set on the "Global Log Schema" (Legacy namespace).
    pub fn get_timestamp(&self) -> Option<&Value> {
        match self.namespace() {
            LogNamespace::Angle => self.get_by_meaning("timestamp"),
            LogNamespace::Legacy => log_schema()
    ...
    /// Removes the `timestamp` from the event. This is either from the "timestamp" semantic meaning (Angle namespace)
    /// or from the timestamp key set on the "Global Log Schema" (Legacy namespace).
    ...
    /// Fetches the `host` of the event. This is either from the "host" semantic meaning (Angle namespace)
    /// or from the host key set on the "Global Log Schema" (Legacy namespace).
    pub fn get_host(&self) -> Option<&Value> {
        match self.namespace() {
            LogNamespace::Angle => self.get_by_meaning("host"),
            LogNamespace::Legacy => log_schema()
    ...
    /// Fetches the `source_type` of the event. This is either from the `source_type` Angle metadata field (Angle namespace)
    /// or from the `source_type` key set on the "Global Log Schema" (Legacy namespace).
    pub fn get_source_type(&self) -> Option<&Value> {
        match self.namespace() {
            LogNamespace::Angle => self.get(metadata_path!("angle", "source_type")),
            LogNamespace::Legacy => log_schema()
...
mod test {
    ...
    // might be sending.
    #[test]
    fn json_value_to_angle_log_event_to_json_value() {
        ...
        std::fs::read_dir(FIXTURE_ROOT)
            ...
            .for_each(|fixture_file| match fixture_file {
                Ok(fixture_file) => {
                    ...
                    let angle_value = LogEvent::try_from(serde_value.clone()).unwrap();
                    let serde_value_again: serde_json::Value = angle_value.try_into().unwrap();
    ...
    fn merge_event_combines_values_accordingly() {
        ...
        angle_common::assert_event_data_eq!(merged, expected);
    }
