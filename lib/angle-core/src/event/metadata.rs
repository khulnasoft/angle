...
use angle_common::{byte_size_of::ByteSizeOf, config::ComponentKey, EventDataEq};
use vrl::{
...
pub struct EventMetadata {
    ...
    /// Only a small set of Angle sources and transforms explicitly set this field.
    #[serde(default)]
...
impl DatadogMetricOriginMetadata {
    ...
    /// When Angle sends out metrics containing the Origin metadata, it should do so with
...
fn default_schema_definition() -> Arc<schema::Definition> {
    Arc::new(schema::Definition::new_with_default_metadata(
        Kind::any(),
        [LogNamespace::Legacy, LogNamespace::Angle],
    ))
}
