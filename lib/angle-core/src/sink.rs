...
impl AngleSink {
    ...
    /// Run the `AngleSink` with a one-time `Vec` of `Event`s, for use in tests
    ///
    /// # Errors
    ///
    /// See `AngleSink::run` for errors.
    ...
    /// Converts `AngleSink` into a `futures::Sink`
    ///
    /// # Panics
    ///
    /// This function will panic if the self instance is not `AngleSink::Sink`.
    ...
    /// Converts `AngleSink` into a `StreamSink`
    ///
    /// # Panics
    ///
    /// This function will panic if the self instance is not `AngleSink::Stream`.
    ...
    /// Converts an event sink into a `AngleSink`
    ///
    /// Deprecated in favor of `AngleSink::from_event_streamsink`. See [angle/9261]
    /// for more info.
    ///
    /// [angle/9261]: https://github.com/khulnasoft/angle/issues/9261
    #[deprecated]
    pub fn from_event_sink(sink: impl Sink<Event, Error = ()> + Send + Unpin + 'static) -> Self {
        AngleSink::Sink(Box::new(EventSink::new(sink)))
    }

    /// Converts an event stream into a `AngleSink`
    pub fn from_event_streamsink(sink: impl StreamSink<Event> + Send + 'static) -> Self {
        let sink = Box::new(sink);
        AngleSink::Stream(Box::new(EventStream { sink }))
    }
}

impl fmt::Debug for AngleSink {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("AngleSink").finish()
    }
}
