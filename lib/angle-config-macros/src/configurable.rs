...
use angle_config_common::{configurable_package_name_hack, validation::Validation};
...
pub fn derive_configurable_impl(input: TokenStream) -> TokenStream {
    ...
    let angle_config = configurable_package_name_hack();
    ...
    if !generic_field_types.is_empty() {
        ...
        for typ in generic_field_types {
            let ty = &typ.ident;
            let predicate = parse_quote! { #ty: #angle_config::Configurable + ::serde::Serialize + #angle_config::ToValue };
    ...
    let configurable_impl = quote! {
        const _: () = {
            ...
            impl #impl_generics #angle_config::Configurable for #name #ty_generics #where_clause {
            ...
            impl #impl_generics #angle_config::ToValue for #name #ty_generics #where_clause {
                #to_value_fn
            }
        };
    };
...
fn build_metadata_fn(container: &Container<'_>) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn metadata() -> #angle_config::Metadata {
            #container_metadata
            #meta_ident
        }
    }
...
fn build_to_value_fn(_container: &Container<'_>) -> proc_macro2::TokenStream {
    let angle_config = configurable_package_name_hack();
    quote! {
        fn to_value(&self) -> #angle_config::serde_json::Value {
            #angle_config::serde_json::to_value(self)
...
fn build_virtual_newtype_schema_fn(virtual_ty: Type) -> proc_macro2::TokenStream {
    let angle_config = configurable_package_name_hack();
    quote! {
        fn generate_schema(schema_gen: &::std::cell::RefCell<#angle_config::schema::SchemaGenerator>) -> std::result::Result<#angle_config::schema::SchemaObject, #angle_config::GenerateError> {
            #angle_config::schema::get_or_generate_schema(
                &<#virtual_ty as #angle_config::Configurable>::as_configurable_ref(),
                schema_gen,
                None,
            )
        }
...
fn build_enum_generate_schema_fn(
    ...
    let generate_block = quote! {
        if #angle_config::schema::has_ambiguous_discriminants(&discriminant_map) {
            Ok(#angle_config::schema::generate_any_of_schema(&subschemas))
        } else {
            Ok(#angle_config::schema::generate_one_of_schema(&subschemas))
        }
    };
...
fn generate_struct_field(field: &Field<'_>) -> proc_macro2::TokenStream {
    ...
    let spanned_generate_schema = quote_spanned! {field.span()=>
        #angle_config::schema::get_or_generate_schema(
            &<#field_schema_ty as #angle_config::Configurable>::as_configurable_ref(),
            schema_gen,
...
fn generate_named_struct_field(
    ...
    let integrate_field = if field.flatten() {
        ...
        let spanned_is_optional = quote_spanned! {field.span()=>
            <#field_schema_ty as #angle_config::Configurable>::is_optional()
        };
...
fn build_named_struct_generate_schema_fn(
    ...
    quote! {
        fn generate_schema(schema_gen: &::std::cell::RefCell<#angle_config::schema::SchemaGenerator>) -> std::result::Result<#angle_config::schema::SchemaObject, #angle_config::GenerateError> {
            let mut properties = #angle_config::indexmap::IndexMap::new();
            let mut required = ::std::collections::BTreeSet::new();
            let mut flattened_subschemas = ::std::vec::Vec::new();

            let metadata = <Self as #angle_config::Configurable>::metadata();
            #(#mapped_fields)*

            let had_unflatted_properties = !properties.is_empty();

            let additional_properties = None;
            let mut schema = #angle_config::schema::generate_struct_schema(
                properties,
                required,
            ...
            if !flattened_subschemas.is_empty() {
                ...
                #angle_config::schema::convert_to_flattened_schema(&mut schema, flattened_subschemas);
            }
...
fn build_tuple_struct_generate_schema_fn(fields: &[Field<'_>]) -> proc_macro2::TokenStream {
    ...
    quote! {
        fn generate_schema(schema_gen: &::std::cell::RefCell<#angle_config::schema::SchemaGenerator>) -> std::result::Result<#angle_config::schema::SchemaObject, #angle_config::GenerateError> {
            ...
            Ok(#angle_config::schema::generate_tuple_schema(&subschemas))
        }
    }
}
...
fn generate_container_metadata(
    ...
    let angle_config = configurable_package_name_hack();
    quote! {
        let mut #meta_ident = #angle_config::Metadata::default();
        #maybe_title
        #maybe_description
        #maybe_default_value
        #maybe_deprecated
    ...
    let mapped_custom_attributes = custom_attributes
        .map(|attr| match attr {
            LazyCustomAttribute::Flag(key) => quote! {
                #meta_ident.add_custom_attribute(#angle_config::attributes::CustomAttribute::flag(#key));
            },
            LazyCustomAttribute::KeyValue { key, value } => quote! {
                #meta_ident.add_custom_attribute(#angle_config::attributes::CustomAttribute::kv(
                    #key, #value
                ));
            },
        });
...
fn generate_named_enum_field(field: &Field<'_>) -> proc_macro2::TokenStream {
    ...
    let angle_config = configurable_package_name_hack();
    let spanned_is_optional = quote_spanned! {field.span()=>
        <#field_ty as #angle_config::Configurable>::is_optional()
    };
    ...
    quote! {
        {
            ...
            #angle_config::schema::generate_struct_schema(
                properties,
                required,
                None
            )
...
fn generate_enum_variant_tag_schema(variant: &Variant<'_>) -> proc_macro2::TokenStream {
    ...
    quote! {
        {
            let mut tag_subschema = #angle_config::schema::generate_const_string_schema(#variant_name.to_string());
    ...
    let variant_schema = match variant.tagging() {
        ...
        Tagging::Internal { tag } => match variant.style() {
            ...
            Style::Newtype => {
                ...
                quote! {
                    let tag_schema = #angle_config::schema::generate_internal_tagged_variant_schema(#tag.to_string(), #tag_schema);
                    let mut flattened_subschemas = ::std::vec::Vec::new();
                    flattened_subschemas.push(tag_schema);

                    let mut newtype_schema = #newtype_schema;
                    #angle_config::schema::convert_to_flattened_schema(&mut newtype_schema, flattened_subschemas);
        ...
        Tagging::Adjacent { tag, content } => {
            ...
            quote! {
                let mut wrapper_properties = #angle_config::indexmap::IndexMap::new();
        ...
        Tagging::None => {
            ...
            match variant.style() {
                ...
                Style::Unit => quote! { #angle_config::schema::generate_null_schema() },
            }
        }
    };
...
fn generate_enum_variant_apply_metadata(variant: &Variant<'_>) -> proc_macro2::TokenStream {
    ...
    quote! {
        #variant_metadata
        #angle_config::schema::apply_base_metadata(&mut subschema, #variant_metadata_ref);
    }
}
...
fn generate_enum_variant_tag_apply_metadata(variant: &Variant<'_>) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        #angle_config::schema::apply_base_metadata(&mut tag_subschema, #variant_tag_metadata_ref);
    }
}
