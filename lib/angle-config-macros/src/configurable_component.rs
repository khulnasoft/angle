        ...
        self.component_name.as_ref().map(|component_name| {
            ...
            let desc_ty: syn::Type = match self.component_type {
                ComponentType::EnrichmentTable => {
                    parse_quote! { #angle_config::component::EnrichmentTableDescription }
                }
                ComponentType::Provider => {
                    parse_quote! { #angle_config::component::ProviderDescription }
                }
                ComponentType::Secrets => {
                    parse_quote! { #angle_config::component::SecretsDescription }
                }
                ComponentType::Sink => parse_quote! { #angle_config::component::SinkDescription },
                ComponentType::Source => {
                    parse_quote! { #angle_config::component::SourceDescription }
                }
                ComponentType::Transform => {
                    parse_quote! { #angle_config::component::TransformDescription }
                }
            };
    ...
    fn get_component_name_registration(&self) -> proc_macro2::TokenStream {
        ...
        match self.component_name.as_ref() {
            None => quote_spanned! {self.span=>
                #[derive(#angle_config::NamedComponent)]
                #[#helper_attr]
            },
...
struct Options {
    ...
    /// component in a Angle topology, versus simply what is allowed as a configurable "component"
    /// within a Angle configuration.
...
pub fn configurable_component_impl(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    //   which powers the `angle generate` subcommand by maintaining a name -> config type map
