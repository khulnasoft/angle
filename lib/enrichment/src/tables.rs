...
//! into Angle. Enrichment Tables go through two stages.
//!
//! ## 1. Writing
//!
...
//! tables are passed through Angles `Transform` components, particularly the
//! `Remap` transform. These Transforms are able to determine which fields we
//! will want to lookup whilst Angle is running. They can notify the tables of
...
impl TableRegistry {
    ...
    /// complicated. This can occur when the config is reloaded. Angle will be
