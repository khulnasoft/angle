...
use super::{
    ...
    test_pod, up_down, angle, wait_for_resource, wait_for_rollout, Interface, PortForwarder,
    Reader, Result,
};
...
impl Framework {
    ...
    pub async fn helm_chart(
        ...
        release_name: &str,
        helm_repo: &str,
        config: angle::Config<'_>,
    ) -> Result<up_down::Manager<angle::CommandBuilder>> {
        let env = vec![("CHART_REPO".to_owned(), helm_repo.to_owned())];
        let mut manager = angle::manager(
    ...
    /// Return the Angle pod that is deployed on the same node as the given pod. We want to make
    /// sure we are scanning the Angle instance that is deployed with the test pod.
    pub async fn get_angle_pod_with_pod(
        &self,
        pod_namespace: &str,
        pod_name: &str,
        angle_pod_namespace: &str,
        angle_pod_name: &str,
    ) -> Result<String> {
        let node = self
        ...
        Ok(self
            .get_pod_on_node(angle_pod_namespace, &node, angle_pod_name)
            .await
            .map_err(|_| "cant get the angle pod running on the test node")?)
    }
}
