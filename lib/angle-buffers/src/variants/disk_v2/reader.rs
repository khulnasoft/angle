...
use angle_common::{finalization::BatchNotifier, finalizer::OrderedFinalizer};
    ...
    T: Bufferable,
{
    ...
    /// The record is not compatible with this version of Angle.
    ///
    /// This can occur when records written to a buffer in previous versions of Angle are read by
    /// newer versions of Angle where the encoding scheme, or record schema, used in the previous
    /// version of Angle are no longer able to be decoded in this version of Angle.
    ...
    async fn read_length_delimiter(
        ...
        loop {
            ...
            if available >= 8 {
                ...
                // Angle is running on a 128-bit CPU in the future, storing records that are larger
                // than 2^64+1. :)
    ...
    FS::File: Unpin,
{
    ...
    ///
    /// In cases where Angle has restarted, but the reader hasn't yet finished a file, we would
    ...
    pub(super) async fn seek_to_next_record(&mut self) -> Result<(), ReaderError<T>> {
        ...
        // to the next data file, including reading acknowledging records within it.  If Angle
