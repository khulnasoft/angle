...
pub struct DiskBufferConfig<FS> {
    ...
    ///
    /// Must be unique from all other buffers, whether within the same process or other Angle
    ...
    ///
    /// In the event that data had not yet been durably written to disk, and Angle crashed, the
...
mod tests {
    ...
    proptest! {
        ...
        fn test_align16(input in 0..MAX_ALIGNABLE_AMOUNT) {
            ...
            // a future where we, or others, potentially compile Angle with an alternative compiler that does not
