...
use angle_common::finalizer::OrderedFinalizer;
...
pub enum LedgerLoadCreateError {
    ...
    /// The ledger is already opened by another Angle process.
    ///
    /// Advisory locking is used to prevent other Angle processes from concurrently opening the
    ...
    #[snafu(display(
        "failed to lock buffer.lock; is another Angle process running and using this buffer?"
    ))]
    LedgerLockAlreadyHeld,

    /// The ledger state was unable to be deserialized.
    ///
    /// This should only occur if the ledger file was modified or truncated out of the Angle
    ...
    FS::File: Unpin,
{
    ...
    pub(super) async fn load_or_create(
        ...
        // Acquire an exclusive lock on our lock file, which prevents another Angle process from
