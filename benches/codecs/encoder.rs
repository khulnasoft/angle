...
use angle::event::{Event, LogEvent};
use angle_common::{btreemap, byte_size_of::ByteSizeOf};
...
fn encoder(c: &mut Criterion) {
    ...
    group.throughput(Throughput::Bytes(input.size_of() as u64));
    group.bench_with_input("angle::codecs::Encoder::encode", &(), |b, ()| {
        b.iter_batched(
            || {
                angle::codecs::Encoder::<Framer>::new(
