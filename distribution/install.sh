...
# install Angle. It just does platform detection and execute the appropriate
# install method.
#
...
PACKAGE_ROOT="${PACKAGE_ROOT:-"https://packages.timber.io/angle"}"
ANGLE_VERSION="0.34.0"
_divider="--------------------------------------------------------------------------------"
_prompt=">>>"
_indent="   "
...
Community: https://angle.khulnasoft.com/community/
$_divider
...
usage() {
    cat 1>&2 <<EOF
angle-install
The installer for Angle (https://angle.khulnasoft.com)

USAGE:
    angle-install [FLAGS] [OPTIONS]

FLAGS:
    -y                      Disable confirmation prompt.
        --prefix            The directory where the files should be placed, default: "$HOME/.angle"
...
main() {
    ...
    # Confirm with the user before proceeding to install Angle through a
    # package manager. Otherwise, we install from an archive.
    if [ "$prompt" = "yes" ]; then
        echo "$_prompt We'll be installing Angle via a pre-built archive at https://packages.timber.io/angle/${ANGLE_VERSION}/"
        ...
        # Print a divider to separate the Angle installer output and the
...
install_from_archive() {
    ...
    case "$_arch" in
        ...
        aarch64-apple-darwin)
            # This if statement can be removed when Angle publishes aarch64-apple-darwin builds
    ...
    local _url="${PACKAGE_ROOT}/${ANGLE_VERSION}/angle-${ANGLE_VERSION}-${_archive_arch}.tar.gz"

    local _dir
    _dir="$(mktemp -d 2>/dev/null || ensure mktemp -d -t angle-install)"

    local _file="${_dir}/angle-${ANGLE_VERSION}-${_archive_arch}.tar.gz"

    ensure mkdir -p "$_dir"

    printf "%s Downloading Angle via %s" "$_prompt" "$_url"
    ...
    if [ "$use_new_directory_structure" = "no" ]; then
        ...
        # https://github.com/khulnasoft/angle/pull/13613#pullrequestreview-1045524132.
        ...
        local _unpack_dir
        _unpack_dir="$(mktemp -d 2>/dev/null || ensure mktemp -d -t angle-install)"
        ...
        ensure mkdir -p "$prefix/share/angle/config"
        ensure cp -r "$_unpack_dir/config/." "$prefix/share/angle/config"
        ensure cp "$_unpack_dir"/README.md "$prefix/share/angle/"
        ensure cp "$_unpack_dir"/LICENSE "$prefix/share/angle/"
    ...
    printf "%s To start Angle:\n" "$_prompt"
    printf "\n"
    printf "%s angle --config $prefix/config/angle.yaml\n" "$_indent"
    printf "\n"
    printf "%s More information at https://angle.khulnasoft.com/docs/\n" "$_prompt"
...
add_to_path() {
  ...
  printf "%s Adding Angle path to ${file}" "$_prompt"
...
get_gnu_musl_glibc() {
  ...
  # TODO: https://github.com/khulnasoft/angle/issues/10807
  local _ldd_version
